...
TriggerEvent('hhrp_society:registerSociety', 'mechanic', 'mechanic', 'society_mechanic', 'society_mechanic', 'society_mechanic', {type = 'public'})
...
-- Purchase Mech Shop:
HHCore.RegisterServerCallback('t1ger_mechanicjob:buyMechShop',function(source, cb, id, val, name)
...
-- Sell Mech Shop:
HHCore.RegisterServerCallback('t1ger_mechanicjob:sellMechShop',function(source, cb, id, val, sellPrice)
    local xPlayer = HHCore.GetPlayerFromId(source)
...
-- Reanme Mech Shop:
HHCore.RegisterServerCallback('t1ger_mechanicjob:renameMechShop',function(source, cb, id, val, name)
    local xPlayer = HHCore.GetPlayerFromId(source)
    MySQL.Async.fetchAll("SELECT shopID FROM t1ger_mechanic WHERE identifier = @identifier", {['@identifier'] = xPlayer.identifier}, function(data)
        if data[1].shopID ~= nil then 
            if data[1].shopID == id then
...
-- Get Employees:
HHCore.RegisterServerCallback('t1ger_mechanicjob:getEmployees',function(source, cb, id)
    local xPlayer = HHCore.GetPlayerFromId(source)
    local dataFected = false
    local shopEmployees = {}
            ...
            if #employees > 0 then 
                for k,v in pairs(employees) do 
                    if plyIdentifier == v.identifier then 
                        ...
                        local xTarget = HHCore.GetPlayerFromIdentifier(plyIdentifier)
                        xTarget.setJob("unemployed", 0)
                        TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xTarget.source, Lang['mech_employee_fired'])
                        break
                    end
                end
            end
                        ...
                        MySQL.Async.fetchAll('SELECT * FROM job_grades WHERE job_name = @job_name', {['@job_name'] = 'mechanic'}, function (results)
                            if newJobGrade >= 0 and newJobGrade <= results[#results].grade then
                                if xTarget.job.grade ~= newJobGrade then 
                                    ...
                                    Wait(200)
                                    TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xPlayer.source, (Lang['you_updat_job_grade_for']:format(xTarget.getName(), newJobGrade)))
                                    TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xTarget.source, (Lang['your_job_grade_updated']:format(newJobGrade)))
                                else
                                    TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xPlayer.source, (Lang['target_alrdy_has_job_g']:format(xTarget.getName())))
                                end
                            else
                                TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xPlayer.source, (Lang['mix_max_job_grade']:format(results[#results].grade)))
                            end
                        end)
                    end 
                end
...
HHCore.RegisterServerCallback('t1ger_mechanicjob:getOnlinePlayers', function(source, cb)
...
AddEventHandler('t1ger_mechanicjob:reqruitEmployee', function(id, plyIdentifier, newJobGrade, name)
    ...
    if identifierMatch then 
        TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xPlayer.source, Lang['already_hired'])
    end
    if loopDone then
        if noEmployees then
            MySQL.Async.fetchAll("SELECT * FROM t1ger_mechanic WHERE shopID = @shopID", {['@shopID'] = id}, function(data)
                for _,y in pairs(data) do 
                    ...
                    TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xPlayer.source, (Lang['you_recruited_x']:format(name)))
                    local xTarget = HHCore.GetPlayerFromIdentifier(plyIdentifier)
                    xTarget.setJob("mechanic", tonumber(newJobGrade))
                    TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xTarget.source, Lang['you_have_been_recruited'])
                    break
                end
            end)
        else
...
AddEventHandler('t1ger_mechanicjob:withdrawMoney', function(id, amount)
    ...
    MySQL.Async.fetchAll("SELECT money FROM t1ger_mechanic WHERE shopID = @shopID", {['@shopID'] = id}, function(data)
        ...
        if amount <= accountMoney then 
            ...
            TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xPlayer.source, (Lang['you_withdrew_x_amount']:format(amount)))
        else
            TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xPlayer.source, Lang['withdrawal_denied'])
        end
    end)
end)
...
AddEventHandler('t1ger_mechanicjob:depositMoney', function(id, amount)
    ...
    MySQL.Async.fetchAll("SELECT money FROM t1ger_mechanic WHERE shopID = @shopID", {['@shopID'] = id}, function(data)
        ...
        if plyMoney >= amount then 
            ...
            TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xPlayer.source, (Lang['you_deposited_x_amount']:format(amount)))
        else
            TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xPlayer.source, Lang['not_enough_money'])
        end
    end)
end)

-- Check Storage Access:
HHCore.RegisterServerCallback('t1ger_mechanicjob:checkAccess',function(source, cb, id)
    local xPlayer = HHCore.GetPlayerFromId(source)
    MySQL.Async.fetchAll("SELECT * FROM t1ger_mechanic WHERE shopID = @shopID", {['@shopID'] = id}, function(data)
...
-- Get User Inventory:
HHCore.RegisterServerCallback('t1ger_mechanicjob:getUserInventory', function(source, cb)
    local xPlayer = HHCore.GetPlayerFromId(source)
...
AddEventHandler('t1ger_mechanicjob:depositItem', function(item, amount, id)
    ...
    if xPlayer.getInventoryItem(addItem).count >= amount then
                ...
                if #storage > 0 then 
                    for k,v in ipairs(storage) do 
                        if v.item == addItem then
                            ...
                            if k == #storage then
                                if Config.ItemLabelHHCore then
                                    table.insert(storage, {item = addItem, count = amount, label = HHCore.GetItemLabel(addItem)})
                                else
                    ...
                    if itemAdded then 
                        ...
                        local itemLabel = ''
                        if Config.ItemLabelHHCore then itemLabel = HHCore.GetItemLabel(addItem) else itemLabel = tostring(addItem) end
                        TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xPlayer.source, (Lang['storage_deposited_x']:format(amount, itemLabel)))
                    end
                else
                    storage = {}
                    if Config.ItemLabelHHCore then
        ...
        TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xPlayer.source, Lang['not_enough_items'])
    end
end)

-- Get Storage Inventory:
HHCore.RegisterServerCallback('t1ger_mechanicjob:getStorageInventory', function(source, cb, id)
	local xPlayer = HHCore.GetPlayerFromId(source)
    local dataFected = false
    local storageInv = {}
    MySQL.Async.fetchAll("SELECT storage FROM t1ger_mechanic WHERE shopID = @shopID", {['@shopID'] = id}, function(data)
...
HHCore.RegisterServerCallback('t1ger_mechanicjob:getTakenShops', function(source, cb)
    local xPlayer = HHCore.GetPlayerFromId(source)
...
HHCore.RegisterServerCallback('t1ger_mechanicjob:getShopAccounts', function(source, cb)
    local xPlayer = HHCore.GetPlayerFromId(source)
        ...
        if data[1].storage ~= nil then
            ...
            for k,v in pairs(storage) do
                if removeItem == v.item then
                    ...
                    local itemLabel = ''
                    if Config.ItemLabelHHCore then itemLabel = HHCore.GetItemLabel(removeItem) else itemLabel = tostring(removeItem) end
                    TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xPlayer.source, (Lang['storage_withdrew_x']:format(amount, itemLabel)))
                end
            end
        end
    end)
...
AddEventHandler('t1ger_mechanicjob:craftItem', function(item_label, item_name, item_recipe, id, val)
    ...
    if hasRecipeItems then 
        ...
        TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xPlayer.source, Lang['not_enough_materials'])
    end
end)
...
AddEventHandler('t1ger_mechanicjob:sendBill',function(target, amount)
    ...
    if amount ~= nil then
        if amount >= 0 then
            for i = 1, #xPlayers, 1 do
                local tPlayer = HHCore.GetPlayerFromId(xPlayers[i])
                if tPlayer.source == target then
                    tPlayer.removeAccountMoney('bank', tonumber(amount))
                    TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', tPlayer.source, "You paid the invoice of ~g~$"..amount.."~s~ to the mechanic.")
                    xPlayer.addAccountMoney('bank', tonumber(amount))
                    TriggerClientEvent('t1ger_mechanicjob:ShowNotifyHHCore', xPlayer.source, "You received payment from the invoice of ~g~$"..amount)
                    break
                end
            end
        end
...
Citizen.CreateThread(function()
	for k,v in pairs(Config.RepairKits) do 
		HHCore.RegisterUsableItem(v.item, function(source)
			local xPlayer = HHCore.GetPlayerFromId(source)
...
-- Get inventory item:
HHCore.RegisterServerCallback('t1ger_mechanicjob:getInventoryItem',function(source, cb, item)
    local xPlayer = HHCore.GetPlayerFromId(source)
...
HHCore.RegisterServerCallback('t1ger_mechanicjob:getMaterialsForHealthRep',function(source, cb, plate, degName, materials, newValue, addValue, vehOnLift)
    local xPlayer = HHCore.GetPlayerFromId(source)
