...
AddEventHandler('t1ger_mechanicjob:fetchMechShopsCL', function(shopID)
	...
	HHCore.TriggerServerCallback('t1ger_mechanicjob:getTakenShops', function(ownedShops)
...
function MechShopManageMenu(id,val)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "mech_shop_manage_menu",
		{
	...
	function(data, menu)
		if(data.current.value == 'rename_mech_shop') then
			HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'mech_shop_rename', {
				title = "Rename Mech Shop"
			}, function(data2, menu2)
				menu2.close()
				local shopName = tostring(data2.value)
				HHCore.TriggerServerCallback('t1ger_mechanicjob:renameMechShop', function(renamed)
					if renamed then
						ShowNotifyHHCore((Lang['mech_shop_renamed']):format(shopName))
						TriggerServerEvent('t1ger_mechanicjob:fetchMechShops')
						menu.close()
						bossMenu = nil
					else
						ShowNotifyHHCore(Lang['not_your_mech_shop'])
						menu.close()
						bossMenu = nil
					end
				end, id, val, shopName)
...
function AccountsMainMenu(id,val)
	...
	HHCore.TriggerServerCallback('t1ger_mechanicjob:getShopAccounts', function(account)
		HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "mech_shop_employees_list",
			{
		...
		function(data, menu)
			if(data.current.value == 'account_withdraw') then
				menu.close()
				HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'mech_shop_withdraw_money', {
			...
			if(data.current.value == 'account_deposit') then
				menu.close()
				HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'mech_shop_deposit_money', {
					title = "Account Money: $"..account
				}, function(data2, menu2)
					menu2.close()
...
function EmployeesMainMenu(id,val)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "mech_shop_employees_main",
		{
	...
	function(data, menu)
		if(data.current.value == 'reqruit_employee') then
			menu.close()
			HHCore.TriggerServerCallback('t1ger_mechanicjob:getOnlinePlayers', function(players)
				local elements = {}
				...
				HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'mech_shop_reqruit_player', {
					title    = "Recruit Employee",
					align    = 'center',
					elements = elements
				}, function(data2, menu2)
					-- YES / NO OPTION:
					HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'mech_shop_reqruit_confirm', {
...
function OpenEmployeeListMenu(id,val)
	local elements = {}
	HHCore.TriggerServerCallback('t1ger_mechanicjob:getEmployees', function(employees)
		if employees ~= nil then 
			...
			ShowNotifyHHCore(Lang['no_employees_hired'])
		end
...
function OpenEmployeeData(info,user,id,val)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "mech_shop_employee_data_menu",
		{
	...
	function(data, menu)
		...
		if(data.current.value == 'job_grade_manage') then
			menu.close()
			HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'mech_shop_update_new_job_grade', {
...
function SellMechShopMenu(id,val)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "mech_shop_sell_menu",
		{
	...
	function(data, menu)
		if(data.current.value == 'confirm_sale') then
			HHCore.TriggerServerCallback('t1ger_mechanicjob:sellMechShop', function(sold)
				if sold then
					TriggerServerEvent('t1ger_mechanicjob:fetchMechShops')
					ShowNotifyHHCore((Lang['mech_shop_sold']):format(math.floor(sellPrice)))
				else
					ShowNotifyHHCore(Lang['not_your_mech_shop'])
				end
...
function BuyMechShopMenu(id,val)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "mech_shop_purchase",
		{
	...
	function(data, menu)
		if(data.current.value == 'confirm_purchase') then
			...
			HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'mech_shop_choose_name', {
				title = "Enter Name For Mech Shop"
			}, function(data2, menu2)
				menu2.close()
				local shopName = tostring(data2.value)
				HHCore.TriggerServerCallback('t1ger_mechanicjob:buyMechShop', function(purchased)
					if purchased then
						ShowNotifyHHCore((Lang['mech_shop_bought']):format(math.floor(val.price)))
						TriggerServerEvent('t1ger_mechanicjob:fetchMechShops')
						menu.close()
						bossMenu = nil
					else
						ShowNotifyHHCore(Lang['not_enough_money'])
						menu.close()
						bossMenu = nil
					end
				end, id, val, shopName)
	...
	if storageMenu ~= nil then
		...
		elseif distToStorage <= 2.0 then
			...
			if IsControlJustPressed(0, 38) then
				HHCore.TriggerServerCallback('t1ger_mechanicjob:checkAccess', function(hasAccess)
					if hasAccess then 
						storageMenu = v
						MechShopStorageMenu(k,v)
					else
						ShowNotifyHHCore(Lang['no_access'])
					end
				end, k)
			end
		end
...
function MechShopStorageMenu(id, val)
	...
	function(data, menu)
		if(data.current.value == 'storage_deposit') then
			HHCore.TriggerServerCallback('t1ger_mechanicjob:getUserInventory', function(inventory)
				local invItems = {}
				...
				HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "mech_shop_storage_deposit",
					{
				...
				function(data2, menu2)
					menu2.close()
					HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'storage_deposit_dialog', {title = "Enter Amount"}, function(data3, menu3)
                        local count = tonumber(data3.value)

                        if count == nil then
                            ShowNotifyHHCore(Lang['invalid_amount'])
                        else
							if count > 0 then
								menu3.close()
		...
		if(data.current.value == 'storage_withdraw') then
			HHCore.TriggerServerCallback('t1ger_mechanicjob:getStorageInventory', function(inventory)
				if inventory ~= nil then 
					local invItems = {}
					for k,v in pairs(inventory) do
						table.insert(invItems, {label = v.count.."x "..v.label, value = v.item, amount = v.count})
					end
					HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "mech_shop_storage_withdraw",
						{
					...
					function(data2, menu2)
						menu2.close()
						HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'storage_withdraw_dialog', {title = "Enter Amount"}, function(data3, menu3)
							local count = tonumber(data3.value)
							...
							if count == nil then
								...
								if count > 0 then
									...
									if count <= data2.current.amount then
										...
										ShowNotifyHHCore(Lang['too_high_count'])
									end
									Wait(500)
									MechShopStorageMenu(id, val)
								else
					...
					ShowNotifyHHCore(Lang['storage_inv_empty'])
				end
			end, id)
		end
		menu.close()
...
function workbenchMenuFunction(k,v,distToWorkbench)
	if workbenchMenu ~= nil then
		...
		if workbenchMenu == nil then HHCore.UI.Menu.CloseAll() end
	else
		local mk = Config.MarkerSettings
...
function MechShopWorkbenchMenu(id,val)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "mech_workbench_menu",
		{
	...
	function(data, menu)
		...
		HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "mech_workbench_options_menu",
			{
		...
		function(data2, menu2)
			if data2.current.value == "craft_item" then 
				...
				if not keyPressed then 
					keyPressed = true
					exports['hhrp-taskbar']:taskBar((Config.CraftTime * 1000), (Lang['crafting_item']:format(string.upper(selected.label))))
					...
					ShowNotifyHHCore(Lang['crafting_in_progress'])
				end
...
function ViewCraftingRecipe(item_label, item, recipe, id, val)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "mech_workbench_recipe_menu",
		{
								...
								if v.inUse then 
									...
									elseif IsControlJustPressed(0, 172) then 
										if v.pos[3] < v.maxValue then 
											...
											ShowNotifyHHCore(Lang['lift_cannot_go_higher'])
										end
									elseif IsControlJustPressed(0, 173) then 
										if v.pos[3] > v.minValue then
											...
											ShowNotifyHHCore(Lang['lift_cannot_go_lower'])
										end
									end
								else
...
function OpenMechanicActionMenu()
	if PlayerData.job and PlayerData.job.name == "mechanic" then
		...
		HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "mechanic_action_main_menu",
			{
				title    = "Mechanic Menu",
				align    = "center",
				elements = elements
		...
		function(data, menu)
			if(data.current.value == 'mech_billing') then
				menu.close()
				HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'mechanic_billing_dialog', {
					...
					if amount ~= nil then 
						...
						if closestPlayer == -1 or closestDistance > 3.0 then
							ShowNotifyHHCore(Lang['no_players_nearby'])
						else
...
function CarryObjectsMainMenu()
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "mechanic_prop_emotes_menu",
		{
			title    = "Prop Emotes",
			align    = "center",
			elements = elements
	...
	function(data, menu)
		if data.current.value == "remove_obj" then
			...
			HHCore.Game.SpawnObject(selct.prop, {x = coords.x, y = coords.y, z = coords.z}, function(spawnModel)
...
function RepairVehicleEngine()
	...
	if vehicle ~= 0 then 
		if vehOnLift[plate] ~= nil then 
				...
				while true do 
					...
					if distance < 5.0 then
						...
						if IsControlJustPressed(0, 38) and distance < 1.0 then
							...
							exports['hhrp-taskbar']:taskBar(2000, "INSPECTING: ENGINE")
							Citizen.Wait(2000)
							...
							if engineValue < 10.0 then 
								...
								HHCore.TriggerServerCallback('t1ger_mechanicjob:getMaterialsForHealthRep', function(hasMaterials)
									if hasMaterials then 
										-- repair anim:
										SetEntityHeading(player, GetEntityHeading(vehicle))
										Citizen.Wait(500)
										TaskStartScenarioInPlace(player, "WORLD_HUMAN_VEHICLE_MECHANIC", 0, true)
										exports['hhrp-taskbar']:taskBar(3500, "REPAIRING: ENGINE")
										Citizen.Wait(3500)
										SetVehicleEngineHealth(vehicle, 1000.0)
										ClearPedTasks(player)
									else
										ShowNotifyHHCore(Lang['need_more_materials'])
									end
								end, plate, "engine", engineMaterials, 10.0, engineAddVal, vehOnLift)
							else
								ShowNotifyHHCore("Engine is fully functional")
							end
							break
						end
					end
			...
			ShowNotifyHHCore(Lang['veh_must_be_on_lift'])
		end
	else
		ShowNotifyHHCore(Lang['no_vehicle_nearby'])
	end
end
		...
		if vehOnLift[plate] ~= nil then 
			if GetEntityModel(GetHashKey(vehicle)) == GetEntityModel(GetHashKey(vehOnLift[plate].entity)) then 
				if vehOnLift[plate].health ~= nil then
					...
					HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "veh_health_part_menu",
						{
					...
					function(data, menu)
						menu.close()
						local selected = data.current
						HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'mech_veh_health_part_dialog', {
							...
							if vehOnLift[plate].health[selected.degName] ~= nil then 
								...
								ShowNotifyHHCore(Lang['veh_must_be_inspected'])
							end
						end,
						function(data2, menu2)
							menu2.close()	
		...
		while true do 
			...
			if distance < 5.0 then
				...
				if IsControlJustPressed(0, 38) and distance < 1.0 then
					...
					Citizen.Wait(500)
					TaskStartScenarioInPlace(player, "WORLD_HUMAN_VEHICLE_MECHANIC", 0, true)
					exports['hhrp-taskbar']:taskBar(3500, (Lang['lift_repairing_veh']:format(string.upper(label))))
					Citizen.Wait(3500)
					ClearPedTasks(player)
					HHCore.TriggerServerCallback('t1ger_mechanicjob:getMaterialsForHealthRep', function(hasMaterials)
						if hasMaterials then 
							vehOnLift[plate].health[degName].value = round(newValue,2)
							TriggerServerEvent('t1ger_mechanicjob:updateVehDegradation', plate, label, degName, vehOnLift)
	...
	if vehicle ~= 0 then 
		if vehOnLift[plate] ~= nil then 
			if GetEntityModel(GetHashKey(vehicle)) == GetEntityModel(GetHashKey(vehOnLift[plate].entity)) then 
							...
							if distance < 4.0 then
								if not v.done then 
									...
									if IsControlJustPressed(0, 38) and distance < 1.0 then
										...
										exports['hhrp-taskbar']:taskBar(1500, Lang['progbar_inspecting_veh'])
										Citizen.Wait(1500)
				...
				HHCore.TriggerServerCallback('t1ger_mechanicjob:getVehDegradation', function(degradation)
					local vehHealth = {}
					if degradation ~= nil then 
						...
						ShowNotifyHHCore("Works with only player owned vehicles.")
					end
				end, plate)
			else
				print("veh not matched")
			...
			if distance < 6.0 then
				...
				if DoesEntityExist(findObj) then
					...
					if type == 'analyse' then
						if not vehAnalysed then 
							...
							ShowNotifyHHCore(Lang['veh_already_analyzed'])
							break
						end
					end
				else
					...
					elseif type == 'analyse' then 
						ShowNotifyHHCore(Lang['raise_veh_b4_analyze'])
						break
					end
				end
				...
				if IsControlJustPressed(0, 38) and distance < 1.0 then
					if isJackRaised then
						...
						HHCore.TriggerServerCallback('t1ger_mechanicjob:getInventoryItem', function(hasItem)
							if hasItem then 
								UseTheJackFunction(vehicle)
							else
								ShowNotifyHHCore(Lang['car_jack_carry'])
							end
						end, item)
					end
					break
...
function UseTheJackFunction(vehicle)
	...
	exports['hhrp-taskbar']:taskBar((6500), label)
	-- Raise Jack Task:
			...
			if distance < 4.0 then
				if not v.done then 
					...
					if IsControlJustPressed(0, 38) and distance < 1.0 then
						...
						TaskStartScenarioInPlace(player, v.scenario, 0, true)
						exports['hhrp-taskbar']:taskBar(2500, Lang['progbar_analyzing_veh'])
						Citizen.Wait(2500)
						ClearPedTasks(player)
						v.done = true
					end
...
AddEventHandler('t1ger_mechanicjob:installBodyPartCL', function(id, val)
	local vehicle = GetClosestVehicle(coords.x, coords.y, coords.z, 5.0, 0, 71)
	if vehicle ~= 0 then 
		if vehAnalysed then else return ShowNotifyHHCore(Lang['analyze_veh_first']) end
		GetControlOfEntity(vehicle)
		local vehCoords = GetEntityCoords(vehicle, 1)
		...
		if not installingPart then
			if not DoesEntityExist(carryObj) then 
				...
				HHCore.Game.SpawnObject(val.prop, {x = coords.x, y = coords.y, z = coords.z}, function(spawnObj)
					carryObj = spawnObj
						...
						if IsControlJustPressed(0, 47) and distance < 1.5 then
								...
								for i = 0, (GetNumberOfVehicleDoors(vehicleData[plate].entity) - 2) do
									if vehicleData[plate].report.doors[i + 1] == true then
										...
										if tonumber(i + 1) == tonumber(GetNumberOfVehicleDoors(vehicleData[plate].entity) - 2) then 
											ShowNotifyHHCore(Lang['all_doors_intact'])
											break
										end
									end
								end
							...
							if id == 2 then 
								if vehicleData[plate].report.doors[4+1] == true then
									...
									ShowNotifyHHCore(Lang['hood_already_installed'])
								end
							end
							if id == 3 then 
								if vehicleData[plate].report.doors[5+1] == true then
									...
									ShowNotifyHHCore(Lang['trunk_already_installed'])
								end
							end
							if id == 4 then 
								for i = 0, (GetVehicleNumberOfWheels(vehicleData[plate].entity) - 1) do
									if GetVehicleWheelXOffset(vehicleData[plate].entity, i) == 9999.0 then
										...
										if tonumber(i + 1) == tonumber(GetVehicleNumberOfWheels(vehicleData[plate].entity) - 1) then
											ShowNotifyHHCore(Lang['all_wheels_intact'])
							...
							HHCore.Game.DeleteObject(carryObj)
							carryObj = nil
							ClearPedTasks(player)
							Citizen.Wait(100)

							local progression = GetBodyRepairProgression(vehicle)
							ShowNotifyHHCore("Progression: ["..progression.."/100]")
							if progression >= 100 then 
								...
								vehAnalysed = false
								ShowNotifyHHCore(Lang['all_body_repairs_done'])
							end
							break
						end
					else
						ShowNotifyHHCore(Lang['raise_and_analyze'])
						break
					end
				end
				if distance > 4.0 then 
			...
			ShowNotifyHHCore(Lang['finish_current_install'])
		end
	else
		ShowNotifyHHCore(Lang['no_vehicle_nearby'])
	end
			...
			if IsControlJustPressed(0, 38) then 
				if distance < 1.0 then 
					...
					if math.random(100) > val.chanceToKeep then
						TriggerServerEvent('t1ger_mechanicjob:removeItem', val.item, 1)
						ShowNotifyHHCore(Lang['repair_kit_broke'])
					end
					-- end:
					SetVehicleDoorShut(vehicle, 4, 1, 1)
					ClearPedTasks(player)
					ShowNotifyHHCore(Lang['repairkit_veh_repaired'])
					vehRepaired = true
					repairing = false
					break
				else
...
function RepairVehOptions(veh, type, val)
	...
	exports['hhrp-taskbar']:taskBar((repairDuration), val.progbar)
...
function ApplyVehicledegradation(curVehicle, plate)
	HHCore.TriggerServerCallback('t1ger_mechanicjob:getIfVehicleOwned', function(vehOwned)
		if vehOwned then 
			HHCore.TriggerServerCallback('t1ger_mechanicjob:getVehDegradation', function(degradation)
				if degradation ~= nil then 
					local vehHealth = {}
...
function ManageNpcJobs()
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "mech_npc_job_menu",
		{
			title    = "NPC Job Menu",
			align    = "center",
			elements = elements
	...
	function(data, menu)
		if(data.current.value == 'find_job') then
			...
			if count == 100 then
				ShowNotifyHHCore("Wait for a call")
			else
	...
	while not JobDone and not CancelJob do 
		...
		if job.inUse then
			...
			if distance < 50.0 and not jobVehSpawned then
				...
				HHCore.Game.SpawnVehicle(jobVehicle, {x = job.pos[1], y = job.pos[2], z = job.pos[3]}, job.pos[4], function(vehicle)
			...
			if distance < 10.0 then
				...
				if not pedShouted and distance < 6.0 then
					ShowNotifyHHCore(Lang['npc_shout_msg'])
					pedShouted = true
				end
					...
					if IsControlJustPressed(0, 38) and vehDistance < 1.0 and not buttonClicked then
						...
						HHCore.TriggerServerCallback('t1ger_mechanicjob:getInventoryItem', function(hasItem)
							if hasItem then 
								...
								exports['hhrp-taskbar']:taskBar((4000), Lang['progbar_npc_fix'])
								Citizen.Wait(4000)
								...
								if math.random(100) > 10 then
									...
									ShowNotifyHHCore(Lang['npc_kit_broke'])
								end
								SetVehicleDoorShut(vehicle, 4, 1, 1)
								ClearPedTasks(GetPlayerPed(-1))
								ShowNotifyHHCore(Lang['npc_veh_repaired'])
								vehicleRepaired = true
								buttonClicked = false
							else
								ShowNotifyHHCore(Lang['npc_need_repair_kit'])
								buttonClicked = false
							end
						end, "repairkit")
					end
				...
				if pedDistance < 5.0 and vehicleRepaired then
					...
					if IsControlJustPressed(0, 38) and pedDistance < 1.5 then
						...
						exports['hhrp-taskbar']:taskBar((2000), Lang['progbar_npc_cash'])
						Citizen.Wait(2000)
						ClearPedTasks(GetPlayerPed(-1))
						RollUpWindow(JobVeh, 0)
						ShowNotifyHHCore(Lang['npc_thanking_msg'])
...
AddEventHandler('hhrp:onPlayerDeath', function(data)
	...
	if JobVeh ~= nil or JobPed ~= nil then 
		ShowNotifyHHCore(Lang['npc_cancel_job_death'])
	end
	Wait(300)
	CancelJob = false
end)
...
function CancelCurrentJob()
	CancelJob = true
	if JobVeh ~= nil or JobPed ~= nil then 
		ShowNotifyHHCore(Lang['npc_job_cancel_by_ply'])
	end
	Wait(300)
	CancelJob = false
end
