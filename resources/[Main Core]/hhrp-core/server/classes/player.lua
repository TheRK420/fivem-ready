function CreateExtendedPlayer(player, accounts, job, loadout, name, lastPosition)
	...
	self.setMoney = function(money)
		money = HHCore.Math.Round(money)

		if money >= 0 then
			self.player.setMoney(money)
		else
			print(('hhrp-core: %s attempted exploiting! (reason: player tried setting -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.setBankBalance = function(money)
		...
		if money >= 0 then
			...
			print(('hhrp-core: %s attempted exploiting! (reason: player tried setting -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.addMoney = function(money)
		...
		if money >= 0 then
			...
			print(('hhrp-core: %s attempted exploiting! (reason: player tried adding -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.removeMoney = function(money)
		...
		if money >= 0 then
			...
			print(('hhrp-core: %s attempted exploiting! (reason: player tried removing -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.addBank = function(money)
		...
		if money >= 0 then
			...
			print(('hhrp-core: %s attempted exploiting! (reason: player tried adding -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.removeBank = function(money)
		...
		if money >= 0 then
			...
			print(('hhrp-core: %s attempted exploiting! (reason: player tried removing -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.getLastPosition = function()
		if self.lastPosition and self.lastPosition.x and self.lastPosition.y and self.lastPosition.z then
			self.lastPosition.x = HHCore.Math.Round(self.lastPosition.x, 1)
			self.lastPosition.y = HHCore.Math.Round(self.lastPosition.y, 1)
			self.lastPosition.z = HHCore.Math.Round(self.lastPosition.z, 1)
		end
	...
	self.setAccountMoney = function(acc, money)
		if money < 0 then
			print(('hhrp-core: %s attempted exploiting! (reason: player tried setting -1 account balance)'):format(self.identifier))
			return
		end

		local account   = self.getAccount(acc)
		local prevMoney = account.money
		local newMoney  = HHCore.Math.Round(money)
		...
		TriggerClientEvent('hhrp:setAccountMoney', self.source, account)
	end

	self.addAccountMoney = function(acc, money)
		if money < 0 then
			print(('hhrp-core: %s attempted exploiting! (reason: player tried adding -1 account balance)'):format(self.identifier))
			return
		end

		local account  = self.getAccount(acc)
		local newMoney = account.money + HHCore.Math.Round(money)
	...
	self.removeAccountMoney = function(acc, money)
		if money < 0 then
			print(('hhrp-core: %s attempted exploiting! (reason: player tried removing -1 account balance)'):format(self.identifier))
			return
		end
	...
	self.setJob = function(job, grade)
		...
		if HHCore.DoesJobExist(job, grade) then
			...
			TriggerEvent('hhrp:setJob', self.source, self.job, lastJob)
			TriggerClientEvent('hhrp:setJob', self.source, self.job)
		else
			print(('hhrp-core: ignoring setJob for %s due to job not found!'):format(self.source))
		end
	end
	return self
end
