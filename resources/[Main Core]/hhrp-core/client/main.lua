...
AddEventHandler('hhrp:playerLoaded', function(xPlayer)
	HHCore.PlayerLoaded = true
	HHCore.PlayerData = xPlayer

	if Config.EnableHud then
		TriggerEvent('es:setMoneyDisplay', 0.0)
		HHCore.UI.HUD.SetDisplay(0.0)
		for k,v in ipairs(xPlayer.accounts) do
			local accountTpl = '<div><img src="img/accounts/' .. v.name .. '.png"/>&nbsp;{{money}}</div>'

			HHCore.UI.HUD.RegisterElement('account_' .. v.name, k - 1, 0, accountTpl, {
				money = 0
			})

			HHCore.UI.HUD.UpdateElement('account_' .. v.name, {
				money = HHCore.Math.GroupDigits(v.money)
			})
		end
		...
		HHCore.UI.HUD.RegisterElement('job', #xPlayer.accounts, 0, jobTpl, {
			job_label   = '',
			grade_label = ''
		})

		HHCore.UI.HUD.UpdateElement('job', {
...
AddEventHandler('playerSpawned', function()
	while not HHCore.PlayerLoaded do
		Citizen.Wait(1)
	end
	...
	-- Restore position
	if HHCore.PlayerData.lastPosition then
		SetEntityCoords(playerPed, HHCore.PlayerData.lastPosition.x, HHCore.PlayerData.lastPosition.y, HHCore.PlayerData.lastPosition.z)
	end

	TriggerEvent('hhrp:restoreLoadout') -- restore loadout
...
RegisterNetEvent('hhrp:setAccountMoney')
AddEventHandler('hhrp:setAccountMoney', function(account)
	for k,v in ipairs(HHCore.PlayerData.accounts) do
		if v.name == account.name then
			HHCore.PlayerData.accounts[k] = account
			break
		end
	end

	if Config.EnableHud then
		HHCore.UI.HUD.UpdateElement('account_' .. account.name, {
			money = HHCore.Math.GroupDigits(account.money)
		})
	end
end)
...
-- Commands
RegisterNetEvent('hhrp:teleport')
AddEventHandler('hhrp:teleport', function(pos)
	pos.x = pos.x + 0.0
	pos.y = pos.y + 0.0
	pos.z = pos.z + 0.0
...
RegisterNetEvent('hhrp:loadIPL')
AddEventHandler('hhrp:loadIPL', function(name)
	Citizen.CreateThread(function()
		LoadMpDlcMaps()
		RequestIpl(name)
	end)
...
RegisterNetEvent('hhrp:unloadIPL')
AddEventHandler('hhrp:unloadIPL', function(name)
	Citizen.CreateThread(function()
		RemoveIpl(name)
	end)
end)

RegisterNetEvent('hhrp:playAnim')
AddEventHandler('hhrp:playAnim', function(dict, anim)
	Citizen.CreateThread(function()
		local playerPed = PlayerPedId()
...
RegisterNetEvent('hhrp:playEmote')
AddEventHandler('hhrp:playEmote', function(emote)
	Citizen.CreateThread(function()
...
RegisterNetEvent('hhrp:spawnVehicle')
AddEventHandler('hhrp:spawnVehicle', function(model)
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)

	HHCore.Game.SpawnVehicle(model, coords, 90.0, function(vehicle)
...
RegisterNetEvent('hhrp:spawnObject')
AddEventHandler('hhrp:spawnObject', function(model)
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	local forward   = GetEntityForwardVector(playerPed)
	local x, y, z   = table.unpack(coords + forward * 1.0)

	HHCore.Game.SpawnObject(model, {
		x = x,
		y = y,
		z = z
	}, function(obj)
...
RegisterNetEvent('hhrp:pickup')
AddEventHandler('hhrp:pickup', function(id, label, player)
	...
	--[[HHCore.Game.SpawnLocalObject('prop_money_bag_01', {
		x = x,
		y = y,
		z = z - 2.0,
	}, function(obj)
...
RegisterNetEvent('hhrp:removePickup')
AddEventHandler('hhrp:removePickup', function(id)
	HHCore.Game.DeleteObject(pickups[id].obj)
	pickups[id] = nil
end)

RegisterNetEvent('hhrp:spawnPed')
AddEventHandler('hhrp:spawnPed', function(model)
...
RegisterNetEvent('hhrp:deleteVehicle')
AddEventHandler('hhrp:deleteVehicle', function()
	local playerPed = PlayerPedId()
	local vehicle   = HHCore.Game.GetVehicleInDirection()
	...
	while true do
		...
		for k,v in pairs(pickups) do
			...
			if distance <= 1.0 and not v.inRange and IsPedOnFoot(playerPed) and IsControlJustReleased(0, Keys["E"]) then
				...
				TriggerServerEvent('hhrp:onPickup', v.id)
...
Citizen.CreateThread(function()
	while true do
		...
		if HHCore.PlayerLoaded and isPlayerSpawned then
			local coords = GetEntityCoords(playerPed)

			if not IsEntityDead(playerPed) then
				HHCore.PlayerData.lastPosition = {x = coords.x, y = coords.y, z = coords.z}
			end
		end
