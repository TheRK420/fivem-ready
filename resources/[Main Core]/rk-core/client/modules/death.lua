...
function PlayerKilledByPlayer(killerServerId, killerClientId, killerWeapon)
	...
	local data = {
		victimCoords = { x = RKCore.Math.Round(victimCoords.x, 1), y = RKCore.Math.Round(victimCoords.y, 1), z = RKCore.Math.Round(victimCoords.z, 1) },
		killerCoords = { x = RKCore.Math.Round(killerCoords.x, 1), y = RKCore.Math.Round(killerCoords.y, 1), z = RKCore.Math.Round(killerCoords.z, 1) },

		killedByPlayer = true,
		deathCause     = killerWeapon,
		distance       = RKCore.Math.Round(distance, 1),

		killerServerId = killerServerId,
		killerClientId = killerClientId
	}

	TriggerEvent('rk:onPlayerDeath', data)
	TriggerServerEvent('rk:onPlayerDeath', data)
end
