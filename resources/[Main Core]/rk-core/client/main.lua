...
Citizen.CreateThread(function()
	while true do
		...
		if paycheckReady == true and stackTwice == false and stackThree == false then
			RKCore.TriggerServerCallback('paycheck:checkSalary', function(result)
...
AddEventHandler('rk:playerLoaded', function(xPlayer)
	RKCore.PlayerLoaded = true
	RKCore.PlayerData = xPlayer

	if Config.EnableHud then
		TriggerEvent('es:setMoneyDisplay', 0.0)
		RKCore.UI.HUD.SetDisplay(0.0)
		for k,v in ipairs(xPlayer.accounts) do
			local accountTpl = '<div><img src="img/accounts/' .. v.name .. '.png"/>&nbsp;{{money}}</div>'

			RKCore.UI.HUD.RegisterElement('account_' .. v.name, k - 1, 0, accountTpl, {
				money = 0
			})

			RKCore.UI.HUD.UpdateElement('account_' .. v.name, {
				money = RKCore.Math.GroupDigits(v.money)
			})
		end
		...
		RKCore.UI.HUD.RegisterElement('job', #xPlayer.accounts, 0, jobTpl, {
			job_label   = '',
			grade_label = ''
		})

		RKCore.UI.HUD.UpdateElement('job', {
...
AddEventHandler('playerSpawned', function()
	while not RKCore.PlayerLoaded do
		Citizen.Wait(1)
	end
	...
	-- Restore position
	if RKCore.PlayerData.lastPosition then
		SetEntityCoords(playerPed, RKCore.PlayerData.lastPosition.x, RKCore.PlayerData.lastPosition.y, RKCore.PlayerData.lastPosition.z)
	end

	TriggerEvent('rk:restoreLoadout') -- restore loadout
...
RegisterNetEvent('rk:setAccountMoney')
AddEventHandler('rk:setAccountMoney', function(account)
	for k,v in ipairs(RKCore.PlayerData.accounts) do
		if v.name == account.name then
			RKCore.PlayerData.accounts[k] = account
			break
		end
	end

	if Config.EnableHud then
		RKCore.UI.HUD.UpdateElement('account_' .. account.name, {
			money = RKCore.Math.GroupDigits(account.money)
		})
	end
end)
...
-- Commands
RegisterNetEvent('rk:teleport')
AddEventHandler('rk:teleport', function(pos)
	pos.x = pos.x + 0.0
	pos.y = pos.y + 0.0
	pos.z = pos.z + 0.0
...
RegisterNetEvent('rk:loadIPL')
AddEventHandler('rk:loadIPL', function(name)
	Citizen.CreateThread(function()
		LoadMpDlcMaps()
		RequestIpl(name)
	end)
...
RegisterNetEvent('rk:unloadIPL')
AddEventHandler('rk:unloadIPL', function(name)
	Citizen.CreateThread(function()
		RemoveIpl(name)
	end)
end)

RegisterNetEvent('rk:playAnim')
AddEventHandler('rk:playAnim', function(dict, anim)
	Citizen.CreateThread(function()
		local playerPed = PlayerPedId()
...
RegisterNetEvent('rk:playEmote')
AddEventHandler('rk:playEmote', function(emote)
	Citizen.CreateThread(function()
...
RegisterNetEvent('rk:spawnVehicle')
AddEventHandler('rk:spawnVehicle', function(model)
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)

	RKCore.Game.SpawnVehicle(model, coords, 90.0, function(vehicle)
...
RegisterNetEvent('rk:spawnObject')
AddEventHandler('rk:spawnObject', function(model)
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	local forward   = GetEntityForwardVector(playerPed)
	local x, y, z   = table.unpack(coords + forward * 1.0)

	RKCore.Game.SpawnObject(model, {
		x = x,
		y = y,
		z = z
	}, function(obj)
...
RegisterNetEvent('rk:pickup')
AddEventHandler('rk:pickup', function(id, label, player)
	...
	--[[RKCore.Game.SpawnLocalObject('prop_money_bag_01', {
		x = x,
		y = y,
		z = z - 2.0,
	}, function(obj)
...
RegisterNetEvent('rk:removePickup')
AddEventHandler('rk:removePickup', function(id)
	RKCore.Game.DeleteObject(pickups[id].obj)
	pickups[id] = nil
end)

RegisterNetEvent('rk:spawnPed')
AddEventHandler('rk:spawnPed', function(model)
...
RegisterNetEvent('rk:deleteVehicle')
AddEventHandler('rk:deleteVehicle', function()
	local playerPed = PlayerPedId()
	local vehicle   = RKCore.Game.GetVehicleInDirection()
	...
	while true do
		...
		for k,v in pairs(pickups) do
			...
			if distance <= 1.0 and not v.inRange and IsPedOnFoot(playerPed) and IsControlJustReleased(0, Keys["E"]) then
				...
				TriggerServerEvent('rk:onPickup', v.id)
...
Citizen.CreateThread(function()
	while true do
		...
		if RKCore.PlayerLoaded and isPlayerSpawned then
			local coords = GetEntityCoords(playerPed)

			if not IsEntityDead(playerPed) then
				RKCore.PlayerData.lastPosition = {x = coords.x, y = coords.y, z = coords.z}
			end
		end
