Citizen.CreateThread(function()
	Citizen.Wait(500)
	print('\27[32m[rk-core]\27[0m Loaded All Data Successfully')
end)
	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
			...
			table.insert(tasks2, function(cb2)
				...
				MySQL.Async.fetchAll('SELECT job, job_grade, loadout, position FROM users WHERE identifier = @identifier', {
					...
					if RKCore.DoesJobExist(job, grade) then
						...
						print(('rk-core: %s had an unknown job [job: %s, grade: %s], setting as unemployed!'):format(player.getIdentifier(), job, grade))
		...
		Async.parallel(tasks, function(results)
			...
			xPlayer.getMissingAccounts(function(missingAccounts)
				...
				RKCore.Players[_source] = xPlayer

				TriggerEvent('rk:playerLoaded', _source, xPlayer)

				TriggerClientEvent('rk:playerLoaded', _source, {
...
AddEventHandler('playerDropped', function(reason)
	...
	if xPlayer then
		TriggerEvent('rk:playerDropped', _source, reason)
		TriggerEvent('rk:PlayerDropped', xPlayer, source)

		RKCore.SavePlayer(xPlayer, function()
			RKCore.Players[_source] = nil
			RKCore.LastPlayerData[_source] = nil
		end)
	end
end)

RegisterServerEvent('rk:updateLoadout')
AddEventHandler('rk:updateLoadout', function(loadout)
	local xPlayer = RKCore.GetPlayerFromId(source)
	xPlayer.loadout = loadout
end)

RegisterServerEvent('rk:updateLastPosition')
AddEventHandler('rk:updateLastPosition', function(position)
	local xPlayer = RKCore.GetPlayerFromId(source)
	xPlayer.setLastPosition(position)
end)

RKCore.RegisterServerCallback('rk:getPlayerData', function(source, cb)
	local xPlayer = RKCore.GetPlayerFromId(source)
...
RKCore.RegisterServerCallback('rk:getOtherPlayerData', function(source, cb, target)
