function CreateExtendedPlayer(player, accounts, job, loadout, name, lastPosition)
	...
	self.setMoney = function(money)
		money = RKCore.Math.Round(money)

		if money >= 0 then
			self.player.setMoney(money)
		else
			print(('rk-core: %s attempted exploiting! (reason: player tried setting -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.setBankBalance = function(money)
		...
		if money >= 0 then
			...
			print(('rk-core: %s attempted exploiting! (reason: player tried setting -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.addMoney = function(money)
		...
		if money >= 0 then
			...
			print(('rk-core: %s attempted exploiting! (reason: player tried adding -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.removeMoney = function(money)
		...
		if money >= 0 then
			...
			print(('rk-core: %s attempted exploiting! (reason: player tried removing -1 cash balance)'):format(self.identifier))
		end
	end
	...
	self.addBank = function(money)
		...
		if money >= 0 then
			...
			print(('rk-core: %s attempted exploiting! (reason: player tried adding -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.removeBank = function(money)
		...
		if money >= 0 then
			...
			print(('rk-core: %s attempted exploiting! (reason: player tried removing -1 bank balance)'):format(self.identifier))
		end
	end
	...
	self.getLastPosition = function()
		if self.lastPosition and self.lastPosition.x and self.lastPosition.y and self.lastPosition.z then
			self.lastPosition.x = RKCore.Math.Round(self.lastPosition.x, 1)
			self.lastPosition.y = RKCore.Math.Round(self.lastPosition.y, 1)
			self.lastPosition.z = RKCore.Math.Round(self.lastPosition.z, 1)
		end
	...
	self.setAccountMoney = function(acc, money)
		if money < 0 then
			print(('rk-core: %s attempted exploiting! (reason: player tried setting -1 account balance)'):format(self.identifier))
			return
		end

		local account   = self.getAccount(acc)
		local prevMoney = account.money
		local newMoney  = RKCore.Math.Round(money)
		...
		TriggerClientEvent('rk:setAccountMoney', self.source, account)
	end

	self.addAccountMoney = function(acc, money)
		if money < 0 then
			print(('rk-core: %s attempted exploiting! (reason: player tried adding -1 account balance)'):format(self.identifier))
			return
		end

		local account  = self.getAccount(acc)
		local newMoney = account.money + RKCore.Math.Round(money)
	...
	self.removeAccountMoney = function(acc, money)
		if money < 0 then
			print(('rk-core: %s attempted exploiting! (reason: player tried removing -1 account balance)'):format(self.identifier))
			return
		end
	...
	self.setJob = function(job, grade)
		...
		if RKCore.DoesJobExist(job, grade) then
			local jobObject, gradeObject = RKCore.Jobs[job], RKCore.Jobs[job].grades[grade]
			...
			TriggerEvent('rk:setJob', self.source, self.job, lastJob)
			TriggerClientEvent('rk:setJob', self.source, self.job)
		else
			print(('rk-core: ignoring setJob for %s due to job not found!'):format(self.source))
		end
	end
	return self
end
