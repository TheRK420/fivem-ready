		...
		for i=1, #xPlayers, 1 do
			...
			if salary > 0 then
				...
				elseif Config.EnableSocietyPayouts then -- possibly a society
					TriggerEvent('rk-society:getSociety', xPlayer.job.name, function (society)
						if society ~= nil then -- verified society
							TriggerEvent('rk-addonaccount:getSharedAccount', society.account, function (account)
								if account.money >= salary then -- does the society money to pay its employees?
									...
									TriggerClientEvent('rk:showAdvancedNotification', xPlayer.source, _U('bank'), '', _U('company_nomoney'), 'CHAR_BANK_MAZE', 1)
								end
							end)
						else -- not a society
							xPlayer.addAccountMoney('bank', salary)
...
RKCore.RegisterServerCallback('paycheck:checkSalary', function(source, cb)
	if source ~= nil then
				...
				if society ~= nil then -- verified society
					TriggerEvent('rk-addonaccount:getSharedAccount', society.account, function (account)
						if account.money >= salary then -- does the society money to pay its employees?
							...
							TriggerClientEvent('rk:showAdvancedNotification', source, _U('bank'), '', _U('company_nomoney'), 'CHAR_BANK_MAZE', 1)
						end
					end)
				else -- not a society
					xPlayer.addMoney( salary)
