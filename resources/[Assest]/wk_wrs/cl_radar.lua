    ...
    if ( IsPedSittingInAnyVehicle( ped ) ) then 
        ...
        if ( GetVehicleClass( vehicle ) == 18 ) then
            ...
            if ( radarEnabled ) then 
                TriggerEvent('DoLongHudText', 'Radar Enabled', 1)
            else 
                TriggerEvent('DoLongHudText', 'Radar Disabled', 2)
            end 
            ...
            TriggerEvent('DoLongHudText', 'You are not in a police vehicle!', 2)
        end 
    else 
        TriggerEvent('DoLongHudText', 'Trying to use a radar without vehicle LuL!', 2)
    end 
end )
...
function ToggleSpeedType()
    if ( radarInfo.speedType == "mph" ) then 
        ...
        TriggerEvent('DoLongHudText', 'Speed in Km/h ???? Change it!', 2)
    else 
        radarInfo.speedType = "mph"
            TriggerEvent('DoLongHudText', 'Nice you are checking speed in MPH!', 1)
    end
end 
