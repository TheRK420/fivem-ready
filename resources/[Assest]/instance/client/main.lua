...
function LeaveInstance()
	if Instance.host ~= nil then
		...
		if #Instance.players > 1 then
			TriggerEvent('DoLongHudText', 'You have left the instance', 1)
		end
...
AddEventHandler('instance:onPlayerEntered', function(instance, player)
	...
	TriggerEvent('DoLongHudText',  playerName .. ' entered the instance', 1)
end)
...
AddEventHandler('instance:onPlayerLeft', function(instance, player)
	...
	TriggerEvent('DoLongHudText',  playerName .. ' left the instance', 1)
end)
...
AddEventHandler('instance:onInvite', function(instance, type, data)
	...
	Citizen.CreateThread(function()
		...
		if InstanceInvite ~= nil then
			TriggerEvent('DoLongHudText', 'Invite Expired', 2)
			InstanceInvite = nil
		end
	end)
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if InstanceInvite ~= nil and IsControlJustReleased(0, Keys['E']) then
			...
			TriggerEvent('DoLongHudText', 'You entered the instance', 1)
			InstanceInvite = nil
	...
	while true do
		...
		if Instance.host ~= nil then
			...
			for i=0, Config.MaxPlayers, 1 do
				...
				--[[	SetEntityLocallyInvisible(otherPlayerPed) ]] -- Removed Not Compatible with OneSync
					SetEntityVisible(otherPlayerPed, false, 0) -- Added for OneSync
			...
			for i=0, Config.MaxPlayers, 1 do
				...
				--[[	SetEntityLocallyInvisible(otherPlayerPed) ]] -- Removed Not Compatible with OneSync
					SetEntityVisible(otherPlayerPed, true, 0) -- Added for OneSync
					SetEntityNoCollisionEntity(playerPed, otherPlayerPed, true)
				end
