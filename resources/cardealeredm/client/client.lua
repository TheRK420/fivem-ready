											...
											if hasMoney then
												...
												HHCore.TriggerServerCallback('cardealeredm:ticketcheck',function(supporter)
													...
													if supporter == 1 then
														...
														BuyInProgress = false
													elseif supporter == 2 then
														SpawnPurchasedVehicle(currentModel, VehCoords, Heading, player, carPrice, carCommission, price, payment)
														SetVehicleDirtLevel(currentModel, 0)
														BuyInProgress = false
													elseif supporter == 3 then
														SpawnPurchasedVehicle(currentModel, VehCoords, Heading, player, carPrice, carCommission, price, payment)
														SetVehicleDirtLevel(currentModel, 0)
														BuyInProgress = false
														...
														BuyInProgress = false
														exports['mythic_notify']:DoHudText('inform', "You need Ticket to Buy From EDM!")
													end
												end)							
											else
					...
					if PlayerData.job ~= nil and PlayerData.job.name == Config.CarDealerJobLabel then
						...
						if carDist5 then
							...
							if currentCommission > (Config.MinCommission) then
								...
								TriggerServerEvent('carealeredm:DealerDiscountSV', currentModel, -1, currentID)
					...
					if PlayerData.job ~= nil and PlayerData.job.name == Config.CarDealerJobLabel then
						...
						if carDist6 then
							...
							if currentDownpayment < (Config.MaxDownpayment) then
								...
								TriggerServerEvent('carealeredm:DealerDownpaymentSV', currentModel, 1, currentID)
								exports['progressBars']:startUI(500, "APPLYING CHANGES")
								Citizen.Wait(500)
							else
								ChangingValues = false
...
RegisterNetEvent('cardealeredm:ticketcheck')
AddEventHandler('cardealeredm:ticketcheck', function(supporter)
	local src = source
	local support = cb

	if support == 1 then
		--buymon(currentModel, VehCoords, Heading, player, carPrice, carCommission, price, payment)
	else
		exports['mythic_notify']:DoHudText('inform', "You need Ticket to Buy From EDM!")
	end
end)
...
-- REGISTRATION PAPER:
local open = false
RegisterNetEvent('carealeredm:openRegCL')
AddEventHandler('carealeredm:openRegCL', function(data, vehPlate)
	open = true
...
Citizen.CreateThread(function()
    ...
    RegisterNetEvent('carealeredm:ReplaceVehCL')
    AddEventHandler('carealeredm:ReplaceVehCL', function(car, currentID)
		ReplaceVehicle(car, currentID)
		SetVehicleDirtLevel(car, 0)
    end)

    -- Server called event to get car dealer data:
    RegisterNetEvent('carealeredm:DealerDiscountCL')
    AddEventHandler('carealeredm:DealerDiscountCL', function()
        GetCarDealerData()
        ChangingValues = false
    end)

    -- Server called event to get car dealer data:
    RegisterNetEvent('carealeredm:DealerDownpaymentCL')
    AddEventHandler('carealeredm:DealerDownpaymentCL', function()
        GetCarDealerData()
        ChangingValues = false
    end)
