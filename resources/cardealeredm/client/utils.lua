...
RegisterNetEvent('carealeredm:ShowNotifyHHCore')
AddEventHandler('carealeredm:ShowNotifyHHCore', function(msg)
...
function ProduceNumberPlate()
	...
	while true do
		...
		HHCore.TriggerServerCallback('carealeredm:PlateInUse', function (PlateInUse)
	...
	if option == "repay" then
		if plate ~= nil or not plate == '' then
			if amount ~= nil or amount >= 1 then
				HHCore.TriggerServerCallback('carealeredm:GetOwnedVehByPlate', function(vehPlate, vehPrice, vehHash, vehFinance, vehRepaytime)
		...
		if plate ~= nil or not plate == '' then
			HHCore.TriggerServerCallback('carealeredm:GetOwnedVehByPlate', function(vehPlate, vehPrice, vehHash, vehFinance, vehRepaytime)
				...
				if vehFinance < 1 then
					...
					ShowNotifyHHCore("Amount Owed: ~w~$"..vehFinance.."~w~. Pay Next Repayment Before: ~b~"..math.floor(vehRepaytime/60).."~w~ Hours")
				end
