...
TryToCrack = function(safeEntity)
	...
	RKCore.TriggerServerCallback("safebreach:checkIfSafeIsBreachable", function(state)
		if state then
			...
			TriggerEvent('DoLongHudText', 'No enough police to start the robbery.', 2)
			TriggerEvent('DoLongHudText', 'No enough police to start the robbery.', 2)
		end
	...
	Citizen.CreateThread(function()
		while isCracking do
			...
			for i = 1, Config.TotalLocks do
				...
				DrawSprite("MPSafeCracking", lockState, 0.1 + (i / 15), 0.6, 0.2 * 0.2, 0.3 * 0.2, 0.0, 255, 255, 255, 255)
			end
		end
			...
			if IsControlJustPressed(0, 22) then
				if dialRotation == currentSafeCorrectPosition then
					...
					if #safeLocks < Config.TotalLocks then
						...
						RKCore.TriggerServerCallback("safebreach:safeBreached", function(breached)
							if breached then
								local breachObjects = SpawnBreachObjects(safeEntity)
								...
								TriggerEvent('DoLongHudText', 'You cracked the safe!', 1)
							else
								TriggerEvent('DoLongHudText', 'The safe is already open!', 2)
							end
						end, {
							["store"] = currentStreetName,
							["doorCoords"] = {
					...
					TriggerEvent('DoLongHudText', 'Wrong, Try again and listen carefully!', 2)
				end
...
TakeSafeItem = function(itemEntity, safeData)
	...
	if DoesEntityExist(itemEntity) then
		...
		RKCore.TriggerServerCallback("safebreach:receiveReward", function(received)
			if received then
		...
		TriggerEvent('DoLongHudText', 'Someone stole it before you!', 2)
	end
end
