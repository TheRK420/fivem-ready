...
AddEventHandler("rk:playerLoaded", function()
	...
	RKCore.TriggerServerCallback("safebreach:checkSafeBreaches", function(successfull)
		if successfull then
				...
				for objectIndex = 1, #eventData["objects"] do
					if NetworkDoesEntityExistWithNetworkId(eventData["objects"][objectIndex]) then
						...
						if dstCheck <= 2.0 then
							...
							RKCore.Game.Utils.DrawText3D(GetEntityCoords(breachObject), "[~g~G~s~]", 0.4)
						end
					end
				end
			...
			if dstCheck <= 2.5 then 
				...
				if display then
					...
					if dstCheck <= 1.0 then
						local displayText = isCracking and "Police alerted they are on the way!" or "~INPUT_CONTEXT~ To start cracking."

						RKCore.ShowHelpNotification(displayText)

						if IsControlJustReleased(0, 38) then
							if exports['rk-inventory']:hasEnoughOfItem('advlockpick', 1) then
