...
AddEventHandler("safebreach:globalEvent", function(options)
    -- RKCore.Trace((options["event"] or "none") .. " triggered to all clients.")
...
RKCore.RegisterServerCallback("safebreach:checkSafeBreaches", function(source, cb)
...
RKCore.RegisterServerCallback("safebreach:checkIfSafeIsBreachable", function(source, cb, safe)
	local player = RKCore.GetPlayerFromId(source)
	...
	if player then
		if cachedSafes[safe] then
			...
			for i = 1, #players do
				local player = RKCore.GetPlayerFromId(players[i])
...
RKCore.RegisterServerCallback("safebreach:safeBreached", function(source, cb, safeData)
	local player = RKCore.GetPlayerFromId(source)
...
RKCore.RegisterServerCallback("safebreach:receiveReward", function(source, cb, reward)
	local player = RKCore.GetPlayerFromId(source)
