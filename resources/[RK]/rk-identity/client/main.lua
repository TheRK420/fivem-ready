...
RegisterNetEvent('rk-identity:showRegisterIdentity')
AddEventHandler('rk-identity:showRegisterIdentity', function()
	if not isDead then
		EnableGui(true)
	end
end)
...
RegisterNetEvent('rk-identity:saveID')
AddEventHandler('rk-identity:saveID', function(data)
	myIdentifiers = data
end)
...
RegisterNUICallback('register', function(data, cb)
    ...
    if reason == "" then
        ...
        DoScreenFadeIn(1000)
        TriggerServerEvent('rk-identity:setIdentity', data, myIdentifiers)
        EnableGui(false)
		TriggerEvent('rk-clothing:defaultReset', myIdentifiers.id)
