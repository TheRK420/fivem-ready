...
RegisterServerEvent('rk-identity:setIdentity')
AddEventHandler('rk-identity:setIdentity', function(data, myIdentifiers)
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			TriggerClientEvent('rk-identity:identityCheck', myIdentifiers.playerid, true)
		else
...
AddEventHandler('es:playerLoaded', function(source)
	...
	TriggerClientEvent('rk-identity:saveID', source, myID)
	getIdentity(source, function(data)
		if data.firstname == '' then
			TriggerClientEvent('rk-identity:identityCheck', source, false)
			TriggerClientEvent('rk-identity:showRegisterIdentity', source)
		else
			TriggerClientEvent('rk-identity:identityCheck', source, true)
		end
	end)
end)
...
AddEventHandler('onResourceStart', function(resource)
	if resource == GetCurrentResourceName() then
		...
		local xPlayers, xPlayer = RKCore.GetPlayers()
		for i=1, #xPlayers, 1 do
			xPlayer = RKCore.GetPlayerFromId(xPlayers[i])
			...
			TriggerClientEvent('rk-identity:saveID', xPlayer.source, myID)

			getIdentity(xPlayer.source, function(data)
				if data.firstname == '' then
					TriggerClientEvent('rk-identity:identityCheck', xPlayer.source, false)
					TriggerClientEvent('rk-identity:showRegisterIdentity', xPlayer.source)
				else
					TriggerClientEvent('rk-identity:identityCheck', xPlayer.source, true)
				end
			end)
		end
	end
