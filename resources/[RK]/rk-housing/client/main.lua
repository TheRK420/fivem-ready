...
local servername = 'rk Development'
local cam = nil
local viewCam = false
...
RegisterNetEvent('rk-housing:client:sellHouse')
AddEventHandler('rk-housing:client:sellHouse', function()
    if closesthouse ~= nil and hasKey then
        TriggerServerEvent('rk-housing:server:viewHouse', closesthouse)
    end
end)
...
AddEventHandler('rk:playerLoaded', function()
    TriggerServerEvent('rk-housing:client:setHouses')
    isLoggedIn = true
    SetClosestHouse()
    TriggerEvent('rk-housing:client:setupHouseBlips')
    Citizen.Wait(100)
    TriggerServerEvent("rk-housing:server:setHouses")
end)

RegisterNetEvent('rk-housing:client:setHouseConfig')
AddEventHandler('rk-housing:client:setHouseConfig', function(houseConfig)
    Config.Houses = houseConfig
    --TriggerEvent("rk-housing:client:refreshHouse")
	
end)

RegisterNetEvent('rk-housing:client:lockHouse')
AddEventHandler('rk-housing:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('rk-housing:client:createHouses')
AddEventHandler('rk-housing:client:createHouses', function(price, tier)
    ...
    TriggerServerEvent('rk-housing:server:addNewHouse', area, coords, price, tier)
end)

RegisterNetEvent('rk-housing:client:toggleDoorlock')
AddEventHandler('rk-housing:client:toggleDoorlock', function()
    ...
    if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
        if hasKey then
            if Config.Houses[closesthouse].locked then
                TriggerServerEvent('rk-housing:server:lockHouse', false, closesthouse)
				TriggerEvent('DoLongHudText', 'The Door is unlocked.', 1)
            else
                TriggerServerEvent('rk-housing:server:lockHouse', true, closesthouse)
                    ...
                    if CurrentDoorBell ~= 0 then
                        if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                            ...
                            if IsControlJustPressed(0, Keys["G"]) then
                                TriggerServerEvent("rk-housing:server:OpenDoor", CurrentDoorBell, closesthouse)
                                CurrentDoorBell = 0
                            end
                        end
                    end
                            ...
                            if not inOwned then
                                if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
                                    if not Config.Houses[closesthouse].locked then
                                        ...
                                        if IsControlJustPressed(0, Keys["G"]) then
                                            TriggerServerEvent('rk-housing:server:RingDoor', closesthouse)
                                        end
                                    end
                                end
                            elseif inOwned then
                    ...
                    if closesthouse ~= nil then
                        if logoutLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, logoutLocation.x, logoutLocation.y, logoutLocation.z, true) < 1.5)then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerEvent('rk-login:swapcharacter')
                                    leaveOwnedHouse(closesthouse)
                                    showClothing = false
                                end
...
RegisterNetEvent('rk-housing:client:RingDoor')
AddEventHandler('rk-housing:client:RingDoor', function(player, house)
...
RegisterNetEvent('rk-housing:client:giveHouseKey')
AddEventHandler('rk-housing:client:giveHouseKey', function(data)
    ...
    if player ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('rk-housing:server:giveHouseKey', playerId, closesthouse)
        else
...
RegisterNetEvent('rk-housing:client:removeHouseKey')
AddEventHandler('rk-housing:client:removeHouseKey', function(data)
    if closesthouse ~= nil then 
        local housedist = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z)
        if housedist < 5 then
            RKCore.TriggerServerCallback('rk-housing:server:getHouseOwner', function(result)
                if RKCore.GetPlayerData().identifier == result then
                    inHoldersMenu = true
                    HouseKeysMenu()
...
RegisterNetEvent('rk-housing:client:refreshHouse')
AddEventHandler('rk-housing:client:refreshHouse', function(data)
    Citizen.Wait(100)
    SetClosestHouse()
end)

RegisterNetEvent('rk-housing:client:setstash')
AddEventHandler('rk-housing:client:setstash', function(data)
ExecuteCommand("setlocation stash")
end)

RegisterNetEvent('rk-housing:client:setstash2')
AddEventHandler('rk-housing:client:setstash2', function(data)
ExecuteCommand("setlocation stash2")
end)

RegisterNetEvent('rk-housing:client:setoutfit')
AddEventHandler('rk-housing:client:setoutfit', function(data)
ExecuteCommand("setlocation outfit")
end)

RegisterNetEvent('rk-housing:client:setlogout')
AddEventHandler('rk-housing:client:setlogout', function(data)
ExecuteCommand("setlocation logout")
end)

RegisterNetEvent('rk-housing:client:SpawnInApartment')
AddEventHandler('rk-housing:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(GetPlayerPed(-1))
...
function removeHouseKey(citizenData)
    TriggerServerEvent('rk-housing:server:removeHouseKey', closesthouse, citizenData)
    closeMenuFull()
end
...
function enterOwnedHouse(house)
    ...
    if Config.Houses[house].tier == 1 then
        data = exports['rk-interior']:CreateTier1House(coords)
	elseif Config.Houses[house].tier == 2 then
        data = exports['rk-interior']:CreateMichaelShell(coords)
	elseif Config.Houses[house].tier == 3 then
        data = exports['rk-interior']:CreateTrevorsShell(coords)
	elseif Config.Houses[house].tier == 4 then
        data = exports['rk-interior']:CreateFranklinShell(coords)
	elseif Config.Houses[house].tier == 5 then
        data = exports['rk-interior']:CreateFranklinAuntShell(coords)
	elseif Config.Houses[house].tier == 6 then
        data = exports['rk-interior']:CreateApartmentShell(coords)
	elseif Config.Houses[house].tier == 7 then
        data = exports['rk-interior']:CreateCaravanShell(coords)
    end
    Citizen.Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
    ...
    TriggerEvent('rk-weathersync:client:DisableSync')
    TriggerEvent('rk-weed:client:getHousePlants', house)
    Citizen.Wait(100)
...
RegisterNetEvent('rk-housing:client:enterOwnedHouse')
AddEventHandler('rk-housing:client:enterOwnedHouse', function(house)
	enterOwnedHouse(house)
end)

RegisterNUICallback('HasEnoughMoney', function(data, cb)
    RKCore.TriggerServerCallback('rk-housing:server:HasEnoughMoney', function(hasEnough)
        
    end, data.objectData)
end)

RegisterNetEvent('rk-housing:client:LastLocationHouse')
AddEventHandler('rk-housing:client:LastLocationHouse', function(houseId)
...
function leaveOwnedHouse(house)
    ...
    DoScreenFadeOut(250)
    Citizen.Wait(500)
    exports['rk-interior']:DespawnInterior(houseObj, function()
        UnloadDecorations()
        TriggerEvent('rk-weathersync:client:EnableSync')
        Citizen.Wait(250)
        DoScreenFadeIn(250)
        ...
        inside = false
        TriggerEvent('rk-weed:client:leaveHouse')
        showClothing = false
    end)
end
...
function enterNonOwnedHouse(house)
    ...
    Citizen.Wait(500)
    SetRainFxIntensity(0.0)
    TriggerServerEvent('rk-housing:server:SetInsideMeta', house, true)
    TriggerEvent('rk-weathersync:client:DisableSync')
    TriggerEvent('rk-weed:client:getHousePlants', house)
    Citizen.Wait(100)
    --SetWeatherTypePersist('CLEAR')
...
function leaveNonOwnedHouse(house)
    ...
    exports['rk-interior']:DespawnInterior(houseObj, function()
        ...
        showClothing = false
        TriggerEvent('rk-weed:client:leaveHouse')
        TriggerServerEvent('rk-housing:server:SetInsideMeta', house, false)
    end)
end
...
function HouseKeysMenu()
    ...
    MenuTitle = "Keys"
    ClearMenu()
    RKCore.TriggerServerCallback('rk-housing:server:getHouseKeyHolders', function(holders)
        ped = GetPlayerPed(-1);
...
RegisterNetEvent('rk-housing:client:setupHouseBlips')
AddEventHandler('rk-housing:client:setupHouseBlips', function()
    Citizen.CreateThread(function()
        Citizen.Wait(2000)
        if isLoggedIn then
            RKCore.TriggerServerCallback('rk-housing:server:getOwnedHouses', function(ownedHouses)
...
RegisterNetEvent('rk-housing:client:SetClosestHouse')
AddEventHandler('rk-housing:client:SetClosestHouse', function()
    SetClosestHouse()
end)
...
RegisterNUICallback('buy', function()
    openContract(false)
    disableViewCam()
    TriggerServerEvent('rk-housing:server:buyHouse', closesthouse)
end)
...
RegisterNetEvent('rk-housing:client:viewHouse')
AddEventHandler('rk-housing:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
function SetClosestHouse()
    ...
    if not inside then
        ...
        if closesthouse ~= nil then 
            RKCore.TriggerServerCallback('rk-housing:server:hasKey', function(result)
                hasKey = result
            end, closesthouse)
        
            RKCore.TriggerServerCallback('rk-housing:server:isOwned', function(result)
                isOwned = result
            end, closesthouse)
        end
    end
...
function setHouseLocations()
    if closesthouse ~= nil then
        RKCore.TriggerServerCallback('rk-housing:server:getHouseLocations', function(result)
            if result ~= nil then
...
RegisterNetEvent('rk-housing:client:setLocation')
AddEventHandler('rk-housing:client:setLocation', function(data)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    ...
    if inside then
        if hasKey then
            if data == 'stash' then
                TriggerServerEvent('rk-housing:server:setLocation', coords, closesthouse, 1)
            elseif data == 'outift' then
                TriggerServerEvent('rk-housing:server:setLocation', coords, closesthouse, 2)
            elseif data == 'logout' then
                TriggerServerEvent('rk-housing:server:setLocation', coords, closesthouse, 3)
			elseif data == 'stash2' then
                TriggerServerEvent('rk-housing:server:setLocation', coords, closesthouse, 4)
            end
        else
			TriggerEvent('DoLongHudText', 'You dont have keys to this house', 2)
        end
    elseif data == 'garage' then
        TriggerServerEvent('rk-housing:server:setLocation', coords, closesthouse, 5)
    else
		TriggerEvent('DoLongHudText', 'You are not inside any house', 2)
    end
end)

RegisterNetEvent('rk-housing:client:refreshLocations')
AddEventHandler('rk-housing:client:refreshLocations', function(house, location, type)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)

    if closesthouse == house then
        if inside then
            TriggerEvent('rk-housing:getid', closesthouse, house)
            if type == 1 then
