...
RegisterNetEvent("rk-housing:client:decorate")
AddEventHandler("rk-housing:client:decorate", function()
	Citizen.Wait(500)
	if inside then 
		if hasKey then 
...
RegisterNetEvent("rk-housing:server:sethousedecorations")
AddEventHandler("rk-housing:server:sethousedecorations", function(house, decorations)
...
function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		RKCore.TriggerServerCallback('rk-housing:server:getHouseDecorations', function(result)
...
function SaveDecorations()
	if closesthouse ~= nil then
		...
		TriggerServerEvent("rk-housing:server:savedecorations", closesthouse, ObjectList)
	end
end
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, Keys["LEFTSHIFT"]) then
		...
		RKCore.ShowNotification('Speed is '.. tostring(speeds[curSpeed]))
	end
