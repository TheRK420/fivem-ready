...
function rk.Admin.CheckForSessions(self)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            local players = rk._Admin.Players

            for k,v in pairs(players) do
                local src = v.src
                local playerId = GetPlayerFromServerId(src)
                if not src then
                    rk._Admin.Players[src].sessioned = true
                else
                    if not NetworkIsPlayerActive(playerId) or not NetworkIsPlayerConnected(playerId) then rk._Admin.Players[src].sessioned = true end
                end
            end
        end
    end)
...
stop = true
function rk.Admin.BeginAFK(self, stop)
    if stop then
        afk.isAfk = false
        afk.msgAFK = false
        afk.stringToType = ""
    ...
    afk.event = AddEventHandler("rk-admin:afkStringCheck", function(text)
        if string.lower(text) == afk.stringToType then rk.Admin:BeginAFK(true) return end
    end)
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if curTime - beginTime >= 300000 then TriggerServerEvent("rk-admin:Disconnect", "AFK Kick") return end
        end
    end)
end

function rk.Admin.AFKCheck(self)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            local rank = rk.Admin:GetPlayerRank()
            local rankData = rk.Admin:GetRankData(rank)
...
RegisterNetEvent("rk-admin:afkStringCheck")

RegisterNetEvent("rk-admin:setStatus")
AddEventHandler("rk-admin:setStatus", function(src, status)
    local player = rk._Admin.Players[src]
    if not player then return else rk._Admin.Players[src].status = status end
end)

RegisterNetEvent("rk-admin:sendPlayerInfo")
AddEventHandler("rk-admin:sendPlayerInfo", function(data, discData)
    rk._Admin.Players = data
    rk._Admin.DiscPlayers = discData
end)

RegisterNetEvent("rk-admin:RemovePlayer")
AddEventHandler("rk-admin:RemovePlayer", function(data)
    rk._Admin.DiscPlayers[data.src] = data
    rk._Admin.Players[data.src] = nil
end)

RegisterNetEvent("rk-admin:RemoveRecent")
AddEventHandler("rk-admin:RemoveRecent", function(data)
    if data ~= nil then
        rk._Admin.DiscPlayers[data.src] = nil
    end
end)

RegisterNetEvent("rk-admin:AddPlayer")
AddEventHandler("rk-admin:AddPlayer", function(player)
    rk._Admin.Players[player.src] = player
end)

RegisterNetEvent('rk-admin:teleportUser')
AddEventHandler('rk-admin:teleportUser', function(x, y, z)
    SetEntityCoords(PlayerPedId(), x, y, z)
    TriggerEvent('DoLongHudText', 'You were summoned')
end)

RegisterNetEvent("rk-admin:AddAllPlayers")
AddEventHandler("rk-admin:AddAllPlayers", function(data)
    rk._Admin.Players[data.src] = data
end)

function rk.Admin.RunCommand(self, args)
    if not args or not args.command then return end
    TriggerServerEvent("rk-admin:runCommand", args)
end

function rk.Admin.RunClCommand(self, cmd, args)
    if not cmd or not self:CommandExists(cmd) then return end
    self:GetCommandData(cmd).runclcommand(args)
end

RegisterNetEvent("rk-admin:RunClCommand")
AddEventHandler("rk-admin:RunClCommand", function(cmd, args)
    rk.Admin:RunClCommand(cmd, args)
end)

RegisterNetEvent("rk-admin:updateData")
AddEventHandler("rk-admin:updateData", function(src, type, data)
    if not src or not type or not data then return end
    if not rk._Admin.Players[src] then return end
    
    rk._Admin.Players[src][type] = data
end)

RegisterNetEvent("rk-admin:noLongerAdmin")
AddEventHandler("rk-admin:noLongerAdmin", function()
    rk._Admin.Players = {}
...
RegisterNetEvent("rk-admin:bringPlayer")
AddEventHandler("rk-admin:bringPlayer", function(targPos)
    local ped = PlayerPedId()
...
RegisterNetEvent("rk-admin:bring")
AddEventHandler("rk-admin:bring", function(target)
    ...
    TriggerServerEvent("rk-admin:bringPlayerServer",pos,target)   
end)



 
RegisterNetEvent("rk-admin:runSpawnCommand")
AddEventHandler("rk-admin:runSpawnCommand", function(model, livery)
    Citizen.CreateThread(function()
...
function rk.Admin.teleportMarker(self)
...
function rk.Admin.GetBanTimeFromString(self, time)
...
RegisterNetEvent("rk-admin:ReviveInDistance")
AddEventHandler("rk-admin:ReviveInDistance", function()
    local playerList = {}
