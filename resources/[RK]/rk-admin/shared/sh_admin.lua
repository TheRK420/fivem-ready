...
function rk.Admin.Log(self, log, user)
...
function rk.Admin.ChatPrint(self, msg, target)

end

function rk.Admin.RankExists(self, rank)
    rank = tostring(rank)
...
function rk.Admin.CommandExists(self, cmd)
    cmd = tostring(cmd)
...
function rk.Admin.RankInheritsCommand(self, rank, cmd)
...
function rk.Admin.GetPlayerRank(self, user)
    if not IsDuplicityVersion() then
        return group
    end
end

function rk.Admin.RankHasCommand(self, rank, cmd)
...
function rk.Admin.GetRanks(self)
    return rk._Admin.Ranks
end

function rk.Admin.GetRankData(self, rank)
    if not rank then return false end
...
function rk.Admin.GetCommands(self)
    return rk._Admin.Commands
end

function rk.Admin.GetCategories(self)
    local tmp = {}
...
function rk.Admin.GetCommandData(self, cmd)
    if not cmd then return false end
...
function rk.Admin.IsAdmin(self, user)
...
function rk.Admin.IsSuperAdmin(self, user)
    if not IsDuplicityVersion() then
...
function rk.Admin.IsValidUser(self, user)
    if not user then return false end
...
function rk.Admin.CanTarget(self, user, target)
...
function rk.Admin.CanTargetRank(self, user, rank)
...
function rk.Admin.AddCommand(self, cmd)
    local tmp = {}
    ...
    if chicken then
        ...
        tmp.runcommand = function(args) args.command = tmp.command rk.Admin:RunCommand(args) end
...
function rk.Admin.AddRank(self, rank, data)
    local tmp = {}
...
function rk.Admin.SetStatus(self, status, src)
    ...
    if IsDuplicityVersion() then
        ...
        if not player then else rk._Admin.Players[src].status = status end

        for k,v in pairs(rk._Admin.CurAdmins) do
            TriggerClientEvent("rk-admin:setStatus", k, src, status)
        end

        return
    end

    TriggerServerEvent("rk-admin:setStatus", status)
end

AddEventHandler("np-base:exportsReady", function()
    exports["np-base"]:addModule("Admin", rk.Admin)
    exports["np-base"]:addModule("_Admin", rk._Admin)
end)