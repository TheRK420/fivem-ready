rk = rk or {}
rk.Doors = rk.Doors or {}
rk.DoorCoords = rk.DoorCoords or {}
rk.offSet = rk.offSet or {}
...
function rk.alterState(alterNum)

    if rk.DoorCoords[alterNum]["lock"] == 0 then 
        rk.DoorCoords[alterNum]["lock"] = 1 
    else 
        rk.DoorCoords[alterNum]["lock"] = 0 
    end
    TriggerClientEvent("rk:Door:alterState",-1,alterNum,rk.DoorCoords[alterNum]["lock"])

end

RegisterNetEvent( 'rk:Door:alterState' )
AddEventHandler( 'rk:Door:alterState', function(alterNum,num)
	rk.DoorCoords[alterNum]["lock"] = num
end)

RegisterNetEvent("rk-doors:alterlockstateclient")
AddEventHandler("rk-doors:alterlockstateclient", function(doorCoords)
    rk.DoorCoords = doorCoords
end)
