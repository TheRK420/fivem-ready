...
RegisterNetEvent('rk-jewelrobbery:policeclosure')
AddEventHandler('rk-jewelrobbery:policeclosure', function()
	policeclosed = true
	storeclosed = false
	IsAbleToRob = false
end)
...
AddEventHandler('rk:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	TriggerServerEvent('rk-jewelrobbery:loadconfig')
end)

RegisterNetEvent('rk-jewelrobbery:resetcases')
AddEventHandler('rk-jewelrobbery:resetcases', function(list)
...
RegisterNetEvent('rk-jewelrobbery:setcase')
...
RegisterNetEvent('rk-jewelrobbery:loadconfig')
AddEventHandler('rk-jewelrobbery:loadconfig', function(casestatus)
...
RegisterNetEvent('rk-jewelrobbery:playsound')
...
AddEventHandler('rk-jewelrobbery:EnteredArea', function()
	for i, v in pairs(Config.CaseLocations) do
		if v.Broken then
			CreateModelSwap(v.Pos.x, v.Pos.y, v.Pos.z, 0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
		end
...
AddEventHandler('rk-jewelrobbery:LeftArea', function()
	for i, v in pairs(Config.CaseLocations) do
		if v.Broken then
			RemoveModelSwap(v.Pos.x, v.Pos.y, v.Pos.z, 0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
		end
...
Citizen.CreateThread( function()
	while true do 
		...
		if IsInArea and not HasAlreadyEnteredArea then
			TriggerEvent('rk-jewelrobbery:EnteredArea')
			shockingevent = false
		...
		if not IsInArea and HasAlreadyEnteredArea then
			TriggerEvent('rk-jewelrobbery:LeftArea')
			HasAlreadyEnteredArea = false
			shockingevent = false
			IsAbleToRob = false
			storeclosed = false
...
function CheckPolice()
	if CopsOnline ~= exports["rk-jobnumbers"]:jobonline('cops') then
		HasAlreadyEnteredArea = false
	end
	CopsOnline = exports["rk-jobnumbers"]:jobonline('cops')
	return exports["rk-jobnumbers"]:jobonline('cops')
end
						...
						if IsControlJustPressed(0, 38) and not IsBusy and not IsPedWalking(ply) and not IsPedRunning(ply) and not IsPedSprinting(ply) then
							...
							if breakchance <= robalbe.chance then
								if policenotify <= Config.PoliceNotifyBroken and not HasNotified then
									local playerCoords = GetEntityCoords(PlayerPedId())
									TriggerServerEvent('rk-jewelrobbery:policenotify')
									HasNotified = true
								end
								Citizen.Wait(2100)
								TriggerServerEvent('rk-jewelrobbery:playsound', v.Pos.x, v.Pos.y, v.Pos.z, 'break')
								CreateModelSwap(v.Pos.x, v.Pos.y, v.Pos.z,  0.1, GetHashKey(v.Prop1), GetHashKey(v.Prop), false )
								ClearPedTasksImmediately(ply)
								TriggerServerEvent("rk-jewelrobbery:setcase", i, true)	
							else
								Citizen.Wait(2100)
								TriggerServerEvent('rk-jewelrobbery:playsound', v.Pos.x, v.Pos.y, v.Pos.z, 'nonbreak')
								ClearPedTasksImmediately(ply)
        ...
        if distance < 3.0 then
        	Wait(1)
             ...
             if IsControlJustReleased(0,38) and distance < 1.0 then
             	if exports["rk-inventory"]:hasEnoughOfItem("Gruppe6Card3",1,false) then
             		TriggerEvent("inventory:removeItem", "Gruppe6Card3", 1)
             		TriggerServerEvent("rk-doors:alterlockstate",61)
             		TriggerServerEvent("rk-doors:alterlockstate",62)
             	end
             end
		else
            Wait(3000)
