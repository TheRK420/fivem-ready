...
function SetVehicleMaxMods(vehicle)
  ...
  RKCore.Game.SetVehicleProperties(vehicle, props)
    ...
    function(data, menu)
      if data.current.value == 'vehicle_list' then
        ...
        if Config.EnableSocietyOwnedVehicles then
            ...
            RKCore.TriggerServerCallback('rk_society:getVehiclesInGarage', function(vehicles)
              ...
              RKCore.UI.Menu.Open(
                ...
                function(data, menu)
                  ...
                  RKCore.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
                    RKCore.Game.SetVehicleProperties(vehicle, vehicleProps)
                    local playerPed = GetPlayerPed(-1)
                    TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
                  end)

                  TriggerServerEvent('rk_society:removeVehicleFromGarage', 'weazelnews', vehicleProps)
        ...
        {label = 'Weazel News Heli', value = 'newsheli'},
        {label = 'Weazel News Compact Heli', value = 'newsheli2'}
            }
              ...
              function(data, menu)
                for i=1, #elements, 1 do
                  if Config.MaxInService == -1 then
                    RKCore.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 90.0, function(vehicle)
                      local playerPed = GetPlayerPed(-1)
                      TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
                    ...
                    RKCore.TriggerServerCallback('rk_service:enableService', function(canTakeService, maxInService, inServiceCount)
                      if canTakeService then
                        ...
                        RKCore.ShowNotification(_U('service_full') .. inServiceCount .. '/' .. maxInService)
                      end
                    end, 'weazelnews')
                    break
                  end
      ...
      if data.current.value == 'boss_actions' then
        TriggerEvent('rk_society:openBossMenu', 'weazelnews', function(data, menu)
          menu.close()
        end)
      end
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('rk_billing:sendBill1', GetPlayerServerId(player), 'society_weazelnews', _U('billing'), amount)
        end

      else
        RKCore.ShowNotification(_U('no_players_nearby'))
      end
...
function openMenucam()
	...
	'default', GetCurrentResourceName(), 'id_card_menu',
	{
      ...
      {label = '| 🎤 | Take/Put away the Boom Microphone', value = 'BMic'},
...
function OpenGetStocksMenu()
  ...
  RKCore.TriggerServerCallback('sp_weazelnews:getStockItems', function(items)
...
RKCore.TriggerServerCallback('sp_weazelnews:getPlayerInventory', function(inventory)
  ...
  if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
    ...
    if DoesEntityExist(vehicle) then
      ...
      Citizen.CreateThread(function()
        ...
        RKCore.ShowNotification(_U('veh_repaired'))
      end)
    end
  end
end)
          ...
          if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'weazelnews' then
            ...
            if CurrentAction == 'delete_vehicle' then
              ...
              if Config.EnableSocietyOwnedVehicles then
                ...
                TriggerServerEvent('rk_society:putVehicleInGarage', 'weazelnews', vehicleProps)
                ...
                then
                  TriggerServerEvent('rk_service:disableService', 'weazelnews')
                end
