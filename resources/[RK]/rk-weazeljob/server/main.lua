...
if Config.MaxInService ~= -1 then
  TriggerEvent('rk_service:activateService', 'weazelnews', Config.MaxInService)
end

TriggerEvent('rk_society:registerSociety', 'weazelnews', 'Karting', 'society_weazelnews', 'society_weazelnews', 'society_weazelnews', {type = 'private'})
...
AddEventHandler('sp_weazelnews:getStockItem', function(itemName, count)
  ...
  TriggerEvent('rk_addoninventory:getSharedInventory', 'society_weazelnews', function(inventory)
    ...
    TriggerClientEvent('rk:showNotification', xPlayer.source, _U('you_removed') .. count .. ' ' .. item.label)

  end)

end)

RKCore.RegisterServerCallback('sp_weazelnews:getStockItems', function(source, cb)
...
AddEventHandler('sp_weazelnews:putStockItems', function(itemName, count)
  ...
  TriggerEvent('rk_addoninventory:getSharedInventory', 'society_weazelnews', function(inventory)
    ...
    TriggerClientEvent('rk:showNotification', xPlayer.source, _U('you_added') .. count .. ' ' .. item.label)

  end)

end)

--RKCore.RegisterServerCallback('sp_weazelnews:putStockItems', function(source, cb)

--  TriggerEvent('rk_addoninventory:getSharedInventory', 'society_policestock', function(inventory)
--    cb(inventory.items)
--  end)

--end)

RKCore.RegisterServerCallback('sp_weazelnews:getPlayerInventory', function(source, cb)
