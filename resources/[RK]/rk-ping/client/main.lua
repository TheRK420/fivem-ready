    ...
    Citizen.CreateThread(function()
        ...
        while isPending do
            ...
            if count >= Config.Timeout and isPending then
                ...
                TriggerServerEvent('rk-ping:server:SendPingResult', pendingPing.id, 'timeout')
                pendingPing = nil
                isPending = false
            end
            Citizen.Wait(1000)
...
RegisterNetEvent('rk-ping:client:SendPing')
AddEventHandler('rk-ping:client:SendPing', function(sender, senderId)
    if pendingPing == nil then
        ...
        TriggerServerEvent('rk-ping:server:SendPingResult', pendingPing.id, 'received')
        ...
        TriggerServerEvent('rk-ping:server:SendPingResult', senderId, 'unable')
    end
end)

RegisterNetEvent('rk-ping:client:AcceptPing')
AddEventHandler('rk-ping:client:AcceptPing', function()
    if isPending then
        ...
        TriggerServerEvent('rk-ping:server:SendPingResult', pendingPing.id, 'accept')
        isPending = false
    else
...
RegisterNetEvent('rk-ping:client:RejectPing')
AddEventHandler('rk-ping:client:RejectPing', function()
    if pendingPing ~= nil then
        TriggerEvent('DoLongHudText', 'Rejected Ping From ' .. pendingPing.name, 2)
        TriggerServerEvent('rk-ping:server:SendPingResult', pendingPing.id, 'reject')
        pendingPing = nil
        isPending = false
    else
        TriggerEvent('DoLongHudText', 'You Have No Pending Ping', 2)
...
RegisterNetEvent('rk-ping:client:RemovePing')
AddEventHandler('rk-ping:client:RemovePing', function()
    if pendingPing ~= nil then
        RemoveBlip(pendingPing.blip)
        pendingPing = nil
