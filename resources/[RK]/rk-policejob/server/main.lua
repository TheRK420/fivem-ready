...
if Config.MaxInService ~= -1 then
	TriggerEvent('rk-service:activateService', 'police', Config.MaxInService)
end
...
AddEventHandler('playerDropped', function()
	...
	if _source ~= nil then
		...
		local user = RKCore.GetPlayerFromId(_source)
		--local player = user.identifier
		-- Is it worth telling all clients to refresh?
		if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'police' then
			Citizen.Wait(5000)
			TriggerClientEvent('rk-policejob:updateBlip', -1)
...
RegisterServerEvent('rk-policejob:spawned')
AddEventHandler('rk-policejob:spawned', function()
	local _source = source
	local xPlayer = RKCore.GetPlayerFromId(_source)
...
RegisterServerEvent('rk-policejob:forceBlip')
AddEventHandler('rk-policejob:forceBlip', function()
	TriggerClientEvent('rk-policejob:updateBlip', -1)
end)
...
AddEventHandler('reviveGranted', function(target)
	local xPlayer = RKCore.GetPlayerFromId(source)
		...
		--TriggerClientEvent('RKCore_ambulancejob:revive', target)
		Wait(5000)
...
RegisterCommand('givelicense', function(source, cb)
	...
	for k, v in ipairs(players) do
	  ...
	  local xSource = RKCore.GetPlayerFromId(source)
		...
		TriggerEvent('rk-license:checkLicense', v, 'weapon', function(cb)
		  if cb == false then 
			TriggerEvent('rk-license:addLicense', v, 'weapon', function()
			end)
