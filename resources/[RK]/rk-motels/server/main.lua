...
RKCore.RegisterServerCallback('pw-motels:myIdent', function(source, cb)
    src = source
    local xPlayer = RKCore.GetPlayerFromId(src)
    local ident = xPlayer.getIdentifier()
    cb(ident)
...
RKCore.RegisterServerCallback('motels:mycash', function(source, cb)
    src = source
    local xPlayer = RKCore.GetPlayerFromId(src)
        ...
        for i=1, #owners, 1 do
            ...
            for k,v in pairs(Config.Rooms) do
                if owners[i].room == tostring(v.roomno) and owners[i].motelid == v.motelid then
                    local xPlayer = RKCore.GetPlayerFromIdentifier(owner)
                    if xPlayer then
...
RKCore.RegisterServerCallback('pw-motels:checkUserOnline', function(source, cb, motel, room)
    for k,v in pairs(Config.Rooms) do
        if motel == v.motelid and room == v.roomno then
            if v.ident == nil then
                cb(true)
            else
                local xPlayer = RKCore.GetPlayerFromIdentifier(v.ident)
                if xPlayer then
                    cb(true)
                else
                    cb(false)
