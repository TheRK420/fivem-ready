...
$('.create-button').click(function () {
    ...
    $.post("http://rk-characters/CharacterChosen", JSON.stringify({
...
$("#deletechar").click(function () {
	...
	if (chosenCharacterId > 0) {
		$.post("http://rk-characters/DeleteCharacter", JSON.stringify({
			charid: Number(chosenCharacterId),
		}));
    }
    ...
    window.onload = function(e) {
        ...
        document.onkeydown = function(data) {
            if (data.which == 13 && IsInMainMenu) {
                ...
                $.post("http://rk-characters/ShowSelection", JSON.stringify({}));
                ShowLoading(true)
            }
        }
    }
...
function disconnect(){
    $.post("http://rk-characters/DisconnectGame", JSON.stringify({
    }));
