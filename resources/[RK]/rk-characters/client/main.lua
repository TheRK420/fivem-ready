...
Citizen.CreateThread(function()
    while true do
        ...
        if NetworkIsSessionStarted() then
			...
			TriggerServerEvent("rk-scoreboard:AddPlayer")
			TriggerServerEvent('rk-admin:AddPlayer')
...
RegisterNetEvent('rk-login:swapcharacter')
AddEventHandler('rk-login:swapcharacter', function()
	TriggerEvent("kashactersC:WelcomePage")
...
local function disconnect()
    TriggerServerEvent("rk-login:disconnectPlayer")
end
...
AddEventHandler('kashactersC:SpawnCharacter', function(spawn, isnew)
	...
	if isnew then
		...
		TriggerEvent('rk-identity:showRegisterIdentity')
		...
		IsChoosing = false
		Citizen.Wait(5)
		TriggerEvent("spn", pos)
		TriggerEvent("hotel:createRoom")
		TriggerEvent("DoLongHudText", "Tax is currently set to: 0%", 1)
		TriggerServerEvent("rk-clothing:get_character_current")
