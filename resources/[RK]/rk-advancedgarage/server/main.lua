...
MySQL.ready(function()
	if Config.ParkVehicles then
		...
		print('rk_advancedgarage: Parking Vehicles on restart is currently set to false.')
	end
end)
...
function ParkVehicles()
	MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = true WHERE `stored` = @stored', {
		...
		if rowsChanged > 0 then
			print(('rk_advancedgarage: %s vehicle(s) have been stored!'):format(rowsChanged))
		end
	end)
end
...
RKCore.RegisterServerCallback('rk_advancedgarage:getOwnedProperties', function(source, cb)
...
RKCore.RegisterServerCallback('rk_advancedgarage:getOwnedAircrafts', function(source, cb)
...
-- Fetch Owned Boats
RKCore.RegisterServerCallback('rk_advancedgarage:getOwnedBoats', function(source, cb)
	local ownedBoats = {}
	local xPlayer = RKCore.GetPlayerFromId(source)
...
-- Fetch Owned Cars
--[[ RKCore.RegisterServerCallback('rk_advancedgarage:getOwnedCars', function(source, cb)
	local ownedCars = {}
	local xPlayer = RKCore.GetPlayerFromId(source)
...
-- Store Vehicles
--[[ RKCore.RegisterServerCallback('rk_advancedgarage:storeVehicle', function (source, cb, vehicleProps , damage)
	local ownedCars = {}
		...
		if result[1] ~= nil then
			...
			if originalvehprops.model == vehiclemodel then
				MySQL.Async.execute('UPDATE owned_vehicles SET vehicle = @vehicle WHERE owner = @owner AND plate = @plate AND damage = @damage', {
					...
					if rowsChanged == 0 then
						print(('rk_advancedgarage: %s attempted to store an vehicle they don\'t own!'):format(xPlayer.identifier))
					end
					cb(true)
				end)
			else
				if Config.KickPossibleCheaters == true then
					if Config.UseCustomKickMessage == true then
						print(('rk_advancedgarage: %s attempted to Cheat! Tried Storing: ' .. vehiclemodel .. '. Original Vehicle: ' .. originalvehprops.model):format(xPlayer.identifier))
...
--[[ RKCore.RegisterServerCallback('tiger_cardealer:GetOwnedVehByPlate',function(source, cb, plate)	
	local ownedCars = {}
	local xPlayer = RKCore.GetPlayerFromId(source)
...
-- Store Vehicles
RKCore.RegisterServerCallback('rk_advancedgarage:storeVehicle', function (source, cb, vehicleProps)
	local ownedCars = {}
	local vehplate = vehicleProps.plate:match("^%s*(.-)%s*$")
	local vehiclemodel = vehicleProps.model
	local xPlayer = RKCore.GetPlayerFromId(source)
...
RKCore.RegisterServerCallback('rk_advancedgarage:getOutOwnedAircrafts', function(source, cb)
	local ownedAircrafts = {}
	local xPlayer = RKCore.GetPlayerFromId(source)
...
-- Fetch Pounded Boats
RKCore.RegisterServerCallback('rk_advancedgarage:getOutOwnedBoats', function(source, cb)
	local ownedBoats = {}
	local xPlayer = RKCore.GetPlayerFromId(source)
...
-- Fetch Pounded Cars
RKCore.RegisterServerCallback('rk_advancedgarage:getOutOwnedCars', function(source, cb)
	local ownedCars = {}
	local xPlayer = RKCore.GetPlayerFromId(source)
...
RKCore.RegisterServerCallback('rk_advancedgarage:getOutOwnedPolicingCars', function(source, cb)
...
RKCore.RegisterServerCallback('rk_advancedgarage:getOutOwnedAmbulanceCars', function(source, cb)
...
RKCore.RegisterServerCallback('rk_advancedgarage:checkMoneyAircrafts', function(source, cb)
	local xPlayer = RKCore.GetPlayerFromId(source)
...
RKCore.RegisterServerCallback('rk_advancedgarage:checkMoneyBoats', function(source, cb)
	local xPlayer = RKCore.GetPlayerFromId(source)
...
RKCore.RegisterServerCallback('rk_advancedgarage:checkMoneyCars', function(source, cb)
	local xPlayer = RKCore.GetPlayerFromId(source)
...
RKCore.RegisterServerCallback('rk_advancedgarage:checkMoneyPolicing', function(source, cb)
	local xPlayer = RKCore.GetPlayerFromId(source)
...
RKCore.RegisterServerCallback('rk_advancedgarage:checkMoneyAmbulance', function(source, cb)
	local xPlayer = RKCore.GetPlayerFromId(source)
...
RegisterServerEvent('rk_advancedgarage:payAircraft')
AddEventHandler('rk_advancedgarage:payAircraft', function()
	local xPlayer = RKCore.GetPlayerFromId(source)
	xPlayer.removeMoney(Config.AircraftPoundPrice)
	TriggerClientEvent('RKCore:showNotification', source, _U('you_paid') .. Config.AircraftPoundPrice)

	if Config.GiveSocietyMoney then
		TriggerEvent('rk_addonaccount:getSharedAccount', 'society_mechanic', function(account)
...
RegisterServerEvent('rk_advancedgarage:payBoat')
AddEventHandler('rk_advancedgarage:payBoat', function()
	local xPlayer = RKCore.GetPlayerFromId(source)
	xPlayer.removeMoney(Config.BoatPoundPrice)
	TriggerClientEvent('RKCore:showNotification', source, _U('you_paid') .. Config.BoatPoundPrice)
...
RegisterServerEvent('rk_advancedgarage:payCar')
AddEventHandler('rk_advancedgarage:payCar', function()
	local xPlayer = RKCore.GetPlayerFromId(source)
	xPlayer.removeMoney(Config.CarPoundPrice)
	TriggerClientEvent('RKCore:showNotification', source, _U('you_paid') .. Config.CarPoundPrice)
...
RegisterServerEvent('rk_advancedgarage:payPolicing')
AddEventHandler('rk_advancedgarage:payPolicing', function()
	local xPlayer = RKCore.GetPlayerFromId(source)
	xPlayer.removeMoney(Config.PolicingPoundPrice)
	TriggerClientEvent('RKCore:showNotification', source, _U('you_paid') .. Config.PolicingPoundPrice)
...
RegisterServerEvent('rk_advancedgarage:payAmbulance')
AddEventHandler('rk_advancedgarage:payAmbulance', function()
	local xPlayer = RKCore.GetPlayerFromId(source)
	xPlayer.removeMoney(Config.AmbulancePoundPrice)
	TriggerClientEvent('RKCore:showNotification', source, _U('you_paid') .. Config.AmbulancePoundPrice)
...
RegisterServerEvent('rk_advancedgarage:payhealth')
AddEventHandler('rk_advancedgarage:payhealth', function(price)
	local xPlayer = RKCore.GetPlayerFromId(source)
	xPlayer.removeMoney(price)
	TriggerClientEvent('RKCore:showNotification', source, _U('you_paid') .. price)
...
RegisterServerEvent('rk_advancedgarage:setVehicleState')
AddEventHandler('rk_advancedgarage:setVehicleState', function(plate, state, damage)
	local xPlayer = RKCore.GetPlayerFromId(source)
	...
	if damage ~= nil then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = @stored, damage = @damage, body_damage = @bd, engine_damage = @ed WHERE plate = @plate', {
			...
			if rowsChanged == 0 then
				print(('rk_advancedgarage: %s exploited the garage!'):format(xPlayer.identifier))
			end
		end)
	else
...
RKCore.RegisterServerCallback('rk_advancedgarage:getVehicledamage', function(source, cb, plate)
...
------------old-------------
--[[ RegisterServerEvent('rk_advancedgarage:setVehicleState')
AddEventHandler('rk_advancedgarage:setVehicleState', function(plate, state)
	local xPlayer = RKCore.GetPlayerFromId(source)
