...
RegisterServerEvent('rk-billing:sendBill')
AddEventHandler('rk-billing:sendBill', function(playerId, sharedAccountName, label, amount)
	local _source = source
	local xPlayer = RKCore.GetPlayerFromId(_source)
	local xTarget = RKCore.GetPlayerFromId(playerId)

	TriggerEvent('rk-addonaccount:getSharedAccount', sharedAccountName, function(account)
...
RKCore.RegisterServerCallback('rk-billing:getBills', function(source, cb)
...
RKCore.RegisterServerCallback('rk-billing:getTargetBills', function(source, cb, target)
...
RKCore.RegisterServerCallback('rk-billing:payBill', function(source, cb, id)
	local xPlayer = RKCore.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll(
		'SELECT * FROM billing WHERE id = @id',
		{
			...
			local xTarget = RKCore.GetPlayerFromIdentifier(sender)
			...
			if targetType == 'player' then
				...
				TriggerEvent('rk-addonaccount:getSharedAccount', target, function(account)
