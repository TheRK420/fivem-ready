...
function MFS:Awake(...)
    ...
    while not RKCore.IsPlayerLoaded() do Citizen.Wait(0); end
    self.PlayerData = RKCore.GetPlayerData()
    RKCore.TriggerServerCallback('MF_PlayerSafes:GetStartData', function(retVal,retData) self.dS = true; self.cS = retVal; self:Start(retData); end)
end
  ...
  while true do
    ...
    if closestDist and closestDist < self.DrawTextDist and not self.UsingSafe and ((not self.Instance and closestSafe.instance == 'false') or (self.Instance and self.Instance == closestSafe.instance)) then
      ...
      if IsControlJustPressed(0, 38) and closestDist <= self.InteractDist then
        RKCore.TriggerServerCallback('MF_PlayerSafes:TryAccessSafe',function(inUse)
          if inUse then
            RKCore.ShowNotification("Somebody else is using this safe.")
          else
            if not isOwner then
              ...
              self.NUIClosed = false
              self.UsingSafe = closestSafe
              -- RKCore.TriggerServerCallback('MF_PlayerSafes:GetSafeInventory', function(inventory)
              --   TriggerEvent("rk_inventoryhud:openPropertyInventory", inventory, closestSafe)
...
function MFS:EndMinigame(didWin)
  ...
  if didWin then
    -- RKCore.TriggerServerCallback('MF_PlayerSafes:GetSafeInventory', function(inventory)
    --   TriggerEvent("rk_inventoryhud:openPropertyInventory", inventory, safe)
    -- end, safe.safeid)  
...
function MFS:DoNotifyPolice(pos)
  Citizen.CreateThread(function(...)
    ...
    RKCore.ShowNotification("Somebody reported suspicious activity at "..nearStreet..". [~g~LEFTALT~s~]")
...
function MFS:EnterInstance(instance)  
  ...
  RKCore.TriggerServerCallback('MF_PlayerSafes:GetInstanceOwner', function(identifier)
...
AddEventHandler('rk_inventoryhud:closeInventory', function(...) MFS.NUIClosed = true; end)
