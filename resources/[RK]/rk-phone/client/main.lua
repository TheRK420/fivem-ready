...
local nearHousesID = 0
local homeOwner = ""
local loadedHouse = false
...
RegisterNUICallback('btnTaskGroups', function()
    local jobgrade = RKCore.PlayerData.job.grade
    local rankConverted = RKCore.PlayerData.job.grade_label

    local groupObject = {}
    table.insert(groupObject, {
        namesent = RKCore.PlayerData.job.label,
...
RegisterNUICallback('vehiclePay', function(data)
  TriggerServerEvent('RS7x:phonePayment', data.vehiclePlate)
end)

RegisterNUICallback('vehiclePayImpound', function(data)
  TriggerServerEvent('garages:PayedImpoundCar', data.vehiclePlate)
  TriggerEvent('notification', 'You payed $700 for this transaction with tax.')
end)
    ...
    for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
            if Trim(GetVehicleNumberPlateText(vehicle)) == plate then
                ...
                if phoneNotifications then
                  TriggerEvent("DoLongHudText","Your vehicle has been marked on your GPS.", 1)
...
RegisterNUICallback('btnAttempCreateHouse', function(data, cb)
  TriggerEvent("rk-housing:client:createHouses", data.price, data.tier)
  loadedHouse = false
...
AddEventHandler("phone:Garage", function(vehs)
  ...
  for ind, value in pairs(vehs) do
    ...
    local lp
    if value.repaytime >= 0 then
      lp = (value.repaytime/60)/24
    else
      lp = "Go Contact Cardealer"
    end
...
function hasPhone()
  ...
  if
    (
    (exports["rk-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
    exports["rk-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
    exports["rk-inventory"]:hasEnoughOfItem("stolens8",1,false) or
    exports["rk-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
    exports["rk-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
    exports["rk-inventory"]:hasEnoughOfItem("boomerphone",1,false))
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsDisabledControlJustReleased(1, 199) then
      TriggerEvent("rk:heHasPhone")
    end
  end
end)

RegisterNetEvent('rk:heHasPhone')
AddEventHandler('rk:heHasPhone', function()
    GotPhone()
end)


function GotPhone()
  local dead = exports["rk-ambulancejob"]:GetDeath()
      if not dead then
        openGuiNow()
      else
...
function openGuiNow()
  ...
  if hasPhone() then
    ...
    if RKCore.PlayerData.job.name == "realestateagent" then
     isREAgent = true
    end
...
RegisterNUICallback('newPostSubmit', function(data, cb)
  if RKCore.PlayerData.job.name ~= "taxi" and RKCore.PlayerData.job.name ~= "towtruck" and RKCore.PlayerData.job.name ~= "trucker" then
    TriggerServerEvent('phone:updatePhoneJob', data.advert)
  else
    TriggerServerEvent('phone:updatePhoneJob', RKCore.PlayerData.job.name .. " | " .. data.advert)
...
AddEventHandler('refreshContacts', function()
  TriggerServerEvent('getContacts')
 -- print('im here')
  --TriggerEvent('phone:loadContacts')
  Citizen.Wait(250)
  --TriggerEvent('phone:loadContacts')
...
RegisterNUICallback('newContactSubmit', function(data, cb)
  TriggerEvent('chatMessagess', 'Service:', 5, 'Contact Saved!')
...
AddEventHandler('animation:phonecallstart', function()
  ...
  TriggerEvent("DoLongHudText","[E] Toggles Call.", 1)
...
function receivingCall(callFrom)
  ...
  while (callTimer > 0 and callStatus == isReceivingCall) do
    if hasPhone() then
      TriggerEvent("DoLongHudText","Call from: " .. callFrom .. " /answer | /hangup",10)
...
AddEventHandler('Client:UpdateTweets', function(data, handle)
    myHandlerTwatter = handle
...
AddEventHandler('openGPS', function(mansions,houses,rented)
  ...
  for i = 1, #customGPSlocations do
    ...
    SetNewWaypoint(customGPSlocations[i]["x"], customGPSlocations[i]["y"])
...
RegisterNUICallback('newTwatSubmit', function(data, cb)
  local myHandlerTwat = myHandlerTwatter
    closeGui()
...
AddEventHandler('getAccountInfo', function(cash, bank, licences)
    local responseObject = {
        cash = cash,
        bank = bank,
        job = RKCore.PlayerData.job.name .. ', ' .. RKCore.PlayerData.job.grade_label,
...
AddEventHandler('phone:newContact', function(name, number)
  ...
  TriggerEvent('notification', 'Contact Saved!', 1)
  TriggerServerEvent('getContacts')
end)
...
RegisterNetEvent("rk-phone:userTwitter")

AddEventHandler("rk-phone:userTwitter", function(user)
  userTwitter = user
 end)
