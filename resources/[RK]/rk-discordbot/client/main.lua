...
AddEventHandler('rk:playerLoaded', function(xPlayer)
  PlayerData = xPlayer
	TriggerServerEvent("rk:playerconnected")
end)
	...
	while true do
		...
		if(IsPedInAnyVehicle(GetPlayerPed(-1)))then
			...
			if(IsVehicleStolen(vehicle) and isStolen )then
				Wait(1000)
				TriggerServerEvent("rk:jackingcar",GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)))
				isStolen = false
			end
		else
			isStolen = true
	...
	while true do
		...
		if(IsPedInAnyPoliceVehicle(GetPlayerPed(-1))) then
				if(not isIncarPolice and PlayerData.job.name ~= "police") then
					TriggerServerEvent("rk:enterpolicecar",GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), 0))))
					isIncarPolice = true
				end
				...
				if not isIncar then
					...
					for i=1, #blacklistedModels, 1 do
						...
						if(blacklistedModels[i] == GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), 0))))then
							TriggerServerEvent("rk:enterblacklistedcar",GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), 0))))
							isIncar = true
						end
					end
				end
        ...
        if NetworkIsPlayerActive(player) then
            ...
            if IsPedFatallyInjured(ped) and not isDead then
                ...
                if killer == ped or killer == -1 then
                    TriggerEvent('rk:killerlog',0, killertype, { table.unpack(GetEntityCoords(ped)) })
                    TriggerServerEvent('rk:killerlog',0, killertype, { table.unpack(GetEntityCoords(ped)) })
                    hasBeenDead = true
                else
                    TriggerEvent('rk:killerlog', 1,killerid, {killertype=killertype, weaponhash = killerweapon, killerinveh=killerinvehicle, killervehseat=killervehicleseat, killervehname=killervehiclename, killerpos=table.unpack(GetEntityCoords(ped))})
                    TriggerServerEvent('rk:killerlog',1, killerid, {killertype=killertype, weaponhash = killerweapon, killerinveh=killerinvehicle, killervehseat=killervehicleseat, killervehname=killervehiclename, killerpos=table.unpack(GetEntityCoords(ped))})
                    hasBeenDead = true
                end
