...
-- Load Spot Status:
RegisterNetEvent('rk_MinerJob:loadSpot')
AddEventHandler('rk_MinerJob:loadSpot', function(list, list2, list3)
    MiningPos = list
    WashingPos = list2
    SmeltingPos = list3
end)
...
RegisterNetEvent('rk_MinerJob:updateStatus')
AddEventHandler('rk_MinerJob:updateStatus', function(id,status)
			...
			if not v.mining and not currentlyMining then
				...
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.spot[1], v.spot[2], v.spot[3], true) <= 1.0 then
					if IsControlJustPressed(0,Config.KeyToStartMining) and not keyPressed then
						keyPressed = true
						--RKCore.TriggerServerCallback("rk_MinerJob:getPickaxe", function(pickaxe)
						if exports['rk-inventory']:hasEnoughOfItem('pickaxe', 1, true) then
							local stone = exports['rk-inventory']:getQuantity('stone')
							if stone <= 95 then		
								RKCore.TriggerServerCallback("rk_MinerJob:didmining", function(didminingok) 
									if didminingok then
										TriggerServerEvent("rk_MinerJob:spotStatus", k, true)
								...
								TriggerServerEvent("rk_MinerJob:getStoneLimit")
...
function MiningEvent(k,v)
	...
	exports['rk-taskbar']:taskBar(1000, "Mining")
	Citizen.Wait(200)
	...
	--TriggerServerEvent("rk_MinerJob:reward",Config.Stone,Config.StoneReward)
	TriggerServerEvent("rk_MinerJob:spotStatus", k, false)
			...
			if not currentlyWashing then
				...
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.spot[1], v.spot[2], v.spot[3], true) <= 1.2 then
					if IsControlJustPressed(0,Config.KeyToStartWashing) and not keyPressed then
						keyPressed = true
						--RKCore.TriggerServerCallback("rk_MinerJob:getWashPan", function(washPan)
							--if washPan then
							local WStone = exports['rk-inventory']:getQuantity('washedstone')
								--RKCore.TriggerServerCallback("rk_MinerJob:getWashedStoneLimit", function(washedStoneLimitOK)
									if WStone <= 95  then
...
function WashingEvent(k,v)
	...
	--RKCore.TriggerServerCallback("rk_MinerJob:removeStone", function(stoneCount)
	local WStoneCount = exports['rk-inventory']:hasEnoughOfItem('stone', 10, true)
		if WStoneCount then
			-- exports['pogressBar']:drawBar(10000, 'Plaunami akmenys')
			exports['rk-taskbar']:taskBar(500, "Washing Stones")
			Citizen.Wait(200)
			...
			--TriggerServerEvent("rk_MinerJob:reward",Config.WStone,Config.WStoneReward)
		else
				...
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, v.spot[1], v.spot[2], v.spot[3], true) <= 1.0 then
					if IsControlJustPressed(0,Config.KeyToStartSmelting) and not keyPressed then
						...
						if closestPlayer == -1 or closestDistance >= 0.7 then
							local WStoneCount1 = exports['rk-inventory']:hasEnoughOfItem('washedstone', 10, true)
...
function SmeltingEvent()
	...
	exports['rk-taskbar']:taskBar(10000, "Smelting Stones")
	--Citizen.Wait(10000)
	...
	--TriggerServerEvent("rk_MinerJob:rewardSmelting")
...
-- Refresh Spots:
Citizen.CreateThread(function()
    TriggerServerEvent("rk_MinerJob:refreshSpots")
end)
