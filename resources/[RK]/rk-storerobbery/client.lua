...
RegisterNetEvent('rk-storerobbery:onPedDeath')
AddEventHandler('rk-storerobbery:onPedDeath', function(store)
    SetEntityHealth(peds[store], 0)
end)

RegisterNetEvent('rk-storerobbery:msgPolice')
AddEventHandler('rk-storerobbery:msgPolice', function(store, robber)
    --TriggerEvent('rk-alerts:storerob')
    TriggerEvent('rk-dispatch:storerobbery')
end)

RegisterNetEvent('rk-storerobbery:removePickup')
AddEventHandler('rk-storerobbery:removePickup', function(bank)
    for i = 1, #objects do 
...
RegisterNetEvent('rk-storerobbery:robberyOver')
AddEventHandler('rk-storerobbery:robberyOver', function()
    robbing = false
end)

RegisterNetEvent('rk-storerobbery:talk')
AddEventHandler('rk-storerobbery:talk', function(store, text, time)
    robbing = false
...
RegisterNetEvent('rk-storerobbery:rob')
AddEventHandler('rk-storerobbery:rob', function(i)
                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                ...
                                TriggerServerEvent('rk-storerobbery:pickUp', i)
                                break
                            end
                        else
                            break
...
RegisterNetEvent('rk-storerobbery:resetStore')
AddEventHandler('rk-storerobbery:resetStore', function(i)
    while not RKCoreLoaded do Wait(0) end
...
Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
        while true do
            for i = 1, #peds do
                if IsPedDeadOrDying(peds[i]) then
                    TriggerServerEvent('rk-storerobbery:pedDead', i)
                end
            end
            Wait(5000)
        end
            ...
            if IsPlayerFreeAiming(PlayerId()) then
                for i = 1, #peds do
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 5.0 then
                        if not robbing then
                            local canRob = nil
                            RKCore.TriggerServerCallback('rk-storerobbery:canRob', function(cb)
                                canRob = cb
                            end, i)
                            while canRob == nil do
                                Wait(0)
                            ...
                            if canRob == true then
                                ...
                                TriggerServerEvent('rk-storerobbery:notif', i)
                                ...
                                if GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 7.5 then
                                    if not IsPedDeadOrDying(peds[i]) then
                                        TriggerServerEvent('rk-storerobbery:rob', i)
                                        while robbing do Wait(0) if IsPedDeadOrDying(peds[i]) then robbing = false end end
                                    end
                                else
                                    ClearPedTasks(peds[i])
                            ...
                            elseif canRob == 'no_cops' then
                                ...
                                TriggerEvent('rk-storerobbery:talk', i, '~g~*' .. Translation[Config.Locale]['shopkeeper'] .. '* ~w~' .. Translation[Config.Locale]['robbed'], 5)
                                Wait(2500)
                            end
                        end
                    end
