...
----markers
AddEventHandler('rk-duty:hasEnteredMarker', function (zone)
...
AddEventHandler('rk-duty:hasExitedMarker', function (zone)
  CurrentAction = nil
end)
  ...
  while true do
    ...
    for k,v in pairs(Config.Zones) do
      if(GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
        ...
        RKCore.Game.Utils.DrawText3D(vector3(v.Pos.x, v.Pos.y, v.Pos.z), closestString, 0.4)
      end
    end
  end
end)
...
Citizen.CreateThread(function ()
  while true do
    ...
    if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
      ...
      TriggerEvent('rk-duty:hasEnteredMarker', currentZone)
    end

    if not isInMarker and HasAlreadyEnteredMarker then
      HasAlreadyEnteredMarker = false
      TriggerEvent('rk-duty:hasExitedMarker', LastZone)
    end
  end
end)