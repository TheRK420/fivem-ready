...
function TakeMilk()
	if wait then
		...
		exports['rk-taskbar']:taskBar(time,Config.TakingLabel)
	...
	while true do
		...
		if Config.Text3d then
			if (GetDistanceBetweenCoords(coords, -635.33, 233.84, 81.88, true) < 2.0)  then
				...
				if IsControlJustReleased(0, 46) then
					if exports['rk-inventory']:hasEnoughOfItem("milkcan", 1, false) then
						SellMilk()
					else
...
function SellMilk()
	exports['rk-taskbar']:taskBar(3000,'Selling ' .. Config.PrizeLabel .. '')
	TriggerServerEvent('rk_milkerjob:sellitem')
