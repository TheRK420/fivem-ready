...
AddEventHandler('crossbite_milk:sell', function(itemName, amount)
	...
	price = RKCore.Math.Round(price * amount)
...
RKCore.RegisterServerCallback('crossbite_milk:canPickUp', function(source, cb, item)
	local xPlayer = RKCore.GetPlayerFromId(source)
...
RKCore.RegisterServerCallback('crossbite_milk:haveItem', function(source, cb, item)
	local xPlayer = RKCore.GetPlayerFromId(source)
	local xItem = xPlayer.getInventoryItem(item)
...
AddEventHandler('crossbite_milk:process', function()
	if not playersProcessing[source] then
		...
		playersProcessing[_source] = RKCore.SetTimeout(Config.WaitProcess*1000, function()
...
function CancelProcessing(playerID)
	if playersProcessing[playerID] then
		RKCore.ClearTimeout(playersProcessing[playerID])
...
AddEventHandler('rk:playerDropped', function(playerID, reason)
	CancelProcessing(playerID)
end)

RegisterServerEvent('rk:onPlayerDeath')
