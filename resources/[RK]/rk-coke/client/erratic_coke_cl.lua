...
Citizen.CreateThread(function()
	while RKCore == nil do TriggerEvent('rk:getSharedObject', function(obj) RKCore = obj end) Wait(0) end
    RKCore.TriggerServerCallback('coke:processcoords', function(servercoords)
...
Citizen.CreateThread(function()
	while RKCore == nil do TriggerEvent('rk:getSharedObject', function(obj) RKCore = obj end) Wait(0) end
    RKCore.TriggerServerCallback('coke:startcoords', function(servercoords)
        coord = servercoords
	end)
end)
		...
		if not inUse then
			if dist <= 1 then
				...
				if IsControlJustPressed(1, 51) then
					RKCore.TriggerServerCallback('coke:pay', function(success)
						if success then
							main()
						end
					end)
			...
			if disttocoord <= 3 then
				...
				if IsControlJustPressed(1, 51) then
					...
					if Config.progBar then
						exports['rk-taskbar']:taskBar(1000, _U'picking_jerry')
					end
		...
		while dodo do
			...
			if disttocoord <= 5 then
				...
				if IsControlJustPressed(1, 51) then
					...
					if Config.progBar then
						exports['rk-taskbar']:taskBar(20000, _U'refueling')
					end
					Citizen.Wait(20000)
					if Config.useMythic then
		...
		while delivering do
			...
			if disttocoord <= 20 then
				...
				if IsControlJustPressed(1, 51) then
					delivering = false
					if Config.progBar then
						exports['rk-taskbar']:taskBar(Config.pickupTime, _U'picking_deliv')
					end
			...
			if disttocoord <= 5 then
				...
				if IsControlJustPressed(1, 51) then
					if DoesEntityExist(airplane) and differnce <= 5 then
						...
						FreezeEntityPosition(airplane, true)
						if Config.progBar then
							exports['rk-taskbar']:taskBar(2000, _U'leaving_key')
						end
						Citizen.Wait(2000)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if dist <= 3 and not isProcessing then
			...
			if IsControlJustPressed(1, 51) then		
				isProcessing = true
				--RKCore.TriggerServerCallback('coke:process', function(success)
					if exports['rk-inventory']:hasEnoughOfItem('coke50g', 1) then				
						processing()
					else
						isProcessing = false
					end
...
function processing()
	...
	FreezeEntityPosition(player, true)
	if Config.progBar then
		exports['rk-taskbar']:taskBar(6000, _U'breaking_coke')
	end
