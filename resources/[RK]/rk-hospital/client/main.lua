...
function LeaveBed()
    ...
    TriggerServerEvent('rk-hospital:server:LeaveBed', bedOccupying)
...
RegisterNetEvent('rk-hospital:client:RPCheckPos')
AddEventHandler('rk-hospital:client:RPCheckPos', function()
    TriggerServerEvent('rk-hospital:server:RPRequestBed', GetEntityCoords(PlayerPedId()))
end)

RegisterNetEvent('rk-hospital:client:RPSendToBed')
AddEventHandler('rk-hospital:client:RPSendToBed', function(id, data)
    bedOccupying = id
...
RegisterNetEvent('rk-hospital:client:SendToBed')
AddEventHandler('rk-hospital:client:SendToBed', function(id, data)
    bedOccupying = id
    bedOccupyingData = data
    ...
    Citizen.CreateThread(function ()
        ...
        TriggerServerEvent('rk-hospital:server:EnteredBed')
    end)
end)

RegisterNetEvent('rk-hospital:client:FinishServices')
AddEventHandler('rk-hospital:client:FinishServices', function()
                ...
                if hspDist < 3 then
                    ...
                    if IsControlJustReleased(0, 54) then
                        if (GetEntityHealth(PlayerPedId()) < 201) or (IsInjuredOrBleeding()) then
                            ...
                            exports["rk-taskbar"]:taskBar(1500, "Checking Credentials")
                                TriggerEvent('rk-ambulancejob:revive')
                                ClearPedTasks(PlayerPedId())
                                Citizen.Wait(1000)
                                TriggerServerEvent('rk-hospital:server:RequestBed')
                            else
