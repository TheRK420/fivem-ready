...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('rk-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('rk-hospital:client:SyncBleed')
AddEventHandler('rk-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('rk-hospital:client:FieldTreatLimbs')
AddEventHandler('rk-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('rk-hospital:client:FieldTreatBleed')
AddEventHandler('rk-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('rk-hospital:client:ReduceBleed')
AddEventHandler('rk-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('rk-hospital:client:ResetLimbs')
AddEventHandler('rk-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('rk-hospital:client:RemoveBleed')
AddEventHandler('rk-hospital:client:RemoveBleed', function()
    isBleeding = 0
...
RegisterNetEvent('rk-hospital:client:UsePainKiller')
AddEventHandler('rk-hospital:client:UsePainKiller', function(tier)
    if tier < 4 then
...
RegisterNetEvent('rk-hospital:client:UseAdrenaline')
AddEventHandler('rk-hospital:client:UseAdrenaline', function(tier)
    if tier < 4 then
        onDrugs = 180 * tier
    end
