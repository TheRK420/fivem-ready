...
RegisterServerEvent('rk-hospital:server:RequestBed')
AddEventHandler('rk-hospital:server:RequestBed', function()
    for k, v in pairs(beds) do
        if not v.taken then
            v.taken = true
            bedsTaken[source] = k
            TriggerClientEvent('rk-hospital:client:SendToBed', source, k, v)
            return
        end
    end
...
RegisterServerEvent('rk-hospital:server:RPRequestBed')
AddEventHandler('rk-hospital:server:RPRequestBed', function(plyCoords)
    ...
    for k, v in pairs(beds) do
        ...
        if distance < 3.0 then
            if not v.taken then
                v.taken = true
                foundbed = true
                TriggerClientEvent('rk-hospital:client:RPSendToBed', source, k, v)
                return
            else
...
RegisterServerEvent('rk-hospital:server:EnteredBed')
AddEventHandler('rk-hospital:server:EnteredBed', function()
    local src = source
    ...
    TriggerClientEvent('rk-hospital:client:FinishServices', src)
end)

RegisterServerEvent('rk-hospital:server:LeaveBed')
AddEventHandler('rk-hospital:server:LeaveBed', function(id)
    beds[id].taken = false
end)