...
AddEventHandler('rk:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	TriggerServerEvent('cr7842-882ga:498-568-7243-5583-67')
end)

RegisterNetEvent('cr7842-882ga:496-15-9149-81-785')
AddEventHandler('cr7842-882ga:496-15-9149-81-785', function(trucknumber)
...
RegisterNetEvent('cr7842-882ga:46-4565-47-656-68')
AddEventHandler('cr7842-882ga:46-4565-47-656-68', function(binpos, platenumber,  bags)
	if isInService then
...
RegisterNetEvent('cr7842-882ga:7510-44-3987-440')
AddEventHandler('cr7842-882ga:7510-44-3987-440', function(platenumber, bags, crewmember)
	if isInService then
...
RegisterNetEvent('cr7842-882ga:974-4888-365-44')
AddEventHandler('cr7842-882ga:974-4888-365-44', function(platenumber, amount)
	if isInService then
		if platenumb == platenumber then
			TriggerServerEvent('cr7842-882ga:445-266-8887-112', amount)
			platenumb = nil
		end
	end
end)

RegisterNetEvent('cr7842-882ga:4169-656-88-01')
AddEventHandler('cr7842-882ga:4169-656-88-01', function(platenumber)
	if isInService then
		if platenumb == platenumber then
...
RegisterNetEvent('cr7842-882ga:54-9416-44-8887')
AddEventHandler('cr7842-882ga:54-9416-44-8887', function(platenumber)
	if isInService then
		if platenumb == platenumber then
			if not iscurrentboss then
			TriggerServerEvent('cr7842-882ga:7669-652-8884-79', platenumb, totalbagpay)
			totalbagpay = 0
			end
		end
	end
...
RegisterNetEvent('cr7842-882ga:294-22-483-0027')
AddEventHandler('cr7842-882ga:294-22-483-0027', function(platenumber)
	if platenumb == platenumber then
	...
	RKCore.UI.Menu.Open(
		...
		function(data, menu)
			if data.current.value == 'citizen_wear' then
				isInService = false
				TriggerEvent('rk:restoreLoadout')
			end
...
function MenuVehicleSpawner()
	...
	RKCore.UI.Menu.Open(
		...
		function(data, menu)
			RKCore.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
				...
				TriggerServerEvent('cr7842-882ga:54-9415-146-9581')   
                MissionLivraisonSelect()
				if data.current.value == 'phantom3' then
					RKCore.Game.SpawnVehicle("trailers2", Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(trailer)
						AttachVehicleToTrailer(vehicle, trailer, 1.1)
					end)
				end
				local plate = GetVehicleNumberPlateText(vehicle)
				TriggerServerEvent('garage:addKeys', plate)
				exports["rk-oGasStations"]:SetFuel(vehicle, 100)		
...
AddEventHandler('cr7842-882ga:123-56-4984-622', function(zone)
	...
	if zone == 'VehicleSpawner' then
		if isInService and IsJobgarbage() then
			if MissionRetourCamion or MissionLivraison then
				--CurrentAction = 'hint'
				TriggerEvent('DoLongHudText', 'We have already provided a truck!', 2)
		...
		if isInService and MissionLivraison and MissionNum == namezonenum and MissionRegion == namezoneregion and IsJobgarbage() then
			if IsPedSittingInAnyVehicle(playerPed) and IsATruck() then
				...
				if plaquevehicule == plaquevehiculeactuel then
					...
					TriggerEvent('DoLongHudText', 'Press [E] to Pickup.', 1)
				else
					TriggerEvent('DoLongHudText', 'It is not the truck that you have provided!', 2)
				end
			else
				TriggerEvent('DoLongHudText', 'You must be in the truck that you were provided with!', 2)
			end
		end
	end
...
AddEventHandler('cr7842-882ga:789-456-5544-8711', function(zone)
	RKCore.UI.Menu.CloseAll()    
    CurrentAction = nil
	CurrentActionMsg = ''
end) 
...
function nouvelledestination()
	...
	TriggerServerEvent('cr7842-882ga:284-4588-658-4446', platenumb,  temppayamount)
	TriggerServerEvent('cr7842-882ga:66-111-894-1346', platenumb)
...
function SelectBinandCrew()
	...
	local NewBin, NewBinDistance = RKCore.Game.GetClosestObject(Config.DumpstersAvaialbe)
	trashcollectionpos = GetEntityCoords(NewBin)
	platenumb = GetVehicleNumberPlateText(GetVehiclePedIsIn(GetPlayerPed(-1), true))
	TriggerServerEvent("cr7842-882ga:8475-249-37-5874", trashcollectionpos, platenumb, bagsoftrash)
end
				...
				if CollectionAction == 'collection' then
					...
					if dist <= 3.5 then
						if currentbag > 0 then
							...
							TriggerServerEvent('cr7842-882ga:7894-670-56-0943', platenumb)
							trashcollection = false
							Citizen.Wait(4000)
							...
							if iscurrentboss then
								...
								TriggerServerEvent('cr7842-882ga:975-348-75-34', platenumb)
								Citizen.Wait(4000)
								ClearPedTasks(PlayerPedId())
								setring = false
								...
								trashcollection = false
								truckdeposit = false
								TriggerEvent('DoLongHudText', 'Collection finished return to truck!', 1)
...
Citizen.CreateThread(function()
	while true do
		...
		if truckdeposit then
			...
			dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, trunk.x , trunk.y, trunk.z)
			if dist <= 2.0 then
			RKCore.Game.Utils.DrawText3D(vector3(trunk.x , trunk.y ,trunk.z + 0.50), "[~g~E~s~] Throw bag in truck.", 1.0)
			end
		end
		...
		if trashcollection then
			...
			if dist <= 5.0 then
				if currentbag <= 0 then
					if iscurrentboss then
					RKCore.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Clean up area", 1.0)		
					else
					RKCore.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "Collection done.. Wait on truck..", 1.0)		
					end
				else
					RKCore.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Collect bag from Trashbin ["..currentbag.."/"..bagsoftrash.."]", 1.0)
				end
			end
		end
		...
		if not paused then 
			...
			if IsJobgarbage() then
				...
				if isInMarker and not hasAlreadyEnteredMarker then
					...
					TriggerEvent('cr7842-882ga:123-56-4984-622', currentZone)
				end

				if not isInMarker and hasAlreadyEnteredMarker then
					hasAlreadyEnteredMarker = false
					TriggerEvent('cr7842-882ga:789-456-5544-8711', lastZone)
				end
			end
		end
...
function MissionLivraisonLetsGo()
	...
	if MissionRegion == 1 then -- Los santos
		TriggerEvent('DoLongHudText', 'Go to the next pickup Location!', 1)
	elseif MissionRegion == 2 then -- Blaine County
		TriggerEvent('DoLongHudText', 'Go to the next pickup Location!', 1)
