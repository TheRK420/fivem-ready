...
AddEventHandler('dispatch:clNotify', function(pNotificationData)
  local currentJob = RKCore.GetPlayerData().job.name
            ...
            for key, value in pairs(pNotificationData.recipientList) do
                if key == currentJob and value and not disableNotifications then
                    ...
                    elseif currentJob == "news" then
                        if exports["rk-inventory"]:getQuantity("scanner") > 0 then
                            local newsObject = {}
...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
        if (IsControlJustReleased(0, 178) and (RKCore.GetPlayerData().job.name == 'ambulance' or RKCore.GetPlayerData().job.name == 'police') and not exports["rk-ambulancejob"]:GetDeath()) then
...
AddEventHandler("civilian:alertPolice",function(basedistance,alertType,objPassed,isGunshot,isSpeeder)
    ...
    if alertType == "robberyhouse" and not RKCore.GetPlayerData().job.name == 'police' then
      AlertCheckRobbery2()
    end
end)
...
--       end
--       TriggerServerEvent('rk-alerts:gunshots', {
...
AddEventHandler('police:panic', function()
      ...
      recipientList = {
		...
		TriggerEvent('rk-alerts:policepanic')
end)
...
AddEventHandler("police:1047", function()
    ...
    recipientList = {
      police = "police", ambulance = "ambulance"
		...
		TriggerEvent('rk-alerts:downguy')
end)
...
function AlertGunShot()
    Citizen.CreateThread(function() 
      ...
      TriggerEvent('rk-alerts:gunshot')
...
function CarCrash()
  ...
  TriggerEvent('rk-alerts:vehiclecrash')
...
function AlertCheckLockpick(object)
  ...
  if not DoesEntityExist(targetVehicle) then
    ...
    TriggerEvent('rk-alerts:stolenveh')
  end
end
...
function AlertpersonRobbed(vehicle)
  ...
  TriggerEvent('rk-alerts:robstore')
  Wait(math.random(5000,15000))
...
function AlertCheckRobbery2()
  ...
  TriggerEvent('rk-alerts:robhouse')
  Wait(math.random(5000,15000))
...
RegisterNetEvent('rk-dispatch:drugjob')
AddEventHandler("rk-dispatch:drugjob",function()
  AlertDrugJob()
end)

RegisterNetEvent('rk-dispatch:bankrobbery')
AddEventHandler("rk-dispatch:bankrobbery",function()
  AlertFleecaRobbery()
end)

RegisterNetEvent('rk-dispatch:jailbreak')
AddEventHandler("rk-dispatch:jailbreak",function()
  AlertJailBreak()
end)

RegisterNetEvent('rk-dispatch:jewelrobbery')
AddEventHandler("rk-dispatch:jewelrobbery",function()
  AlertJewelRob()
  return
end)

RegisterNetEvent('rk-dispatch:houserobbery')
AddEventHandler("rk-dispatch:houserobbery",function()
  AlertCheckRobbery2()
end)

RegisterNetEvent('rk-dispatch:storerobbery')
AddEventHandler("rk-dispatch:storerobbery",function()
  AlertpersonRobbed(vehicle)
end)

RegisterNetEvent('rk-dispatch:carjacking')
AddEventHandler("rk-dispatch:carjacking",function()
  AlertCheckLockpick(object)
end)

RegisterNetEvent('rk-ambulancejob:downplayer')
AddEventHandler("rk-ambulancejob:downplayer",function()
    AlertDeath()
end)
