...
RegisterNetEvent('rk-ambulancejob:drag')
AddEventHandler('rk-ambulancejob:drag', function(copID)
...
RegisterCommand('escort', function()
	...
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		TriggerServerEvent('rk-ambulancejob:drag', GetPlayerServerId(closestPlayer))
	else
...
function OpenAmbulanceActionsMenu()
	...
	if Config.EnablePlayerManagement and RKCore.PlayerData.job.grade_name == 'boss' then
		table.insert(elements, {label = _U('boss_actions'), value = 'boss_actions'})
	end

	RKCore.UI.Menu.CloseAll()

	RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'ambulance_actions', {
		title    = _U('ambulance'),
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('rk-society:openBossMenu', 'ambulance', function(data, menu)
				menu.close()
			end, {wash = false})
		end
...
function OpenMobileAmbulanceActionsMenu()
	...
	RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
		title    = _U('ambulance'),
		align    = 'bottom-right',
		elements = {
		...
		if data.current.value == 'citizen_interaction' then
			RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer == -1 or closestDistance > 2.0 then
					...
					if data.current.value == 'revive' then
						...
						RKCore.TriggerServerCallback('rk-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_a", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_b", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_c", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_d", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_e", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_f", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_g", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_h", 3)then
									...
									TriggerServerEvent('rk-ambulancejob:removeItem', 'medikit')
									TriggerServerEvent('rk-ambulancejob:revive', GetPlayerServerId(closestPlayer))
						...
						RKCore.TriggerServerCallback('rk-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('rk-ambulancejob:removeItem', 'bandage')
									TriggerServerEvent('rk-ambulancejob:heal', GetPlayerServerId(closestPlayer), 'small')
						...
						RKCore.TriggerServerCallback('rk-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('rk-ambulancejob:removeItem', 'medikit')
									TriggerServerEvent('rk-ambulancejob:heal', GetPlayerServerId(closestPlayer), 'big')
					...
					elseif data.current.value == 'put_in_vehicle' then
						TriggerServerEvent('rk-ambulancejob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif data.current.value == 'pull_out_vehicle' then
						TriggerServerEvent('rk-ambulancejob:pullOutVehicle', GetPlayerServerId(closestPlayer))
					elseif data.current.value == 'drag_player' then
						TriggerServerEvent('rk-ambulancejob:drag', GetPlayerServerId(closestPlayer))
					elseif data.current.value == 'un_drag' then
						TriggerServerEvent('rk-ambulancejob:undrag', GetPlayerServerId(closestPlayer))
					end
				end
			end, function(data, menu)
				menu.close()
...
function FastTravel(coords, heading)
	...
	RKCore.Game.Teleport(playerPed, coords, function()
		DoScreenFadeIn(800)
...
Citizen.CreateThread(function()
	while true do
		...
		if RKCore.PlayerData.job and RKCore.PlayerData.job.name == 'ambulance' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastHospital ~= currentHospital or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('rk-ambulancejob:hasExitedMarker', LastHospital, LastPart, LastPartNum)
					hasExited = true
				end

				HasAlreadyEnteredMarker, LastHospital, LastPart, LastPartNum = true, currentHospital, currentPart, currentPartNum

				TriggerEvent('rk-ambulancejob:hasEnteredMarker', currentHospital, currentPart, currentPartNum)
...
AddEventHandler('rk-ambulancejob:hasEnteredMarker', function(hospital, part, partNum)
	if RKCore.PlayerData.job and RKCore.PlayerData.job.name == 'ambulance' then
...
AddEventHandler('rk-ambulancejob:hasExitedMarker', function(hospital, part, partNum)
...
Citizen.CreateThread(function()
	while true do
		...
		elseif RKCore.PlayerData.job ~= nil and RKCore.PlayerData.job.name == 'ambulance' and not IsDead then
...
RegisterNetEvent('rk-ambulancejob:putInVehicle')
AddEventHandler('rk-ambulancejob:putInVehicle', function()
	local playerPed = PlayerPedId()
...
RegisterNetEvent('rk-ambulancejob:pullOutVehicle')
AddEventHandler('rk-ambulancejob:pullOutVehicle', function()
	local playerPed = PlayerPedId()
...
RegisterNetEvent('rk-ambulancejob:drag')
AddEventHandler('rk-ambulancejob:drag', function(cop)
	IsDragged = not IsDragged
	CopPed = tonumber(cop)
end)

RegisterNetEvent('rk-ambulancejob:un_drag')
AddEventHandler('rk-ambulancejob:un_drag', function(cop)
	IsDragged = not IsDragged
...
function OpenCloakroomMenu()
	RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		title    = _U('cloakroom'),
		align    = 'bottom-right',
		elements = {
		...
		if data.current.value == 'citizen_wear' then
			RKCore.TriggerServerCallback('rk-skin:getPlayerSkin', function(skin, jobSkin)
...
function OpenVehicleSpawnerMenu(hospital, partNum)
	...
	RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			local authorizedVehicles = Config.AuthorizedVehicles[RKCore.PlayerData.job.grade_name]

			if #authorizedVehicles > 0 then
				for k,vehicle in ipairs(authorizedVehicles) do
					table.insert(shopElements, {
						label = ('%s - <span style="color:green;">%s</span>'):format(vehicle.label, _U('shop_item', RKCore.Math.GroupDigits(vehicle.price))),
						name  = vehicle.label,
						model = vehicle.model,
						price = vehicle.price,
						type  = 'car'
		...
		elseif data.current.action == 'garage' then
			...
			RKCore.TriggerServerCallback('rk-vehicleshop:retrieveJobVehicles', function(jobVehicles)
				if #jobVehicles > 0 then
					...
					RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_garage', {
						title    = _U('garage_title'),
						align    = 'bottom-right',
						elements = garage
						...
						if data2.current.stored then
							...
							if foundSpawn then
								...
								RKCore.Game.SpawnVehicle(data2.current.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
									RKCore.Game.SetVehicleProperties(vehicle, data2.current.vehicleProps)

									TriggerServerEvent('rk-vehicleshop:setJobVehicleState', data2.current.vehicleProps.plate, false)
...
function StoreNearbyVehicle(playerCoords)
	local vehicles, vehiclePlates = RKCore.Game.GetVehiclesInArea(playerCoords, 30.0), {}
		...
		for k,v in ipairs(vehicles) do
			...
			if GetVehicleNumberOfPassengers(v) == 0 and IsVehicleSeatFree(v, -1) then
				table.insert(vehiclePlates, {
					vehicle = v,
					plate = RKCore.Math.Trim(GetVehicleNumberPlateText(v))
				})
			end
		end
	else
	...
	RKCore.TriggerServerCallback('rk-ambulancejob:storeNearbyVehicle', function(storeSuccess, foundNum)
		if storeSuccess then
			local vehicleId = vehiclePlates[foundNum]
			local attempts = 0
			RKCore.Game.DeleteVehicle(vehicleId.vehicle)
			IsBusy = true
			...
			while DoesEntityExist(vehicleId.vehicle) do
				...
				vehicles = RKCore.Game.GetVehiclesInArea(playerCoords, 30.0)
				if #vehicles > 0 then
					for k,v in ipairs(vehicles) do
						if RKCore.Math.Trim(GetVehicleNumberPlateText(v)) == vehicleId.plate then
							RKCore.Game.DeleteVehicle(v)
							break
						end
					end
				end
...
function GetAvailableVehicleSpawnPoint(hospital, part, partNum)
	...
	for i=1, #spawnPoints, 1 do
		if RKCore.Game.IsSpawnPointClear(spawnPoints[i].coords, spawnPoints[i].radius) then
...
function OpenHelicopterSpawnerMenu(hospital, partNum)
	...
	RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'helicopter_spawner', {
		...
		if data.current.action == 'buy_helicopter' then
			...
			local authorizedHelicopters = Config.AuthorizedHelicopters[RKCore.PlayerData.job.grade_name]

			if #authorizedHelicopters > 0 then
				for k,helicopter in ipairs(authorizedHelicopters) do
					table.insert(shopElements, {
						label = ('%s - <span style="color:green;">%s</span>'):format(helicopter.label, _U('shop_item', RKCore.Math.GroupDigits(helicopter.price))),
		...
		elseif data.current.action == 'garage' then
			...
			RKCore.TriggerServerCallback('rk-vehicleshop:retrieveJobVehicles', function(jobVehicles)
				if #jobVehicles > 0 then
					...
					RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'helicopter_garage', {
...
function OpenShopMenu(elements, restoreCoords, shopCoords)
	...
	RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		title    = _U('vehicleshop_title'),
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)

		RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				local vehicle  = GetVehiclePedIsIn(playerPed, false)
				local props    = RKCore.Game.GetVehicleProperties(vehicle)
				props.plate    = newPlate

				RKCore.TriggerServerCallback('rk-ambulancejob:buyJobVehicle', function (bought)
					if bought then
						RKCore.ShowNotification(_U('vehicleshop_bought', data.current.name, RKCore.Math.GroupDigits(data.current.price)))
						...
						RKCore.Game.Teleport(playerPed, restoreCoords)
					else
		...
		RKCore.Game.SpawnLocalVehicle(data.current.model, shopCoords, 0.0, function(vehicle)
	...
	RKCore.Game.SpawnLocalVehicle(elements[1].model, shopCoords, 0.0, function(vehicle)
		table.insert(spawnedVehicles, vehicle)
		TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
		FreezeEntityPosition(vehicle, true)
	end)
...
function WarpPedInClosestVehicle(ped)
	...
	local vehicle, distance = RKCore.Game.GetClosestVehicle(coords)
...
RegisterNetEvent('rk-ambulancejob:heal')
AddEventHandler('rk-ambulancejob:heal', function(healType)
    local playerPed = PlayerPedId()
    ...
    if healType == 'small' then
        ...
        TriggerEvent('rk-hospital:client:RemoveBleed')
	elseif healType == 'big' then
		local health = GetEntityHealth(playerPed)
        local newHealth = math.min(maxHealth, math.floor(health + maxHealth / 10))
        SetEntityHealth(playerPed, newHealth)
        TriggerEvent('rk-hospital:client:RemoveBleed')
        TriggerEvent('rk-hospital:client:ResetLimbs')
    end
