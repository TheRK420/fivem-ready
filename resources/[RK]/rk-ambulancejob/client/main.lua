...
AddEventHandler('playerSpawned', function()
	...
	if FirstSpawn then
		...
		RKCore.TriggerServerCallback('rk-ambulancejob:getDeathStatus', function(isDead)
...
function OnPlayerDeath()
	...
	TriggerServerEvent('rk-ambulancejob:setDeathState', true)
...
function releaseBody()
    ...
    RKCore.TriggerServerCallback("rk-jail:retrieveTime", function(inJail, newJailTime)
		if inJail then
...
function StartDeathTimer()
	...
	if Config.EarlyRespawnFine then
		RKCore.TriggerServerCallback('rk-ambulancejob:checkBalance', function(canPay)
			canPayFine = canPay
		end)
	end

	local earlySpawnTimer = RKCore.Math.Round(Config.EarlyRespawnTimer / 1000)
	local bleedoutTimer = RKCore.Math.Round(Config.BleedoutTimer / 1000)
	...
	Citizen.CreateThread(function()
		...
		while bleedoutTimer > 0 and IsDead do
			...
			elseif Config.EarlyRespawnFine and canPayFine then
				text = text .. _U('respawn_bleedout_fine', RKCore.Math.GroupDigits(Config.EarlyRespawnFineAmount))

				if IsControlPressed(0, Keys['H']) and timeHeld > 60 then
					TriggerServerEvent('rk-ambulancejob:payFine')
					RemoveItemsAfterRPDeath()
...
function RemoveItemsAfterRPDeath()
	TriggerServerEvent('rk-ambulancejob:setDeathState', false)
	...
	Citizen.CreateThread(function()
		...
		RKCore.TriggerServerCallback('rk-ambulancejob:removeItemsAfterRPDeath', function()
			local playerpos = GetEntityCoords( GetPlayerPed(-1) )
				
			RKCore.SetPlayerData('lastPosition', playerpos)
			RKCore.SetPlayerData('loadout', {})
			RespawnPed(PlayerPedId(), playerpos, Config.RespawnPoint.heading)

			TriggerServerEvent('rk:updateLastPosition', playerpos)
...
RegisterNetEvent('rk-ambulancejob:useItem')
AddEventHandler('rk-ambulancejob:useItem', function(itemName)
	RKCore.UI.Menu.CloseAll()
	...
	if itemName == 'medikit' then
		...
		exports["rk-taskbar"]:taskBar(10000, "Healing")
	
			TriggerEvent('rk-ambulancejob:heal', 'big', true)
			TriggerEvent('rk-hospital:client:RemoveBleed')
			TriggerEvent('rk-hospital:client:ResetLimbs')
	...
	elseif itemName == 'bandage' then
		...
		exports["rk-taskbar"]:taskBar(10000, "Healing")
			...
			TriggerEvent('rk-ambulancejob:heal', 'small', true)
			TriggerEvent('rk-hospital:client:RemoveBleed')
			ClearPedTasks(playerPed)
	end
end)
...
function RespawnPed(ped, coords, heading)
	...
	TriggerEvent('rk-hospital:client:RemoveBleed', ped) 
	TriggerEvent('rk-hospital:client:ResetLimbs', ped)

	RKCore.UI.Menu.CloseAll()
end

AddEventHandler('rk:onPlayerDeath', function(data)
	OnPlayerDeath()
end)
...
RegisterNetEvent('rk-ambulancejob:revive')
AddEventHandler('rk-ambulancejob:revive', function()
	attemptRevive()
end)
