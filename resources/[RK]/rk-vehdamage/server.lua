...
local _src = source
local xPlayer = RKCore.GetPlayerFromId(_src)
local matAmount = amount
        local amount = amount*10
            ...
            --print(updateDB)
            MySQL.Async.execute("UPDATE owned_vehicles SET degredation = @degredation WHERE plate = @plate", {
...
AddEventHandler('veh.updateVehicleHealth', function(tempReturn)
    ...
    MySQL.Async.execute("UPDATE owned_vehicles SET engine_damage = @engine_damage, body_damage = @body_damage, fuel = @fuel WHERE plate = @plate", {
