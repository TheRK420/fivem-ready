...
AddEventHandler("rk:playerLoaded", function(response)
	PlayerData = response
...
RegisterNetEvent("rk-qalle-sellvehicles:refreshVehicles")
AddEventHandler("rk-qalle-sellvehicles:refreshVehicles", function()
	RemoveVehicles()
		...
		while true do
			...
			if dstCheck <= 10.0 then
				...
				if dstCheck <= 4.2 then
					RKCore.Game.Utils.DrawText3D(SellPos, "[E] Open Menu", 0.4)
					if IsControlJustPressed(0, 38) then
						if IsPedInAnyVehicle(ped, false) then
							OpenSellMenu(GetVehiclePedIsUsing(ped))
						else
							RKCore.ShowNotification("You must sit in a ~g~vehicle")
						end
					end
				end
			end
			...
			for i = 1, #Config.VehiclePositions, 1 do
				if Config.VehiclePositions[i]["entityId"] ~= nil then
					...
					if dstCheck <= 2.0 then
						...
						RKCore.Game.Utils.DrawText3D(vehCoords, "[E] " .. Config.VehiclePositions[i]["price"] .. ":-", 0.4)
						...
						if IsControlJustPressed(0, 38) then
							if IsPedInVehicle(ped, Config.VehiclePositions[i]["entityId"], false) then
								...
								RKCore.ShowNotification("You must sit in the ~g~vehicle~s~!")
							end
						end
					end
				end
...
function OpenSellMenu(veh, price, buyVehicle, owner)
	...
	RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'sell_veh',
		{
			title    = "Vehicle Menu",
			align    = 'top-right',
			elements = elements
	...
	function(data, menu)
		...
		if action == "price" then
			RKCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'sell_veh_price',
				{
					title = "Vehicle Price"
				},
		...
		elseif action == "sell" then
			local vehProps = RKCore.Game.GetVehicleProperties(veh)

			RKCore.TriggerServerCallback("rk-qalle-sellvehicles:isVehicleValid", function(valid)

				if valid then
					DeleteVehicle(veh)
					RKCore.ShowNotification("You put out the vehicle for sale - " .. price .. " :-")
					menu.close()
				else
					RKCore.ShowNotification("You must own the vehicle! or its Finance Isnt Cleared ")
				end
	
			end, vehProps, price)
		elseif action == "buy" then
			RKCore.TriggerServerCallback("rk-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					RKCore.ShowNotification("You bought the vehicle for " .. price .. " :-")
					menu.close()
				else
					RKCore.ShowNotification("You don't have enough cash, it's missing " .. price - totalMoney .. " :-")
				end
			end, RKCore.Game.GetVehicleProperties(veh), price)
		elseif action == "remove" then
			RKCore.TriggerServerCallback("rk-qalle-sellvehicles:buyVehicle", function(isPurchasable, totalMoney)
				if isPurchasable then
					DeleteVehicle(veh)
					RKCore.ShowNotification("You removed the vehicle")
					menu.close()
				end
			end, RKCore.Game.GetVehicleProperties(veh), 0)
		end
...
function RemoveVehicles()
	...
	for i = 1, #VehPos, 1 do
		local veh, distance = RKCore.Game.GetClosestVehicle(VehPos[i])
...
function SpawnVehicles()
	...
	RKCore.TriggerServerCallback("rk-qalle-sellvehicles:retrieveVehicles", function(vehicles)
		for i = 1, #vehicles, 1 do
			...
			RKCore.Game.SetVehicleProperties(VehPos[i]["entityId"], vehicleProps)
