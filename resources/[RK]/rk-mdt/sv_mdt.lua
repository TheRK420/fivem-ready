    ...
    if xPlayer.job.name == 'police' or xPlayer.job.name == 'sasp' then
    	MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
    		...
    		MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_warrants` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(warrants)
    			...
    			TriggerClientEvent('rk-mdt:toggleVisibilty', usource, reports, warrants, officer)
    		end)
    	end)
    end
end)

RegisterServerEvent("rk-mdt:hotKeyOpen")
AddEventHandler("rk-mdt:hotKeyOpen", function()
	local usource = source
    local xPlayer = RKCore.GetPlayerFromId(source)
...
RegisterServerEvent("rk-mdt:getOffensesAndOfficer")
AddEventHandler("rk-mdt:getOffensesAndOfficer", function()
	local usource = source
	local charges = {}
	local jailtime = {}
	MySQL.Async.fetchAll('SELECT * FROM fine_types', {
		...
		TriggerClientEvent("rk-mdt:returnOffensesAndOfficer", usource, charges, officer)
	end)
end)

RegisterServerEvent("rk-mdt:performOffenderSearch")
AddEventHandler("rk-mdt:performOffenderSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `characters` WHERE LOWER(`firstname`) LIKE @query OR LOWER(`lastname`) LIKE @query OR CONCAT(LOWER(`firstname`), ' ', LOWER(`lastname`)) LIKE @query", {
		...
		TriggerClientEvent("rk-mdt:returnOffenderSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("rk-mdt:getOffenderDetails")
AddEventHandler("rk-mdt:getOffenderDetails", function(offender)
	local usource = source
	...
	MySQL.Async.fetchAll('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
		...
		MySQL.Async.fetchAll('SELECT * FROM `user_convictions` WHERE `char_id` = @id', {
			...
			MySQL.Async.fetchAll('SELECT * FROM `mdt_warrants` WHERE `char_id` = @id', {
				...
				TriggerClientEvent("rk-mdt:returnOffenderDetails", usource, offender)
			end)
		end)
	end)
end)

RegisterServerEvent("rk-mdt:getOffenderDetailsById")
AddEventHandler("rk-mdt:getOffenderDetailsById", function(char_id)
	local usource = source
...
RegisterServerEvent("rk-mdt:saveOffenderChanges")
AddEventHandler("rk-mdt:saveOffenderChanges", function(id, changes, identifier)
...
RegisterServerEvent("rk-mdt:saveReportChanges")
AddEventHandler("rk-mdt:saveReportChanges", function(data)
    local usource = source
	...
	sendToDiscord('rk- Report Logs', author .. ' Updated a report with id: '..data.id..' Title: '..data.title..' Incident: '..data.incident..'')
end)

RegisterServerEvent("rk-mdt:deleteReport")
AddEventHandler("rk-mdt:deleteReport", function(id)
    local usource = source
	local author = GetCharacterName(source)
	MySQL.Async.execute('DELETE FROM `mdt_reports` WHERE `id` = @id', {
		['@id']  = id
	})
	sendToDiscord('rk- Report Logs', author .. ' Deleted a report with id: '..id..'')
end)

RegisterServerEvent("rk-mdt:submitNewReport")
AddEventHandler("rk-mdt:submitNewReport", function(data)
	local usource = source
	local author = GetCharacterName(source)
	...
	MySQL.Async.insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`, `jailtime`) VALUES (@id, @title, @incident, @charges, @author, @name, @date, @sentence)', {
		...
		TriggerEvent("rk-mdt:getReportDetailsById", id, usource)
		sendToDiscord('rk- Report Logs', author .. ' Created a report with id: '..data.char_id..' Title: '..data.title..' Incident: '..data.incident..' Author of report: '..author..' Date of report: '..data.date..' Name: '..data.name..'')
	end)
...
RegisterServerEvent("rk-mdt:sentencePlayer")
AddEventHandler("rk-mdt:sentencePlayer", function(jailtime, charges, char_id, fine, players)
	local usource = source
	local jailmsg = ""
	local xPlayer = RKCore.GetPlayerFromId(source)
			...
			MySQL.Async.fetchAll('SELECT * FROM `characters` WHERE `identifier` = @identifier', {
				...
				if result[1].id == char_id then
					if jailtime and jailtime > 0 then
						...
						TriggerEvent("rk-jail:jailPlayer", src, jailtime, jailmsg)
					end
					if fine > 0 then
						TriggerClientEvent("rk-mdt:billPlayer", usource, src, 'society_police', 'Fine: '..jailmsg, fine)
					end
					return
				end
			end)
...
RegisterServerEvent("rk-mdt:performReportSearch")
AddEventHandler("rk-mdt:performReportSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` LIKE @query OR LOWER(`title`) LIKE @query OR LOWER(`name`) LIKE @query OR LOWER(`author`) LIKE @query or LOWER(`charges`) LIKE @query", {
		...
		TriggerClientEvent("rk-mdt:returnReportSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("rk-mdt:performVehicleSearch")
AddEventHandler("rk-mdt:performVehicleSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `owned_vehicles` WHERE LOWER(`plate`) LIKE @query", {
		...
		TriggerClientEvent("rk-mdt:returnVehicleSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("rk-mdt:performVehicleSearchInFront")
AddEventHandler("rk-mdt:performVehicleSearchInFront", function(query)
	local usource = source
    ...
    if xPlayer.job.name == 'police' then
		MySQL.Async.fetchAll("SELECT * FROM `owned_vehicles` WHERE `plate` = @query", {
			...
			TriggerClientEvent("rk-mdt:toggleVisibilty", usource)
			TriggerClientEvent("rk-mdt:returnVehicleSearchInFront", usource, result, query)
		end)
	end
end)

RegisterServerEvent("rk-mdt:getVehicle")
AddEventHandler("rk-mdt:getVehicle", function(vehicle)
	local usource = source
	MySQL.Async.fetchAll("SELECT * FROM `characters` WHERE `identifier` = @query", {
		...
		TriggerClientEvent("rk-mdt:returnVehicleDetails", usource, vehicle)
	end)
end)

RegisterServerEvent("rk-mdt:getWarrants")
AddEventHandler("rk-mdt:getWarrants", function()
	local usource = source
	MySQL.Async.fetchAll("SELECT * FROM `mdt_warrants`", {}, function(warrants)
		...
		TriggerClientEvent("rk-mdt:returnWarrants", usource, warrants)
	end)
end)

RegisterServerEvent("rk-mdt:submitNewWarrant")
AddEventHandler("rk-mdt:submitNewWarrant", function(data)
	local usource = source
	...
	MySQL.Async.insert('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (@name, @char_id, @report_id, @report_title, @charges, @date, @expire, @notes, @author)', {
		...
		TriggerClientEvent("rk-mdt:completedWarrantAction", usource)
		sendToDiscord('rk- Warrant Logs', data.author .. ' Created a Warrant with id: '..data.name..' Report id: '..data.report_id..' Title: '..data.report_title..' Author of report: '..data.author..' Date of report: '..data.date..'')
	end)
end)

RegisterServerEvent("rk-mdt:deleteWarrant")
AddEventHandler("rk-mdt:deleteWarrant", function(id)
	local usource = source
	local author = GetCharacterName(source)
	MySQL.Async.execute('DELETE FROM `mdt_warrants` WHERE `id` = @id', {
		...
		TriggerClientEvent("rk-mdt:completedWarrantAction", usource)
		sendToDiscord('rk- Warrant Logs', author .. ' Deleted a warrant with id: '..id..'')
	end)
end)

RegisterServerEvent("rk-mdt:getReportDetailsById")
AddEventHandler("rk-mdt:getReportDetailsById", function(query, _source)
	...
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` = @query", {
		...
		if result and result[1] then
			...
			TriggerClientEvent("rk-mdt:returnReportDetails", usource, result[1])
		end
	end)
end)
