...
local weapons = {
    ...
    [GetHashKey('WEAPON_PISTOL_MK2')] = { suppressor = GetHashKey('component_at_pi_supp_02'), flashlight = nil, grip = nil, skin = nil },
...
Citizen.CreateThread(function()
    while RKCore == nil or RKCore.PlayerData == nil or RKCore.PlayerData.job == nil do
        Citizen.Wait(1)
    end
end)
...
AddEventHandler('eden_accesories:use', function( type )
    if weapons[GetSelectedPedWeapon(PlayerPedId())] and weapons[GetSelectedPedWeapon(PlayerPedId())][type] then
        if not HasPedGotWeaponComponent(GetPlayerPed(-1), GetSelectedPedWeapon(PlayerPedId()), weapons[GetSelectedPedWeapon(PlayerPedId())][type]) then
            ...
            RKCore.ShowNotification(string.format('%s %s', "You've equiped your ", type))
        else
            ...
            RKCore.ShowNotification(string.format('%s %s', "You've removed your ", type))
        end
    else
        RKCore.ShowNotification(string.format('%s %s %s', 'The ', type, " doesn't fit on your weapon.."))
    end
end)
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon)
    ...
    if (itemid == "tuner") then
      ...
      local finished = exports["rk-taskbar"]:taskBar(2000,"Connecting Tuner Laptop",false,false,playerVeh)
    ...
    if (itemid == "highgradefemaleseed" ) then
        ...
        TriggerServerEvent("rk-weedplants:highgradefemaleseed",itemid)
        end
    end
    --
    ...
    if (itemid == "lsdtab" or itemid == "badlsdtab") then
        ...
        local finished = exports["rk-taskbar"]:taskBar(3000,"Placing LSD Strip on 👅",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "decryptersess" or itemid == "decrypterfv2" or itemid == "decrypterenzo") then
      if (#(GetEntityCoords(player) - vector3( 1275.49, -1710.39, 54.78)) < 3.0) then
          local finished = exports["rk-taskbar"]:taskBar(25000,"Decrypting Data",false,false,playerVeh)
          if (finished == 100) then
    ...
    if (itemid == "cr") then
        TriggerEvent("rk-doorlock:UseRedKeycard", itemid)
    end

    if (itemid == "weedoz") then
 
      local finished = exports["rk-taskbar"]:taskBar(5000,"Packing Q Bags",false,false,playerVeh)
        if (finished == 100) then
    ...
    if ( itemid == "smallbud" and hasEnoughOfItem("qualityscales",1,false) ) then
        local finished = exports["rk-taskbar"]:taskBar(1000,"Packing Joint",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "weedq") then
        local finished = exports["rk-taskbar"]:taskBar(1000,"Rolling Joints",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "lighter") then
        TriggerEvent("animation:PlayAnimation","lighter")
          local finished = exports["rk-taskbar"]:taskBar(2000,"Starting Fire",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "joint" or itemid == "joint2") then
        local finished = exports["rk-taskbar"]:taskBar(2000,"Smoking Joint",false,false,playerVeh)
        if (finished == 100) then
            Wait(200)
    ...
    if (itemid == "Gruppe6Card") then
        TriggerServerEvent('rk-securityheists:banktstart')
    end


    if (itemid == "usbdevice") then
        local finished = exports["rk-taskbar"]:taskBar(15000,"Scanning For Networks",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "heavyammo") then
        ...
        local finished = exports["rk-taskbar"]:taskBar(3000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "armor") then
        ...
        local finished = exports["rk-taskbar"]:taskBar(5000,"Armor",true,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "bodybag") then
        local finished = exports["rk-taskbar"]:taskBar(10000,"Opening bag",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "bodygarbagebag") then
            local finished = exports["rk-taskbar"]:taskBar(10000,"Opening trash bag",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "foodsupplycrate") then
        ...
        local finished = exports["rk-taskbar"]:taskBar(20000,"Opening the crate (ESC to Cancel)",false,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "organcooler") then
        local finished = exports["rk-taskbar"]:taskBar(5000,"Opening cooler",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "Bankbox") then
        if (hasEnoughOfItem("locksystem",1,false)) then
            local finished = exports["rk-taskbar"]:taskBar(10000,"Opening bank box.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Securebriefcase") then
        if (hasEnoughOfItem("Bankboxkey",1,false)) then
            local finished = exports["rk-taskbar"]:taskBar(5000,"Opening briefcase.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Largesupplycrate") then
        if (hasEnoughOfItem("2227010557",1,false)) then
            local finished = exports["rk-taskbar"]:taskBar(15000,"Opening supply crate.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
            ...
            Citizen.CreateThread(function()
                while finished ~= 100 and not cancelNos do
                    ...
                    if GetEntitySpeed(GetVehiclePedIsIn(player, false)) > 11 then
                        exports["rk-taskbar"]:closeGuiFail()
                        cancelNos = true
                    end
                end
            end)
            finished = exports["rk-taskbar"]:taskBar(20000,"Nitrous")
    ...
    if (itemid == "ciggy") then
        local finished = exports["rk-taskbar"]:taskBar(1000,"Lighting Up",false,false,playerVeh)
        if (finished == 100) then
            Wait(300)
    ...
    if (itemid == "oxygentank") then
        local finished = exports["rk-taskbar"]:taskBar(30000,"Oxygen Tank",true,false,playerVeh)
        if (finished == 100) then        
    ...
    if (itemid == "bandage") then
        TaskItem("amb@world_human_clipboard@male@idle_a", "idle_c", 49,10000,"Healing","rk-hospital:items:bandage",true,itemid,playerVeh)
    end
    ...
    if (itemid == "methbag") then
        local finished = exports["rk-taskbarskill"]:taskBar(2500,10)
        if (finished == 100) then  
            TriggerEvent("attachItemObjectnoanim","crackpipe01")
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",6,1200)
    ...
    if (itemid == "shitlockpick") then
        ...
        local finished = exports["rk-taskbarskill"]:taskBar(2500,math.random(5,20))
        if (finished == 100) then    
...
function TaskItem(dictionary,animation,typeAnim,timer,message,func,remove,itemid,playerVeh)
    ...
    if timer > 0 then
        local finished = exports["rk-taskbar"]:taskBar(timer,message,true,false,playerVeh)
...
AddEventHandler('inv:lockPick', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            local finished = exports["rk-taskbarskill"]:taskBar(25000,3)
            ...
            local finished = exports["rk-taskbarskill"]:taskBar(2200,10)
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            local finished = exports["rk-taskbarskill"]:taskBar(math.random(5000,25000),math.random(10,20))
            if finished ~= 100 then
                 lockpicking = false
                return
            end
            ...
            TriggerEvent("civilian:alertPolice",20.0,"lockpick",targetVehicle)
            local finished = exports["rk-taskbarskill"]:taskBar(1500,math.random(5,15))
            if finished ~= 100 then
...
AddEventHandler('veh:repairing', function(inventoryName,slot,itemid)
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.5 then
           ...
           -- print(targetVehicle)
            local finished = exports["rk-taskbarskill"]:taskBar(15000,math.random(10,20))
            SetVehicleFixed(targetVehicle)
            ...
            --                 timer = 2000
            --             end
            --             local finished = exports["rk-taskbarskill"]:taskBar(timer,math.random(5,15))
            --             if finished ~= 100 then
            --                 fixingvehicle = false
            --                 reapiring = false
            --                 ClearPedTasks(playerped)
...
RegisterCommand("search", function(source, args)
    local player = RKCore.GetPlayerData()
...
RegisterCommand("seizecash", function(source, args)
    local player = RKCore.GetPlayerData()
    local closestPlayer, closestDistance = RKCore.Game.GetClosestPlayer()
    if closestPlayer ~= -1 and closestDistance <= 2.0 and RKCore.PlayerData.job.name == 'police' then
        local searchPlayerPed = GetPlayerPed(closestPlayer)
...
RegisterCommand("steal", function(source, args)
    ...
    if closestPlayer ~= -1 and closestDistance <= 1.0 then
        ...
        if IsEntityPlayingAnim(searchPlayerPed, 'missfbi5ig_22', 'hands_up_anxious_scientist', 3) or (IsEntityPlayingAnim(searchPlayerPed, "dead", "dead_a", 3)) then
            ...
            local finished = exports["rk-taskbar"]:taskBar(3500, "Robbing")
            TriggerServerEvent("people-search", GetPlayerServerId(closestPlayer)) --COPY THIS
...
RegisterNetEvent('rk-inventory:goldtrade')
AddEventHandler('rk-inventory:goldtrade', function()
	if exports["rk-inventory"]:hasEnoughOfItem("goldbar",70,true) then
		...
		if GoldBars < 1.5 then
			if isTime() then
				...
				if IsControlJustReleased(0,38) then
					if exports["rk-inventory"]:hasEnoughOfItem("goldbar",70,true) then
						local finished = exports["rk-taskbar"]:taskBar(6000,"Trading Bars")
						if finished == 100 then
                            TriggerEvent('rk-inventory:goldtrade')
						end
					end
					Citizen.Wait(1000)
				end
