...
$(document).ready(function () {
  ...
  window.addEventListener('message', function (event) {
    ...
    if (item.response == "openGui") {
      ...
      if (itemList[item.id]) {
        $.post("http://rk-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, true, itemList[item.id].nonStack, item.data, itemList[item.id].weight]));
      } else {
        $.post("http://rk-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, false, itemList[item.id].nonStack, item.data]));
      }
    }
  })
...
function invStack(targetSlot, moveAmount, targetInventory, originSlot, originInventory, purchase, itemCosts, itemidsent, amountmoving, crafting, weapon, amountRemaining) {
  ...
  $.post("http://rk-inventory/stack", JSON.stringify(arr));
};
...
function invMove(targetSlot, originSlot, targetInventory, originInventory, purchase, itemCosts, itemidsent, amountmoving, crafting, weapon) {
  ...
  $.post("http://rk-inventory/move", JSON.stringify(arr));
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory) {
  ...
  $.post("http://rk-inventory/swap", JSON.stringify(arr));
};

function removeCraftItems(itemid, moveAmount) {
  let arr = itemList[itemid].craft
  let amount = moveAmount;
  $.post("http://rk-inventory/removeCraftItems", JSON.stringify([arr, amount]));
};
...
function BuildDrop(brokenSlots) {
  $.post("http://rk-inventory/dropIncorrectItems", JSON.stringify({
    slots: brokenSlots
  }));
}
  ...
  for (i = 0; i < slotLimit + 1; i++) {
    ...
    if (isEmpty(inventory[i])) {
      ...
      if (itemList[itemid] !== undefined) {
        ...
        $.post("http://rk-inventory/updateMyQuality", JSON.stringify({
          item_id: itemid,
          slot: slot,
          quality: quality
        }));
...
function UpdateSetWeights() {
  ...
  $.post("http://rk-inventory/Weight", JSON.stringify({
    weight: meh
  }));
}
...
function DragToggle(slot,using) {
  ...
  if (slot) {
    ...
    if (occupiedslot == true && dragging == false) {
      ...
      $.post("http://rk-inventory/SlotInuse", JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function CompileStacks(targetSlot, originSlot, inv1, inv2, originAmount, remainingAmount, targetAmount, purchase, itemCosts, itemidsent, moveAmount) {
  ...
  $.post("http://rk-inventory/SlotJustUsed", JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2) {
  // $.post('http://rk-inventory/swapstack', JSON.stringify({
  //   slot1: slot1,
  //   slot2: slot2,
  //   inv1: inv1,
  //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
  ...
  $.post("http://rk-inventory/ServerCloseInventory", JSON.stringify({
    name: TargetInventoryName
  }));
  TargetInventoryName = "none";
  $.post("http://rk-inventory/Close", JSON.stringify({
...
function useitem() {
  ...
  if (dragging != false) {
    ...
    if (inventoryUsedName == PlayerInventoryName) {
      ...
      $.post("http://rk-inventory/invuse", JSON.stringify(arr));
