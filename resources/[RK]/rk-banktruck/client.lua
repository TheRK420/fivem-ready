...
RegisterNetEvent('rk_TruckRobbery:outlawNotify')
AddEventHandler('rk_TruckRobbery:outlawNotify', function(alert)
	if isPlayerWhitelisted then
		...
		TriggerServerEvent('heist:truck',{
			x = RKCore.Math.Round(playerCoords.x, 1),
			y = RKCore.Math.Round(playerCoords.y, 1),
			z = RKCore.Math.Round(playerCoords.z, 1)
		}, streetName, emergency)
	end
end)

function refreshPlayerWhitelisted()	
	if not RKCore.PlayerData then
		return false
	end

	if not RKCore.PlayerData.job then
		return false
	end

	if Config.PoliceDatabaseName == RKCore.PlayerData.job.name then
		return true
	end
		...
		for k,v in pairs(Config.MissionSpot) do
			...
			if distance <= 1.0 then
				...
				if IsControlJustPressed(0, Config.KeyToStartMission) then
					--RKCore.TriggerServerCallback('truck:HasItemkit', function(cb)
						if exports['rk-inventory']:hasEnoughOfItem('electronickit', 1, true) then
							TriggerServerEvent("rk_TruckRobbery:missionAccepted")
							Citizen.Wait(500)
						else
...
RegisterNetEvent('rk_TruckRobbery:TruckRobberyInProgress')
AddEventHandler('rk_TruckRobbery:TruckRobberyInProgress', function(targetCoords)
...
RegisterNetEvent("rk_TruckRobbery:HackingMiniGame")
AddEventHandler("rk_TruckRobbery:HackingMiniGame",function()
	toggleHackGame()
end)
...
function toggleHackGame()
	...
	exports['rk-taskbar']:taskBar((Config.RetrieveMissionTimer * 1000), Config.Progress1)
...
function AtmHackSuccess(success)
    ...
    if success then
		RKCore.TriggerServerCallback("rk_TruckRobbery:StartMissionNow",function()	end)
    else
		RKCore.ShowNotification(Config.HackingFailed)
...
RegisterNetEvent("rk_TruckRobbery:startMission")
AddEventHandler("rk_TruckRobbery:startMission",function(spot)
	...
	if numy == 100 then
		RKCore.ShowNotification(Config.NoMissionsAvailable)
	else
		CurrentEventNum = num
		TriggerEvent("rk_TruckRobbery:startTheEvent",num)
		PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
		RKCore.ShowNotification(Config.TruckMarkedOnMap)
	end
end)

-- Core Mission Part
RegisterNetEvent('rk_TruckRobbery:startTheEvent')
AddEventHandler('rk_TruckRobbery:startTheEvent', function(num)
	
	local loc = Config.ArmoredTruck[num]
	Config.ArmoredTruck[num].InUse = true
	local playerped = GetPlayerPed(-1)
	
	TriggerServerEvent("rk_TruckRobbery:syncMissionData",Config.ArmoredTruck)
	...
	while not taken do
		...
		if missionInProgress == true then
			...
			if distance <= 30.0  then
				if KillGuardsText == false then
					RKCore.ShowNotification(Config.KillTheGuards)
					KillGuardsText = true
				end
			end
			
			if distance <= 5 and TruckIsDemolished == false then
				RKCore.ShowHelpNotification(Config.OpenTruckDoor)
				if IsControlJustPressed(1, Config.KeyToOpenTruckDoor) then
					--RKCore.TriggerServerCallback('truck:HasItemc4', function(cb)
						if exports['rk-inventory']:hasEnoughOfItem('c4_bank', 1, true) then
		...
		if TruckIsExploded == true then
			...
			if distance <= 4.5 then
				RKCore.ShowHelpNotification(Config.RobFromTruck)
		...
		if missionCompleted == true then
			RKCore.ShowNotification(Config.MissionCompleted)
...
function BlowTheTruckDoor()
	if IsVehicleStopped(ArmoredTruckVeh) then
		if (IsVehicleSeatFree(ArmoredTruckVeh, -1) and IsVehicleSeatFree(ArmoredTruckVeh, 0) and IsVehicleSeatFree(ArmoredTruckVeh, 1)) then
			...
			-- 	TriggerServerEvent('rk_TruckRobbery:TruckRobberyInProgress',GetEntityCoords(PlayerPedId()),streetName)
			...
			exports['rk-taskbar']:taskBar(5500, Config.Progress2)
			Citizen.Wait(5500)
			...
			exports['rk-taskbar']:taskBar((Config.DetonateTimer * 1000), Config.Progress3)	
			...
			RKCore.ShowNotification(Config.BeginToRobTruck)
		else
			RKCore.ShowNotification(Config.GuardsNotKilledYet)
		end
	else
		RKCore.ShowNotification(Config.TruckIsNotStopped)
	end
end
...
function RobbingTheMoney()
	...
	exports['rk-taskbar']:taskBar((Config.RobTruckTimer * 1000), Config.Progress4)
	...
	TriggerServerEvent("rk_TruckRobbery:missionComplete")
...
-- Sync mission data
RegisterNetEvent("rk_TruckRobbery:syncMissionData")
