...
--AddEventHandler('onResourceStart', function(resourceName) -- For debugging to start/stop resources without server restart
	...
	Citizen.Wait(5000)
	RKCore.TriggerServerCallback('sody_clubs:getPlayerClub', function(playerdata)
...
AddEventHandler('sody_clubs:action', function(zone)
	...
	if zone.Type == "storage" then
		...
		RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'job_actions', {
			title    = _U(menutitle),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
			if data.current.value == 'put_stock' then
				RKCore.UI.Menu.CloseAll()
				Citizen.Wait(500)
				TriggerEvent("server-inventory-open", "1", "gangstash: "..PlayerClub)
				--OpenPutStocksMenu()
	...
	elseif zone.Type == "storagepriv" then
		...
		RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'job_actions', {
			...
			if data.current.value == 'put_stock' then
				...
				TriggerEvent("server-inventory-open", "1", "gangstashprv: "..PlayerClub)
			elseif data.current.value == 'get_stock' then
	...
	elseif zone.Type == "storagepub" then
		...
		RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'job_actions', {
			...
			if data.current.value == 'put_stock' then
				...
				TriggerEvent("server-inventory-open", "1", "gangstashpub: "..PlayerClub)
			elseif data.current.value == 'get_stock' then
	...
	elseif zone.Type == "gteleport" then
		...
		elseif not vehicle then
			RKCore.ShowNotification("Must be driving a vehicle to exit through garage")
		elseif vehicle and GetPedInVehicleSeat(vehicle, -1) ~= GetPlayerPed(-1) then
			RKCore.ShowNotification("Must be driving the vehicle to exit garage")
		end
...
Citizen.CreateThread(function()
	while true do
		...
		if hintIsShowed and not menuIsShowed then
			RKCore.ShowHelpNotification(hintToDisplay)
		else
			Citizen.Wait(500)
		end
	end
...
function OpenClubOwnerMenu(club, rank)
	...
	RKCore.TriggerServerCallback('sody_clubs:getOwner', function(result)
		isOwner = tonumber(result)
	end, club)

	RKCore.TriggerServerCallback('sody_clubs:getTreasurer', function(result)
	...
	RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'owner_actions_' .. club, {
		title    = _U('owner_menu'),
		align    = 'top-right',
		elements = elements
	}, function(data, menu)
		...
		elseif data.current.value == 'withdraw_bank_money' then
			...
			RKCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'withdraw_club_money_amount_' .. club, {
				...
				if amount == nil then
					RKCore.ShowNotification(_U('invalid_amount'))
				else
					menu.close()
		...
		elseif data.current.value == 'deposit_bank_money' then
			...
			RKCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_bank_money_amount_' .. club, {
...
function OpenManageMembersMenu(club)
	RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_members_' .. club, {
...
function OpenMemberList(club)
	RKCore.TriggerServerCallback('sody_clubs:getMembers', function(members, club)
		local elements = {}
		...
		RKCore.UI.Menu.Open('list', GetCurrentResourceName(), 'member_list_' .. club, elements, function(data, menu)
			...
			elseif data.value == 'remove' then
				RKCore.ShowNotification(_U('you_have_fired', member.name))

				RKCore.TriggerServerCallback('sody_clubs:setClub', function()
					OpenMemberList(club)
...
function OpenRecruitMenu(club)
	RKCore.TriggerServerCallback('sody_clubs:getOnlinePlayers', function(players)
		...
		RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_' .. club, {
			title    = _U('recruiting'),
			align    = 'top-right',
			elements = elements
		}, function(data, menu)

			RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_confirm_' .. club, {
				...
				if data2.current.value == 'yes' then
					RKCore.ShowNotification(_U('you_have_hired', data.current.name))
...
function OpenPromoteMenu(clubname, member)
	RKCore.TriggerServerCallback('sody_clubs:getClub', function(club)
		...
		RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'promote_member_' .. clubname, {
			title    = _U('promote_member', member.name),
			align    = 'top-right',
			elements = elements
		}, function(data, menu)
			menu.close()
			RKCore.ShowNotification(_U('you_have_promoted', member.name, data.current.label))
...
function OpenManagePayMenu(clubname)
	RKCore.TriggerServerCallback('sody_clubs:getClub', function(club)
		...
		for i=1, #club.ranks, 1 do
			...
			table.insert(elements, {
				label = ('%s - <span style="color:green;">%s</span>'):format(rankLabel, _U('money_generic', RKCore.Math.GroupDigits(club.ranks[i].pay))),
				value = club.ranks[i].club_rank
			})
		end

		RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_pay_' .. clubname, {
			title    = _U('pay_management'),
			align    = 'top-right',
			elements = elements
		}, function(data, menu)

			RKCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'manage_pay_amount_' .. clubname, {
				title = _U('pay_amount')
			}, function(data2, menu2)
				...
				if amount == nil then
					...
					RKCore.TriggerServerCallback('sody_clubs:setClubPay', function()
...
function playerZones()
	Citizen.CreateThread(function()
		while hasClub do
			...
			if RKCore.Table.SizeOf(zones) > 0 then
...
function OpenGetStocksMenu()
	RKCore.TriggerServerCallback('sody_clubs:getStockItems', function(inventory)
		local elements = {}
		local menutitle = PlayerClub .. "menu"

		if inventory.blackMoney > 0 then
			table.insert(elements, {
				label = _U('dirty_money', RKCore.Math.GroupDigits(inventory.blackMoney)),
				type = 'item_account',
				value = 'black_money'
			})
		end
		...
		for i=1, #inventory.weapons, 1 do
			...
			table.insert(elements, {
				label = RKCore.GetWeaponLabel(weapon.name) .. ' [' .. weapon.ammo .. ']',
				type  = 'item_weapon',
				value = weapon.name,
				ammo  = weapon.ammo
			})
		...
		RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			if data.current.type == 'item_weapon' then
				...
				RKCore.SetTimeout(300, function()
					OpenGetStocksMenu()
				end)
			else
				...
				RKCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
					...
					if count == nil then
						RKCore.ShowNotification(_U('invalid_quantity'))
					else
						menu2.close()
						menu.close()
...
function OpenGetStocksMenuPub()
	RKCore.TriggerServerCallback('sody_clubs:getStockItemsPub', function(inventory)
		local elements = {}
		local menutitle = PlayerClub .. "menu"
...
function OpenGetStocksMenuPriv()
	RKCore.TriggerServerCallback('sody_clubs:getStockItemsPriv', function(inventory)
		local elements = {}
		local menutitle = PlayerClub .. "menu"
...
function OpenPutStocksMenu()
	RKCore.TriggerServerCallback('sody_clubs:getPlayerInventory', function(inventory)
		local elements = {}
...
function OpenChangingRoomMenu()
	RKCore.TriggerServerCallback('rk-housing:server:getSavedOutfits', function(dressing)
		local elements = {}
		...
		RKCore.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing',
		{
			...
			TriggerEvent('rk-clothing:getSkin', function(skin)
				RKCore.TriggerServerCallback('rk-housing:server:getSavedOutfits', function(clothes)
					TriggerEvent('rk-clothing:loadClothes', skin, clothes)
					TriggerEvent('rk-skin:setLastSkin', skin)

					TriggerEvent('rk-clothing:getSkin', function(skin)
						TriggerServerEvent('rk-skin:save', skin)
					end)
	...
	RKCore.UI.Menu.Open(
		...
		function(data, menu)
			...
			if(data.current.value == 'list_vehicles') then
				if Config.GarageScript == 'rk-garagenew' then
					ListVehiclesMenu()
				elseif Config.GarageScript == 'rk-garagenew' then
					ListVehiclesMenuAG()
				end
			end
...
function ListVehiclesMenu()
	...
	RKCore.TriggerServerCallback('betrayed_garage:obtenerVehiculos', function(vehicles)
	...
	Citizen.Wait(10)
 	...
 	for _,v in pairs(vehicles) do
			...
			local teststring = tostring(RKCore.Math.Trim(GetLabelText(hashVehicule)))
...
function ListVehiclesMenuAG()
	...
	RKCore.TriggerServerCallback('rk_advancedgarage:getOwnedCars', function(vehicles)
...
function StockVehicleMenu()
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		RKCore.TriggerServerCallback('eden_garage:stockv',function(valid)

			if (valid) then
				RKCore.TriggerServerCallback('eden_garage:getVehicles', function(vehicules)
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		RKCore.TriggerServerCallback('rk_advancedgarage:storeVehicle',function(valid)
			...
			if (valid) then
				...
				TriggerServerEvent('rk_advancedgarage:setVehicleState', vehicleProps.plate, true)
...
function SpawnVehicleClubAG(vehicle)
	...
	TriggerServerEvent('rk_advancedgarage:setVehicleState', vehicle.plate, false)
end
