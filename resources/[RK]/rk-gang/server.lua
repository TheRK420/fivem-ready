...
TriggerEvent('es:addGroupCommand', 'setclub', 'jobmaster', function(source, args, user)
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		local xPlayer = RKCore.GetPlayerFromId(args[1])
...
RKCore.RegisterServerCallback('sody_clubs:getPlayerClub', function(source, cb)
	local _source = source
	local xPlayer = nil
	while xPlayer == nil do
		xPlayer = RKCore.GetPlayerFromId(_source)
		Wait(10)
	end
...
-- RKCore.RegisterServerCallback('sody_clubs:getStockItems', function(source, cb, clubname)
-- 	local clubfullname =  "club_" .. clubname
-- 	local blackMoney = 0
-- 	local items      = {}
-- 	local weapons    = {}

-- 	TriggerEvent('rk-addonaccount:getSharedAccount', clubfullname .. '_black', function(account)
-- 		blackMoney = account.money
-- 	end)

-- 	TriggerEvent('rk-datastore:getSharedDataStore', clubfullname, function(store)
...
-- 	TriggerEvent('rk-addoninventory:getSharedInventory', clubfullname, function(inventory)
...
-- RKCore.RegisterServerCallback('sody_clubs:getStockItemsPub', function(source, cb, clubname)
-- 	local clubfullname =  "club_" .. clubname .. "_pub"
-- 	local blackMoney = 0
-- 	local items      = {}
-- 	local weapons    = {}

-- 	TriggerEvent('rk-addonaccount:getSharedAccount', clubfullname, function(account)
-- 		blackMoney = account.money
-- 	end)
...
-- RKCore.RegisterServerCallback('sody_clubs:getStockItemsPriv', function(source, cb, clubname)
...
-- 				end
-- 			else
-- 				TriggerClientEvent('rk:showNotification', xPlayer.source, _U('invalid_quantity'))
-- 			end
-- 		end)
...
-- 			else
-- 				TriggerClientEvent('rk:showNotification', _source, _U('amount_invalid'))
-- 			end
-- 		end)
...
-- 	local xPlayer = nil
-- 	while xPlayer == nil do
-- 		xPlayer = RKCore.GetPlayerFromId(source)
-- 		Wait(10)
-- 	end
-- 	local clubfullname =  "club_" .. clubname
...
-- RKCore.RegisterServerCallback('sody_clubs:getPlayerInventory', function(source, cb)
-- 	local xPlayer = nil
-- 	while xPlayer == nil do
-- 		xPlayer = RKCore.GetPlayerFromId(source)
-- 		Wait(10)
...
RKCore.RegisterServerCallback('sody_clubs:getOwner', function(source, cb, club)
	local hasOwner = -1
...
RKCore.RegisterServerCallback('sody_clubs:getTreasurer', function(source, cb, club)
...
AddEventHandler('sody_clubs:bankBalance', function(club)
	...
	TriggerEvent('rk-addonaccount:getSharedAccount', bankname, function(account)
		TriggerClientEvent('rk:showNotification', _source, 'Account balance: ~g~$' .. account.money)
	end)
end)
...
AddEventHandler('sody_clubs:withdrawBank', function(club, amount)
	...
	amount = RKCore.Math.Round(tonumber(amount))
	...
	TriggerEvent('rk-addonaccount:getSharedAccount', bankname, function(account)
		if amount > 0 and account.money >= amount then
			...
			TriggerClientEvent('rk:showNotification', xPlayer.source, _U('have_withdrawn', RKCore.Math.GroupDigits(amount)))
		else
			TriggerClientEvent('rk:showNotification', xPlayer.source, _U('invalid_amount'))
		end
	end)
end)
...
AddEventHandler('sody_clubs:depositBank', function(club, amount)
	...
	if amount > 0 and xPlayer.getMoney() >= amount then
		...
		TriggerClientEvent('rk:showNotification', xPlayer.source, _U('have_deposited', RKCore.Math.GroupDigits(amount)))
	else
		TriggerClientEvent('rk:showNotification', xPlayer.source, _U('invalid_amount'))
	end
end)

RKCore.RegisterServerCallback('sody_clubs:getClub', function(source, cb, clubname)
...
RKCore.RegisterServerCallback('sody_clubs:setClub', function(source, cb, identifier, club, rank, type)
...
function setClub(sourceplayer, identifier, club, rank, type)
	...
	while xPlayer == nil do
		xPlayer = RKCore.GetPlayerFromId(sourceplayer)
		Wait(10)
	end

	xTarget = RKCore.GetPlayerFromIdentifier(identifier)
	...
	MySQL.Async.execute('UPDATE users SET club = @club, club_rank = @club_rank WHERE identifier = @identifier', {
		...
		if rowsChanged then
			if type == 'hire' then
				if xTarget then
					TriggerClientEvent('rk:showNotification', xTarget.source, _U('you_have_been_hired', ClubListFull[club]))
					TriggerClientEvent('sody_clubs:clubAdded', xTarget.source, club, rank, Clubs[club].ranks[tostring(rank)].club_rank_name)
				end
			elseif type == 'promote' then
				if xTarget then
					TriggerClientEvent('rk:showNotification', xTarget.source, _U('you_have_been_promoted'))
					TriggerClientEvent('sody_clubs:clubPromoted', xTarget.source, rank, Clubs[club].ranks[tostring(rank)].club_rank_name)
				end
			elseif type == 'remove' then
				if xTarget then
					TriggerClientEvent('rk:showNotification', xTarget.source, _U('you_have_been_fired', ClubListFull[club]))
			...
			TriggerClientEvent('rk:showNotification', xPlayer.source, 'Club: System Error')
		end
...
RKCore.RegisterServerCallback('sody_clubs:getMembers', function(source, cb, club)
	if Config.EnableRKCoreIdentity then
...
RKCore.RegisterServerCallback('sody_clubs:setClubPay', function(source, cb, club, rank, pay)
	if pay == 1 then
		pay = 0
	end
...
RKCore.RegisterServerCallback('sody_clubs:getOnlinePlayers', function(source, cb)
...
function clubPay()
	...
	MySQL.Async.fetchAll('SELECT firstname, lastname, identifier, club, club_rank FROM users WHERE club IS NOT NULL ORDER BY club_rank DESC', {}, function(results)
		...
		for i=1, #xPlayers, 1 do
			local xPlayerCheck = RKCore.GetPlayerFromId(xPlayers[i])
			table.insert(xPlayersChecks, xPlayerCheck.identifier)
		end

		if RKCore.Table.SizeOf(results) > 0 then
		...
		for _,k in pairs(clubMembers) do
			local xPlayer = RKCore.GetPlayerFromIdentifier(k.identifier)
					...
					if Config.EnableClubBankPay then
						...
						TriggerEvent('rk-addonaccount:getSharedAccount', bankname, function (account)
							if account.money >= pay then
								...
								TriggerClientEvent('rk:showAdvancedNotification', xPlayer.source, ClubListFull[club], _U('received_paycheck'), _U('received_pay', pay), 'CHAR_MP_DETONATEPHONE', 9)
							else
								TriggerClientEvent('rk:showAdvancedNotification', xPlayer.source, ClubListFull[club], '', _U('company_nomoney'), 'CHAR_MP_DETONATEPHONE', 1)
								end
						end)
					else
						xPlayer.addAccountMoney('bank', pay)
