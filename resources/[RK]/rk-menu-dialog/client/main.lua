Citizen.CreateThread(function()
	...
	local openMenu = function(namespace, name, data)
		for i=1, #Timeouts, 1 do
			RKCore.ClearTimeout(Timeouts[i])
		end
		...
		local timeoutId = RKCore.SetTimeout(200, function()
	...
	RKCore.UI.Menu.RegisterType(MenuType, openMenu, closeMenu)

	RegisterNUICallback('menu_submit', function(data, cb)
		local menu = RKCore.UI.Menu.GetOpened(MenuType, data._namespace, data._name)
		local post = true
		...
		if menu.submit ~= nil then
			...
			if tonumber(data.value) ~= nil then
				...
				-- Round float values
				data.value = RKCore.Math.Round(tonumber(data.value))
			...
			if post then
				...
				TriggerEvent('DoLongHudText', 'That input is invalid!', 2)
			end
		end
