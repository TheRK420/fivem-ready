...
AddEventHandler('rk-skin:setLastSkin', function(skin)
    LastSkin = skin
  end)
...
function Save(save)
    if save then
        data = GetCurrentPed()
        TriggerServerEvent("rk-clothing:insert_character_current", data)
        if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
            TriggerServerEvent("rk-clothing:insert_character_face", data)
            TriggerServerEvent("rk-clothing:set_tats", currentTats)
        end
    else
        LoadPed(oldPed)
    end
...
RegisterNetEvent("rk-clothing:guardarSkin")
AddEventHandler("rk-clothing:guardarSkin", function()
    data = GetCurrentPed()
    TriggerServerEvent("rk-clothing:insert_character_current", data)
    if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
        TriggerServerEvent("rk-clothing:insert_character_face", data)
...
RegisterNetEvent("rk-clothing:inService")
AddEventHandler("rk-clothing:inService", function(service)
    isService = service
end)

RegisterNetEvent('rk-identity:identityCheck')
AddEventHandler('rk-identity:identityCheck', function(identityCheck)
	hasIdentity = identityCheck
end)

RegisterNetEvent("rk-clothing:hasEnough")
AddEventHandler("rk-clothing:hasEnough", function(menu)
    if menu == "tattoomenu" then
        TriggerServerEvent("rk-clothing:retrieve_tats")
...
RegisterNetEvent("rk-clothing:setclothes")
AddEventHandler("rk-clothing:setclothes", function(data,alreadyExist)
    player = GetPlayerPed(-1)
    local function setDefault()
        Citizen.CreateThread(function()
            firstChar = true
            local gender = nil
            RKCore.TriggerServerCallback('rk-clothing:getSex', function(sex)
				gender = sex
			end)
            Citizen.Wait(1000)
            if gender ~= "m" then
            ...
            DoScreenFadeIn(50)
            Wait(5000)
            if not exports["rk-inventory"]:hasEnoughOfItem("idcard",1,false) then
                TriggerEvent("player:receiveItem","idcard",1,true)
            end
            if not exports["rk-inventory"]:hasEnoughOfItem("mobilephone",1,false)then
    ...
    TriggerServerEvent("rk-clothing:get_character_face")
    TriggerServerEvent("rk-clothing:retrieve_tats")
end)


RegisterNetEvent("rk-clothing:defaultReset")
AddEventHandler("rk-clothing:defaultReset", function()
    local gender = nil
    RKCore.TriggerServerCallback('rk-clothing:getSex', function(sex)
        gender = sex
    end)
...
RegisterNetEvent("rk-clothing:settattoos")
AddEventHandler("rk-clothing:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("rk-clothing:setpedfeatures")
AddEventHandler("rk-clothing:setpedfeatures", function(data)
    player = GetPlayerPed(-1)
    if data == false then
...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if exports["rk-housing"]:imClosesToRoom2() or exports["rk-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] and args[2] then
            TriggerEvent('rk-clothing:outfits', 1, args[1], args[2])
        else
...
RegisterCommand("outfituse", function(source, args, rawCommand)
    if exports["rk-housing"]:imClosesToRoom2() or exports["rk-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            TriggerEvent('rk-clothing:outfits', 3, args[1])
        else
...
RegisterCommand("outfitdel", function(source, args, rawCommand)
    if exports["rk-housing"]:imClosesToRoom2() or exports["rk-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            TriggerEvent('rk-clothing:outfits', 2, args[1])
        else
            TriggerEvent('DoLongHudText', "You need to do something like /outfituse 1 | 1 being the slot id that you will have had previously saved", 1)  
        end
    else
...
RegisterCommand("outfits", function(source, args, rawCommand)
    if exports["rk-housing"]:imClosesToRoom2() or exports["rk-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        TriggerEvent('rk-clothing:outfits', 4)
    else
        TriggerEvent('DoLongHudText', "You are not near a wardrobe", 2)
    end
end, false)  

RegisterNetEvent('rk-clothing:listOutfits')
AddEventHandler('rk-clothing:listOutfits', function(skincheck)
...
RegisterNetEvent('rk-clothing:outfits')
AddEventHandler('rk-clothing:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("rk-clothing:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("rk-clothing:remove_outfit",pId)
    elseif pAction == 3 then
        TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("rk-clothing:get_outfit", pId)
    else
        TriggerServerEvent("rk-clothing:list_outfits")
    end
end)
