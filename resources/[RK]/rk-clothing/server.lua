...
RegisterServerEvent('rk-skin:save')
AddEventHandler('rk-skin:save', function()
    local src = source
    TriggerClientEvent('rk-clothing:guardarSkin',src)
end)

RKCore.RegisterServerCallback('rk-skin:getPlayerSkin', function(source, cb)
...
RKCore.RegisterServerCallback('rk-clothing:getSex', function(source, cb)
    local xPlayer = RKCore.GetPlayerFromId(source)
...
RegisterServerEvent("rk-clothing:insert_character_current")
AddEventHandler("rk-clothing:insert_character_current",function(data)
    if not data then return end
    local src = source
    local user = RKCore.GetPlayerFromId(src)
...
RegisterServerEvent("rk-clothing:insert_character_face")
AddEventHandler("rk-clothing:insert_character_face",function(data)
    if not data then return end
    local src = source
...
RegisterServerEvent("rk-clothing:get_character_face")
AddEventHandler("rk-clothing:get_character_face",function(pSrc)
    ...
    MySQL.Async.fetchAll("SELECT cc.model, cf.hairColor, cf.headBlend, cf.headOverlay, cf.headStructure FROM users cf INNER JOIN users cc on cc.identifier = cf.identifier WHERE cf.identifier = @identifier", {['identifier'] = characterId}, function(result)
        if (result ~= nil and result[1] ~= nil) then
            ...
            if model == 1885233650 or model == -1667301416 then
                TriggerClientEvent("rk-clothing:setpedfeatures", src, temp_data)
            end
        else
            TriggerClientEvent("rk-clothing:setpedfeatures", src, false)
        end
	end)
end)

RegisterServerEvent("rk-clothing:get_character_current")
AddEventHandler("rk-clothing:get_character_current",function(pSrc)
    local src = (not pSrc and source or pSrc)
    local user = RKCore.GetPlayerFromId(src)
    if not user then return end
    local characterId = user.identifier
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['identifier'] = characterId}, function(result)
        ...
        TriggerClientEvent("rk-clothing:setclothes", src, temp_data,0)
	end)
end)

RegisterServerEvent("rk-clothing:retrieve_tats")
AddEventHandler("rk-clothing:retrieve_tats", function(pSrc)
    local src = (not pSrc and source or pSrc)
    local user = RKCore.GetPlayerFromId(src)
    if not user then return end
	MySQL.Async.fetchAll("SELECT * FROM playerstattoos WHERE identifier = @identifier", {['identifier'] = user.identifier}, function(result)
        if(#result == 1) then
			TriggerClientEvent("rk-clothing:settattoos", src, json.decode(result[1].tattoos))
		else
			local tattooValue = "{}"
			...
			TriggerClientEvent("rk-clothing:settattoos", src, {})
		end
	end)
end)

RegisterServerEvent("rk-clothing:set_tats")
AddEventHandler("rk-clothing:set_tats", function(tattoosList)
	local src = source
	local user = RKCore.GetPlayerFromId(src)
...
RegisterServerEvent("rk-clothing:get_outfit")
AddEventHandler("rk-clothing:get_outfit",function(slot)
    ...
    MySQL.Async.fetchAll("SELECT * FROM character_outfits WHERE steamid = @steamid and slot = @slot", {
        ...
        if result and result[1] then
            ...
            TriggerClientEvent("rk-clothing:setclothes", src, data,0)
...
RegisterServerEvent("rk-clothing:set_outfit")
AddEventHandler("rk-clothing:set_outfit",function(slot, name, data)
    if not slot then return end
    local src = source
...
RegisterServerEvent("rk-clothing:remove_outfit")
AddEventHandler("rk-clothing:remove_outfit",function(slot)
...
RegisterServerEvent("rk-clothing:list_outfits")
AddEventHandler("rk-clothing:list_outfits",function()
    local src = source
    local user = RKCore.GetPlayerFromId(src)
    local steamid = user.identifier
    local slot = slot
    ...
    MySQL.Async.fetchAll("SELECT slot, name FROM character_outfits WHERE steamid = @steamid", {['steamid'] = steamid}, function(skincheck)
    	TriggerClientEvent("rk-clothing:listOutfits",src, skincheck)
	end)
end)
        ...
        if result[1] == nil then
            MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {["identifier"] = steamid}, function(result)
                if result[1].skin then
                    TriggerClientEvent('rk-clothing:setclothes',src,{},json.decode(result[1].skin))
                else
                    TriggerClientEvent('rk-clothing:setclothes',src,{},nil)
                end
                return
            end)
        else
            TriggerEvent("rk-clothing:get_character_current", src)
        end
        TriggerClientEvent("rk-clothing:inService",src,isService)
    end)
end)
...
AddEventHandler("clothing:checkMoney", function(menu,askingPrice)
    local src = source
    local target = RKCore.GetPlayerFromId(src)
    ...
    if (tonumber(target.getMoney()) >= askingPrice) then
        ...
        TriggerClientEvent("rk-clothing:hasEnough",src,menu)
    else
