...
AddEventHandler("rk:playerLoaded", function(playerData)
	RKCore.PlayerData = playerData
end)

RegisterNetEvent("rk:setJob")
AddEventHandler("rk:setJob", function(newJob)
	RKCore.PlayerData["job"] = newJob
end)
						...
						if dstCheck <= markerSize - 0.1 then
							...
							if usable then
								if IsControlJustPressed(0, 38) and Menu.hidden then
									...
									RKCore.TriggerServerCallback("betrayed_garage:obtenerVehiculos", function(fetchedVehicles)
