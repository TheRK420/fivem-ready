...
SpawnLocalVehicle = function(data)
	...
	if not RKCoreGame.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
	...
	RKCoreGame.SpawnLocalVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
...
SpawnVehicle = function(data,recuperar)
	...
	CloseMenu()
	local gameVehicles = RKCoreGame.GetVehicles()
	...
	RKCoreGame.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
		SetVehicleProperties(yourVehicle, vehicleProps)
...
PutInVehicle = function()
    ...
    if DoesEntityExist(vehicle) then
		...
		RKCoreTriggerServerCallback("erp_garage:validateVehicle", function(valid)
			if valid then
				AbrirMenuGuardar()
            else
                CloseMenu()
...
SetVehicleProperties = function(vehicle, vehicleProps)
    RKCoreGame.SetVehicleProperties(vehicle, vehicleProps)
...
GetVehicleProperties = function(vehicle)
    if DoesEntityExist(vehicle) then
        local vehicleProps = RKCoreGame.GetVehicleProperties(vehicle)
...
DrawScriptMarker = function(markerData)
    DrawMarker(20, markerData["pos"],0,0,0,0,0,0,0.701,1.0001,0.3001,222, 50, 50, 0.05,0, 0,0,0,0,0,0)
      --  RKCoreShowHelpNotification("Press [E] to Access Garage")
end
