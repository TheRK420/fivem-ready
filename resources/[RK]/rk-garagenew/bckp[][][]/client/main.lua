...
RegisterNetEvent("cc:playerLoaded")
AddEventHandler("cc:playerLoaded", function(playerData)
	RKCorePlayerData = playerData
end)

RegisterNetEvent("cc:setJob")
AddEventHandler("cc:setJob", function(newJob)
	RKCorePlayerData["job"] = newJob
end)
						...
						if dstCheck <= markerSize - 0.1 then
							...
							if usable then
								if IsControlJustPressed(0, 38) and Menu.hidden then
									...
									RKCoreTriggerServerCallback("erp_garage:obtenerVehiculos", function(fetchedVehicles)
