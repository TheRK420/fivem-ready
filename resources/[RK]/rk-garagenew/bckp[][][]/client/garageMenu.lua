...
function MenuGarage(action)
    ...
    DeleteActualVeh()
    if action == "menu" then
        Menu.addButton("See Your Vehicles","ListeVehicule",nil)
   ...
   Menu.addButton("Turn back","MenuGarage",nil)
    for c,v in pairs(fueravehicles) do
        ...
        if v.state == 0 or v.state == false then
            Menu.addButton("SAVE | "..GetDisplayNameFromVehicleModel(vehicle.model), "pagorecupero", vehicle, "IMPOUND", " Engine : " .. round(vehicle.engineHealth) /10 .. "%", " Fuel : " .. round(vehicle.fuelLevel) .. "%","SpawnLocalVehicle")
        end
    end 
end

function pagorecupero(data)
    RKCoreTriggerServerCallback('erp_garage:checkMoney', function(hasEnoughMoney)
        if hasEnoughMoney == true then
            SpawnVehicle({data,nil},true)
        elseif hasEnoughMoney == "deudas" then
            recuperar()
            TriggerEvent('notification', 'You owe the government more than $ 2000, you cant get your car back until you pay the fines!', 2)
        else
            recuperar()
            TriggerEvent('notification', 'There is no money on you', 2)							
        end
    end)
end
...
function ListeVehicule()
   ...
   Menu.addButton("Turn back","MenuGarage",nil)
    for c,v in pairs(fetchedVehicles) do
        if v then
            local vehicle = v.vehiculo
            Menu.addButton("" ..(vehicle.plate).." | "..GetDisplayNameFromVehicleModel(vehicle.model), "OptionVehicle", {vehicle,nil}, "GARAGE: "..currentGarage.."", " Engine : " .. round(vehicle.engineHealth) /10 .. "%", " Fuel : " .. round(vehicle.fuelLevel) .. "%","SpawnLocalVehicle")
        end
    end
end
...
function OptionVehicle(data)
   MenuTitle = "Options :"
   ClearMenu()
   Menu.addButton("Spawn", "SpawnVehicle", data)
