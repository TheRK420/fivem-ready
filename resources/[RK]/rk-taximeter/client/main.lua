...
AddEventHandler('rk:playerLoaded', function(xPlayer)
  currentJob = RKCore.GetPlayerData().job.name
end)

RegisterNetEvent('rk:setJob')
AddEventHandler('rk:setJob', function(xPlayer)
  currentJob = RKCore.GetPlayerData().job.name
end)

RegisterNetEvent("rk_taximeter:updatePassenger")
AddEventHandler("rk_taximeter:updatePassenger", function(attributes)
  meterAttrs = attributes
  meterOwner = false
  updateMeter()
end)

RegisterNetEvent("rk_taximeter:updateLocation")
AddEventHandler("rk_taximeter:updateLocation", function()
  lastLocation = GetEntityCoords(GetPlayerPed(-1))
end)



RegisterNetEvent("rk_taximeter:resetMeter")
AddEventHandler("rk_taximeter:resetMeter", function()
  resetMeter()
  updateMeter()
end)
...
function hasMeterAppropriateJob()
  if not Config.RestrictToCertainRKCoreJobs then
    return true
  end
...
function updatePassengerMeters()
  ...
  for index,player in ipairs(players) do
    ...
    if not has_value(playersInVehicle, playerId) then
      TriggerServerEvent('rk_taximeter:updatePassengerMeters', playerId, meterAttrs)
...
function resetMeter()
  ...
  for i, player in ipairs(playersInVehicle) do
    TriggerServerEvent('rk_taximeter:resetPassengerMeters', player)
  end
end

function updatePassengerLocations()
  for i, player in ipairs(playersInVehicle) do
    TriggerServerEvent('rk_taximeter:updatePassengerLocation', player)
  end
end
