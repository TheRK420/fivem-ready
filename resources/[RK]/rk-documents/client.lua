...
Citizen.CreateThread(function()
    ...
    while RKCore.IsPlayerLoaded == false do
        Citizen.Wait(10)
    end
...
function CopyFormToPlayer(aPlayer)
    --TriggerServerEvent('rk_documents:CopyToPlayer', GetPlayerServerId(player), aDocument)
    TriggerServerEvent('rk_documents:CopyToPlayer', aPlayer, CURRENT_DOCUMENT)
...
function DeleteDocument(aDocument)
    ...
    RKCore.TriggerServerCallback('rk_documents:deleteDocument', function (cb)
        if cb == true then
            --remove form_close
...
function CreateNewForm(aDocument)
    ...
    PlayerData = RKCore.GetPlayerData()
    RKCore.TriggerServerCallback('rk_documents:getPlayerDetails', function (cb_player_details)
...
function ShowDocument(aPlayer)
     --   print("ssss: " .. dump(aPlayer))
        TriggerServerEvent('rk_documents:ShowToPlayer', aPlayer, CURRENT_DOCUMENT)
        CURRENT_DOCUMENT = nil
        CloseMenu()
end

RegisterNetEvent('rk_documents:viewDocument')
AddEventHandler('rk_documents:viewDocument', function( data )
...
RegisterNetEvent('rk_documents:copyForm')
AddEventHandler('rk_documents:copyForm', function( data )
...
function GetAllUserForms()
    ...
    RKCore.TriggerServerCallback('rk_documents:getPlayerDocuments', function (cb_forms)
...
RegisterNUICallback('form_submit', function(data, cb)
   -- print("received: " .. dump(data))
    CloseMenu()
    RKCore.TriggerServerCallback('rk_documents:submitDocument', function (cb_form)
        if cb_form ~= nil then
...
function GetNeareastPlayers()
    ...
    local players, nearbyPlayer = RKCore.Game.GetPlayersInArea(GetEntityCoords(playerPed), 3.0)
