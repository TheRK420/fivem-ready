...
RegisterNetEvent('rk-garages:client:houseGarageConfig')
AddEventHandler('rk-garages:client:houseGarageConfig', function(garageConfig)
...
AddEventHandler('playerSpawned', function(spawn)
	if not hasAlreadyJoined then
		TriggerServerEvent('rk-garage:putcaringarage')
	end
...
function RentrerVehicule()
	...
	--TriggerServerEvent('rk-garage:CheckForVeh')
...
function ListeVehicule(page)
	...
	ClearMenu()
	local carCount = 0
	RKCore.TriggerServerCallback('rk-garage:getOwnedCars', function(ownedCars)
		   ...
		   local vehicle_state = value.state
			...
			--if garages[selectedGarage].garage == curGarage then
					...
					if vehicle_state == "Standard Impound" then
						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, "Impounded: $500", " Engine %:" .. RKCore.Math.Round(enginePercent,1) .. "", " Body %:" .. RKCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					elseif vehicle_state == "Police Impound" then
						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, "Impounded: $1500", " Engine %:" .. RKCore.Math.Round(enginePercent,1) .. ""," Body %:" .. RKCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					elseif curGarage ~= current_used_garage and curGarage ~= "Any" and vehicle_state ~= "Out" and curGarage ~= "house" then
						Menu.addButton(tostring(vehicleName), "OptionVehicle", 0, curGarage, " Engine %:" .. RKCore.Math.Round(enginePercent,1) .. "", " Body %:" .. RKCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					--elseif currentGarage == ""
					else

						Menu.addButton(tostring(vehicleName), "OptionVehicle", value.id, tostring(value.state) , " Engine %:" .. RKCore.Math.Round(enginePercent,1) .. ""," Body %:" .. RKCore.Math.Round(bodyPercent,1) .. "", "Plate: "..plate.."")
					end
				end
			--end
		   count = count + 1
...
function SpawnVehicle(vehicle, plate, fuelSet, spawnx, spawny, spawx, spawnz)
	...
	TriggerServerEvent('rk-advancedgarage:setVehicleState', plate, false)
end
...
AddEventHandler('garages:StoreVehicle', function()
    ...
    if vehicle == 0 then
		...
		if enableHgarage then
			...
			--TriggerServerEvent('drp-garage:modifystate', vehicleProps, 1, garage)
