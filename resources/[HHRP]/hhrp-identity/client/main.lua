...
RegisterNetEvent('hhrp-identity:showRegisterIdentity')
AddEventHandler('hhrp-identity:showRegisterIdentity', function()
	if not isDead then
		EnableGui(true)
	end
end)
...
RegisterNetEvent('hhrp-identity:saveID')
AddEventHandler('hhrp-identity:saveID', function(data)
	myIdentifiers = data
end)
...
RegisterNUICallback('register', function(data, cb)
    ...
    if reason == "" then
        ...
        DoScreenFadeIn(1000)
        TriggerServerEvent('hhrp-identity:setIdentity', data, myIdentifiers)
        EnableGui(false)
		TriggerEvent('hhrp-clothing:defaultReset', myIdentifiers.id)
