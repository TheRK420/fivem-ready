...
RegisterServerEvent('hhrp-identity:setIdentity')
AddEventHandler('hhrp-identity:setIdentity', function(data, myIdentifiers)
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			TriggerClientEvent('hhrp-identity:identityCheck', myIdentifiers.playerid, true)
		else
...
AddEventHandler('es:playerLoaded', function(source)
	...
	TriggerClientEvent('hhrp-identity:saveID', source, myID)
	getIdentity(source, function(data)
		if data.firstname == '' then
			TriggerClientEvent('hhrp-identity:identityCheck', source, false)
			TriggerClientEvent('hhrp-identity:showRegisterIdentity', source)
		else
			TriggerClientEvent('hhrp-identity:identityCheck', source, true)
		end
	end)
end)
...
AddEventHandler('onResourceStart', function(resource)
	if resource == GetCurrentResourceName() then
		...
		local xPlayers, xPlayer = HHCore.GetPlayers()
		for i=1, #xPlayers, 1 do
			xPlayer = HHCore.GetPlayerFromId(xPlayers[i])
			...
			TriggerClientEvent('hhrp-identity:saveID', xPlayer.source, myID)

			getIdentity(xPlayer.source, function(data)
				if data.firstname == '' then
					TriggerClientEvent('hhrp-identity:identityCheck', xPlayer.source, false)
					TriggerClientEvent('hhrp-identity:showRegisterIdentity', xPlayer.source)
				else
					TriggerClientEvent('hhrp-identity:identityCheck', xPlayer.source, true)
				end
			end)
		end
	end
