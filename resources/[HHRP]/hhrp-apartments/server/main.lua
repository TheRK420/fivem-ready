...
HHCore.RegisterServerCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
HHCore.RegisterServerCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
HHCore.RegisterServerCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
HHCore.RegisterServerCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
HHCore.RegisterServerCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = HHCore.GetPlayerFromId(src)
    if Player ~= nil then
        MySQL.Async.fetchAll("SELECT * FROM apartments WHERE  identifier = @identifier",{['@identifier'] = Player.identifier}, function(result)
...
HHCore.RegisterServerCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = HHCore.GetPlayerFromId(src)
