            ...
            if InApartment then
                ...
                if IsOwned then
                    ...
                    if(GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Apartments.Locations[ClosestHouse].coords.enter.x, Apartments.Locations[ClosestHouse].coords.enter.y,Apartments.Locations[ClosestHouse].coords.enter.z, true) < 1.2)then
                        ...
                        if IsControlJustPressed(0, Keys["E"]) then
                            HHCore.TriggerServerCallback('apartments:GetOwnedApartment', function(result)
                                if result ~= nil then
...
function apartal()
	...
	TriggerEvent("hhrp-uimenu:createMenu", myListMenu, function(data)
...
RegisterNetEvent('hhrp-apartments:client:LastLocationHouse')
AddEventHandler('hhrp-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
...
function EnterApartment(house, apartmentId, new)
    ...
    openHouseAnim()
    Citizen.Wait(250)
    HHCore.TriggerServerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            HHCore.TriggerServerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                data = exports['hhrp-interior']:CreateApartmentFurnished(coords)
                Citizen.Wait(100)
                houseObj = data[1]
                POIOffsets = data[2]
        ...
        if new ~= nil then
            if new then
                TriggerEvent('hhrp-interior:client:SetNewState', true)
            else
                TriggerEvent('hhrp-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('hhrp-interior:client:SetNewState', false)
        end
...
function SetClosestApartment()
    ...
    if current ~= ClosestHouse  and not InApartment then
        ...
        HHCore.TriggerServerCallback('apartments:IsOwner', function(result)
            IsOwned = result
        end, ClosestHouse)
    end
end
...
function OwnerList()
    HHCore.TriggerServerCallback('apartments:GetAvailableApartments', function(apartments)
        ped = GetPlayerPed(-1);
...
function OutfitsLijst()
    HHCore.TriggerServerCallback('apartments:GetOutfits', function(outfits)
        ped = GetPlayerPed(-1);
        MenuTitle = "Clothing :"
        ClearMenu()
