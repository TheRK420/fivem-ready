...
$('.create-button').click(function () {
    ...
    $.post("http://hhrp-characters/CharacterChosen", JSON.stringify({
...
$("#deletechar").click(function () {
	...
	if (chosenCharacterId > 0) {
		$.post("http://hhrp-characters/DeleteCharacter", JSON.stringify({
			charid: Number(chosenCharacterId),
		}));
    }
    ...
    window.onload = function(e) {
        ...
        document.onkeydown = function(data) {
            if (data.which == 13 && IsInMainMenu) {
                ...
                $.post("http://hhrp-characters/ShowSelection", JSON.stringify({}));
                ShowLoading(true)
            }
        }
    }
...
function disconnect(){
    $.post("http://hhrp-characters/DisconnectGame", JSON.stringify({
    }));
