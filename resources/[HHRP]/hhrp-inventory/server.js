...
let hasBrought = [];
let CheckedDeginv = [];
const SHOPS_ACCOUNT_ID = 4;
const SHOPS_TAX_CATEGORY = 2;
...
onNet("onResourceStart", async (resource) => {
    if (resource == GetCurrentResourceName()){
        emit("server-request-update",source);
        emit("server-update-item",source)
        emitNet("playerSpawned",source);
        DeleteDropInRestart()
    }
});

function DeleteDropInRestart() {
    let string = `DELETE FROM user_inventory2 WHERE name LIKE 'Drop-%'`
...
function functionRemoveAny(player, itemidsent, amount, openedInv,pId) {
    ...
    exports.ghmattimysql.execute(string,{},function() {
        emit("server-request-update",player,src)
...
onNet("server-request-update", async (player) => {
    ////console.log('server update me')
    let src = source
    let playerinvname = player
    let string = `SELECT count(item_id) as amount, id, item_id, name, information, slot, quality, dropped FROM user_inventory2 WHERE name ='${playerinvname}' group by item_id`;// slot
    exports.ghmattimysql.execute(string, {}, function(inventory) {
...
onNet("server-request-update-src", async (player,src) => {
    let playerinvname = player
    let string = `SELECT count(item_id) as amount, item_id, id, name, information, slot, dropped, creationDate, MIN(creationDate) as creadtionDate FROM user_inventory2 WHERE name = '${playerinvname}' group by item_id`; // slot
    exports.ghmattimysql.execute(string, {}, function(inventory) {
    emitNet("inventory-update-player", src, [inventory,0,playerinvname]);
    });
});
...
onNet("server-inventory-give", async (player, itemid, slot, amount, generateInformation, itemdata, openedInv) => {
        // print(itemid, generateInformation)
        let src = source
        let playerinvname =  player
        let information = "{}"
...
onNet("sendingItemstoClient", async (player, sauce) => {
    ////print('open this or appear afet close')
    let src = source
    if (!src) {
        src = sauce 
    }
        ...
        exports.ghmattimysql.execute(string,{}, function(inventory) {
            ...
            if (inventory == 0){
              //  print('no value')
                ...
                // print('i have value and name')
            var invArray = inventory;
            var i;
            var arrayCount = 0;
            var playerinvname = inventory[0].name
            var invArrayTarget = [];
            let Key = ""+DataEntries+"";
...
onNet("SniffCID", async (cid,src) => {
    ...
    let name = cid;
    let string = `SELECT count(item_id) as amount, id, item_id, name, information, slot, quality, dropped FROM user_inventory2 WHERE name='${name}' and item_id IN (${IllegalSearchString}) group by item_id`;
...
onNet("inv:delete", async (player) => {
	var playerinvname = player
    db(`DELETE FROM user_inventory2 WHERE name='${playerinvname}'`);
});
 ...
 onNet("server-inventory-swap", async (player, data, coords) => {
    ...
    let startname = data[3].replace(/"/g, "");
     ...
     exports.ghmattimysql.execute(string,{}, function(startid) {
         ...
         exports.ghmattimysql.execute(string,{}, function(inventory){
             if (startname.indexOf("Drop") > -1 || startname.indexOf("hidden")){
                //  print("EWAN")
...
onNet("server-frisk-player-inventory", (searcher,cash,  DirtyMoney, inventoryName) => {
    let weapons = 0
    let string = `SELECT count(item_id) as amount, id, item_id, id, name, information, slot, quality, dropped FROM user_inventory2 WHERE name = '${inventoryName}' group by item_id, slot`;
        ...
        exports.ghmattimysql.execute(string,{}, function(inventory) {
            ...
            emitNet('chatmessage', searcher, '', 3, "Frisk: Cannot Seem to feel anything.")
        }
        });
});
...
function HardwareStore() {
    var shopItems = [
        { item_id: "oxygentank", id: 0, name: "Shop", information: "{}", slot: 1, amount: 1 },
        { item_id: "armor", id: 0, name: "Shop", information: "{}", slot: 2, amount: 5 },
        { item_id: "radio", id: 0, name: "Shop", information: "{}", slot: 3, amount: 5 },
        { item_id: "watch", id: 0, name: "Shop", information: "{}", slot: 4, amount: 5 },
        { item_id: "Suitcase", id: 0, name: "Shop", information: "{}", slot: 5, amount: 5 },
        { item_id: "Boombox", id: 0, name: "Shop", information: "{}", slot: 6, amount: 5 },
...
function CoffeeShop() {
    var shopItems = [
        ...
        { item_id: "mshake", id: 0, name: "Craft", information: "{}", slot: 2, amount: 5 },
        { item_id: "icecream", id: 0, name: "Craft", information: "{}", slot: 3, amount: 5 },
        { item_id: "cola", id: 0, name: "Craft", information: "{}", slot: 4, amount: 5 },
        { item_id: "fries", id: 0, name: "Craft", information: "{}", slot: 5, amount: 5 },
