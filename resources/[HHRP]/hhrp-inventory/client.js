		...
		if (GetVehicleDoorLockStatus(vehicleFound) == 2 && distanceRear < 1.5) {
			...
			if (distanceRear > 1.5) {
				...
				if(licensePlate != null){
					...
					if (GetEntityModel(vehicleFound) == GetHashKey('nptaco') && foodtruck) {
						TriggerEvent("server-inventory-open", "18", "Craft");
					} else {
...
RegisterNetEvent('nui-myinventory')
on('nui-myinventory', (toggle) => {
...
function PopulateGuiSingle(playerinventory,itemCount,invName) {
	let ServerId = "ply-"+GetPlayerPed(-1);
