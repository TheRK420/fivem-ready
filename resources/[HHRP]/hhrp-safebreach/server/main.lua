...
AddEventHandler("safebreach:globalEvent", function(options)
    -- HHCore.Trace((options["event"] or "none") .. " triggered to all clients.")
...
HHCore.RegisterServerCallback("safebreach:checkSafeBreaches", function(source, cb)
...
HHCore.RegisterServerCallback("safebreach:checkIfSafeIsBreachable", function(source, cb, safe)
	local player = HHCore.GetPlayerFromId(source)
	...
	if player then
		if cachedSafes[safe] then
			...
			for i = 1, #players do
				local player = HHCore.GetPlayerFromId(players[i])
...
HHCore.RegisterServerCallback("safebreach:safeBreached", function(source, cb, safeData)
	local player = HHCore.GetPlayerFromId(source)
...
HHCore.RegisterServerCallback("safebreach:receiveReward", function(source, cb, reward)
	local player = HHCore.GetPlayerFromId(source)
