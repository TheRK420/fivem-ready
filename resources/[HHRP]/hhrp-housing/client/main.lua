...
local servername = 'hhrp Development'
local cam = nil
local viewCam = false
...
RegisterNetEvent('hhrp-housing:client:sellHouse')
AddEventHandler('hhrp-housing:client:sellHouse', function()
    if closesthouse ~= nil and hasKey then
        TriggerServerEvent('hhrp-housing:server:viewHouse', closesthouse)
    end
end)
...
AddEventHandler('hhrp:playerLoaded', function()
    TriggerServerEvent('hhrp-housing:client:setHouses')
    isLoggedIn = true
    SetClosestHouse()
    TriggerEvent('hhrp-housing:client:setupHouseBlips')
    Citizen.Wait(100)
    TriggerServerEvent("hhrp-housing:server:setHouses")
end)

RegisterNetEvent('hhrp-housing:client:setHouseConfig')
AddEventHandler('hhrp-housing:client:setHouseConfig', function(houseConfig)
    Config.Houses = houseConfig
    --TriggerEvent("hhrp-housing:client:refreshHouse")
	
end)

RegisterNetEvent('hhrp-housing:client:lockHouse')
AddEventHandler('hhrp-housing:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('hhrp-housing:client:createHouses')
AddEventHandler('hhrp-housing:client:createHouses', function(price, tier)
    ...
    TriggerServerEvent('hhrp-housing:server:addNewHouse', area, coords, price, tier)
end)

RegisterNetEvent('hhrp-housing:client:toggleDoorlock')
AddEventHandler('hhrp-housing:client:toggleDoorlock', function()
    ...
    if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
        if hasKey then
            if Config.Houses[closesthouse].locked then
                TriggerServerEvent('hhrp-housing:server:lockHouse', false, closesthouse)
				TriggerEvent('DoLongHudText', 'The Door is unlocked.', 1)
            else
                TriggerServerEvent('hhrp-housing:server:lockHouse', true, closesthouse)
                    ...
                    if CurrentDoorBell ~= 0 then
                        if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                            ...
                            if IsControlJustPressed(0, Keys["G"]) then
                                TriggerServerEvent("hhrp-housing:server:OpenDoor", CurrentDoorBell, closesthouse)
                                CurrentDoorBell = 0
                            end
                        end
                    end
                            ...
                            if not inOwned then
                                if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
                                    if not Config.Houses[closesthouse].locked then
                                        ...
                                        if IsControlJustPressed(0, Keys["G"]) then
                                            TriggerServerEvent('hhrp-housing:server:RingDoor', closesthouse)
                                        end
                                    end
                                end
                            elseif inOwned then
                    ...
                    if closesthouse ~= nil then
                        if logoutLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, logoutLocation.x, logoutLocation.y, logoutLocation.z, true) < 1.5)then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerEvent('hhrp-login:swapcharacter')
                                    leaveOwnedHouse(closesthouse)
                                    showClothing = false
                                end
...
RegisterNetEvent('hhrp-housing:client:RingDoor')
AddEventHandler('hhrp-housing:client:RingDoor', function(player, house)
...
RegisterNetEvent('hhrp-housing:client:giveHouseKey')
AddEventHandler('hhrp-housing:client:giveHouseKey', function(data)
    ...
    if player ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('hhrp-housing:server:giveHouseKey', playerId, closesthouse)
        else
...
RegisterNetEvent('hhrp-housing:client:removeHouseKey')
AddEventHandler('hhrp-housing:client:removeHouseKey', function(data)
    if closesthouse ~= nil then 
        local housedist = GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z)
        if housedist < 5 then
            HHCore.TriggerServerCallback('hhrp-housing:server:getHouseOwner', function(result)
                if HHCore.GetPlayerData().identifier == result then
                    inHoldersMenu = true
                    HouseKeysMenu()
...
RegisterNetEvent('hhrp-housing:client:refreshHouse')
AddEventHandler('hhrp-housing:client:refreshHouse', function(data)
    Citizen.Wait(100)
    SetClosestHouse()
end)

RegisterNetEvent('hhrp-housing:client:setstash')
AddEventHandler('hhrp-housing:client:setstash', function(data)
ExecuteCommand("setlocation stash")
end)

RegisterNetEvent('hhrp-housing:client:setstash2')
AddEventHandler('hhrp-housing:client:setstash2', function(data)
ExecuteCommand("setlocation stash2")
end)

RegisterNetEvent('hhrp-housing:client:setoutfit')
AddEventHandler('hhrp-housing:client:setoutfit', function(data)
ExecuteCommand("setlocation outfit")
end)

RegisterNetEvent('hhrp-housing:client:setlogout')
AddEventHandler('hhrp-housing:client:setlogout', function(data)
ExecuteCommand("setlocation logout")
end)

RegisterNetEvent('hhrp-housing:client:SpawnInApartment')
AddEventHandler('hhrp-housing:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(GetPlayerPed(-1))
...
function removeHouseKey(citizenData)
    TriggerServerEvent('hhrp-housing:server:removeHouseKey', closesthouse, citizenData)
    closeMenuFull()
end
...
function enterOwnedHouse(house)
    ...
    if Config.Houses[house].tier == 1 then
        data = exports['hhrp-interior']:CreateTier1House(coords)
	elseif Config.Houses[house].tier == 2 then
        data = exports['hhrp-interior']:CreateMichaelShell(coords)
	elseif Config.Houses[house].tier == 3 then
        data = exports['hhrp-interior']:CreateTrevorsShell(coords)
	elseif Config.Houses[house].tier == 4 then
        data = exports['hhrp-interior']:CreateFranklinShell(coords)
	elseif Config.Houses[house].tier == 5 then
        data = exports['hhrp-interior']:CreateFranklinAuntShell(coords)
	elseif Config.Houses[house].tier == 6 then
        data = exports['hhrp-interior']:CreateApartmentShell(coords)
	elseif Config.Houses[house].tier == 7 then
        data = exports['hhrp-interior']:CreateCaravanShell(coords)
    end
    Citizen.Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
    ...
    TriggerEvent('hhrp-weathersync:client:DisableSync')
    TriggerEvent('hhrp-weed:client:getHousePlants', house)
    Citizen.Wait(100)
...
RegisterNetEvent('hhrp-housing:client:enterOwnedHouse')
AddEventHandler('hhrp-housing:client:enterOwnedHouse', function(house)
	enterOwnedHouse(house)
end)

RegisterNUICallback('HasEnoughMoney', function(data, cb)
    HHCore.TriggerServerCallback('hhrp-housing:server:HasEnoughMoney', function(hasEnough)
        
    end, data.objectData)
end)

RegisterNetEvent('hhrp-housing:client:LastLocationHouse')
AddEventHandler('hhrp-housing:client:LastLocationHouse', function(houseId)
...
function leaveOwnedHouse(house)
    ...
    DoScreenFadeOut(250)
    Citizen.Wait(500)
    exports['hhrp-interior']:DespawnInterior(houseObj, function()
        UnloadDecorations()
        TriggerEvent('hhrp-weathersync:client:EnableSync')
        Citizen.Wait(250)
        DoScreenFadeIn(250)
        ...
        inside = false
        TriggerEvent('hhrp-weed:client:leaveHouse')
        showClothing = false
    end)
end
...
function enterNonOwnedHouse(house)
    ...
    Citizen.Wait(500)
    SetRainFxIntensity(0.0)
    TriggerServerEvent('hhrp-housing:server:SetInsideMeta', house, true)
    TriggerEvent('hhrp-weathersync:client:DisableSync')
    TriggerEvent('hhrp-weed:client:getHousePlants', house)
    Citizen.Wait(100)
    --SetWeatherTypePersist('CLEAR')
...
function leaveNonOwnedHouse(house)
    ...
    exports['hhrp-interior']:DespawnInterior(houseObj, function()
        ...
        showClothing = false
        TriggerEvent('hhrp-weed:client:leaveHouse')
        TriggerServerEvent('hhrp-housing:server:SetInsideMeta', house, false)
    end)
end
...
function HouseKeysMenu()
    ...
    MenuTitle = "Keys"
    ClearMenu()
    HHCore.TriggerServerCallback('hhrp-housing:server:getHouseKeyHolders', function(holders)
        ped = GetPlayerPed(-1);
...
RegisterNetEvent('hhrp-housing:client:setupHouseBlips')
AddEventHandler('hhrp-housing:client:setupHouseBlips', function()
    Citizen.CreateThread(function()
        Citizen.Wait(2000)
        if isLoggedIn then
            HHCore.TriggerServerCallback('hhrp-housing:server:getOwnedHouses', function(ownedHouses)
...
RegisterNetEvent('hhrp-housing:client:SetClosestHouse')
AddEventHandler('hhrp-housing:client:SetClosestHouse', function()
    SetClosestHouse()
end)
...
RegisterNUICallback('buy', function()
    openContract(false)
    disableViewCam()
    TriggerServerEvent('hhrp-housing:server:buyHouse', closesthouse)
end)
...
RegisterNetEvent('hhrp-housing:client:viewHouse')
AddEventHandler('hhrp-housing:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
function SetClosestHouse()
    ...
    if not inside then
        ...
        if closesthouse ~= nil then 
            HHCore.TriggerServerCallback('hhrp-housing:server:hasKey', function(result)
                hasKey = result
            end, closesthouse)
        
            HHCore.TriggerServerCallback('hhrp-housing:server:isOwned', function(result)
                isOwned = result
            end, closesthouse)
        end
    end
...
function setHouseLocations()
    if closesthouse ~= nil then
        HHCore.TriggerServerCallback('hhrp-housing:server:getHouseLocations', function(result)
            if result ~= nil then
...
RegisterNetEvent('hhrp-housing:client:setLocation')
AddEventHandler('hhrp-housing:client:setLocation', function(data)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    ...
    if inside then
        if hasKey then
            if data == 'stash' then
                TriggerServerEvent('hhrp-housing:server:setLocation', coords, closesthouse, 1)
            elseif data == 'outift' then
                TriggerServerEvent('hhrp-housing:server:setLocation', coords, closesthouse, 2)
            elseif data == 'logout' then
                TriggerServerEvent('hhrp-housing:server:setLocation', coords, closesthouse, 3)
			elseif data == 'stash2' then
                TriggerServerEvent('hhrp-housing:server:setLocation', coords, closesthouse, 4)
            end
        else
			TriggerEvent('DoLongHudText', 'You dont have keys to this house', 2)
        end
    elseif data == 'garage' then
        TriggerServerEvent('hhrp-housing:server:setLocation', coords, closesthouse, 5)
    else
		TriggerEvent('DoLongHudText', 'You are not inside any house', 2)
    end
end)

RegisterNetEvent('hhrp-housing:client:refreshLocations')
AddEventHandler('hhrp-housing:client:refreshLocations', function(house, location, type)
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)

    if closesthouse == house then
        if inside then
            TriggerEvent('hhrp-housing:getid', closesthouse, house)
            if type == 1 then
            ...
            --elseif type == 5 then
                --garageLocation = json.decode(location)
            end
        end
    end
end)
