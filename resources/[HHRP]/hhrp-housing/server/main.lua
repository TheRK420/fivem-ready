...
Citizen.CreateThread(function()
	MySQL.Async.fetchAll("SELECT * FROM `houselocations`", {}, function(result)
		...
		TriggerClientEvent("hhrp-housing:client:setHouseConfig", -1, Config.Houses)
	end)
end)
...
RegisterServerEvent('hhrp-housing:server:setHouses')
AddEventHandler('hhrp-housing:server:setHouses', function()
	local src = source
	TriggerClientEvent("hhrp-housing:client:setHouseConfig", src, Config.Houses)
end)
...
RegisterServerEvent('hhrp-housing:server:addNewHouse')
AddEventHandler('hhrp-housing:server:addNewHouse', function(area, coords, price, tier)
	local src = source
...
RegisterServerEvent('hhrp-housing:server:viewHouse')
AddEventHandler('hhrp-housing:server:viewHouse', function(house)
	local src     		= source
	local pData 		= HHCore.GetPlayerFromId(src)
	...
	MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = @identifier', {
		...
		if pResult[1].firstname ~= nil then 
			TriggerClientEvent('hhrp-housing:client:viewHouse', src, houseprice, brokerfee, bankfee, taxes, pResult[1].firstname, pResult[1].lastname)
		else
			print("^5[hhrp-housing]^0 - ERROR - The player who is trying to buy a house doesn`t have first name")
		end
	end)
end)

RegisterServerEvent('hhrp-housing:server:buyHouse')
AddEventHandler('hhrp-housing:server:buyHouse', function(house)
	local src     	= source
	local pData 	= HHCore.GetPlayerFromId(src)
	...
	if (pData.getAccount('bank').money >= price) then
		...
		TriggerClientEvent('hhrp-housing:client:SetClosestHouse', src)
...
RegisterServerEvent('hhrp-housing:server:lockHouse')
AddEventHandler('hhrp-housing:server:lockHouse', function(bool, house)
	TriggerClientEvent('hhrp-housing:client:lockHouse', -1, bool, house)
end)
...
HHCore.RegisterServerCallback('hhrp-housing:server:hasKey', function(source, cb, house)
	local src = source
	local xPlayer = HHCore.GetPlayerFromId(src)
	local Player = HHCore.GetPlayerFromId(src)
	local retval = false
	if Player ~= nil then 
...
HHCore.RegisterServerCallback('hhrp-housing:server:isOwned', function(source, cb, house)
...
HHCore.RegisterServerCallback('hhrp-housing:server:getHouseOwner', function(source, cb, house)
    local identifier = GetPlayerIdentifiers(source)[1]
	cb(houseowneridentifier[house])
end)

HHCore.RegisterServerCallback('hhrp-housing:server:getHouseKeyHolders', function(source, cb, house)
	local retval = {}
...
HHCore.RegisterServerCallback('hhrp-phone:server:GetPlayerHouses', function(source, cb)
	local src = source
	local Player = HHCore.GetPlayerFromId(src)
	local MyHouses = {}
	local keyholders = {}
...
RegisterServerEvent('hhrp-housing:server:giveKey')
AddEventHandler('hhrp-housing:server:giveKey', function(house, target)
	local pData = HHCore.GetPlayerFromId(target)
...
RegisterServerEvent('hhrp-housing:server:removeHouseKey')
AddEventHandler('hhrp-housing:server:removeHouseKey', function(house, citizenData)
	local src = source
	local pSteam = GetPlayerIdentifier(src, 0)
	local newHolders = {}
	if housekeyholders[house] ~= nil then 
...
RegisterServerEvent('hhrp-housing:server:OpenDoor')
AddEventHandler('hhrp-housing:server:OpenDoor', function(target, house)
    local src = source
    local OtherPlayer = HHCore.GetPlayerFromId(target)
    if OtherPlayer ~= nil then
        TriggerClientEvent('hhrp-housing:client:SpawnInApartment', OtherPlayer.source, house)
    end
end)

RegisterServerEvent('hhrp-housing:server:RingDoor')
AddEventHandler('hhrp-housing:server:RingDoor', function(house)
    local src = source
    TriggerClientEvent('hhrp-housing:client:RingDoor', -1, src, house)
end)

RegisterServerEvent('hhrp-housing:server:savedecorations')
AddEventHandler('hhrp-housing:server:savedecorations', function(house, decorations)
	local src = source
	MySQL.Async.execute("UPDATE `player_houses` SET `decorations` = '"..json.encode(decorations).."' WHERE `house` = '"..house.."'")
	TriggerClientEvent("hhrp-housing:server:sethousedecorations", -1, house, decorations)
end)

HHCore.RegisterServerCallback('hhrp-housing:server:getHouseDecorations', function(source, cb, house)
	local retval = nil
...
HHCore.RegisterServerCallback('hhrp-housing:server:getHouseLocations', function(source, cb, house)
	local retval = nil
	MySQL.Async.fetchAll("SELECT * FROM `player_houses` WHERE `house` = '"..house.."'", {}, function(result)
		if result[1] ~= nil then
			retval = result[1]
...
HHCore.RegisterServerCallback('hhrp-phone:server:GetHouseKeys', function(source, cb)
	local src = source
	local pData = HHCore.GetPlayerFromId(src)
end)

HHCore.RegisterServerCallback('hhrp-housing:server:getOwnedHouses', function(source, cb)
	local src = source
	local pData = HHCore.GetPlayerFromId(src)
...
HHCore.RegisterServerCallback('hhrp-housing:server:getSavedOutfits', function(source, cb)
	local src = source
	local pData = HHCore.GetPlayerFromId(src)
...
RegisterCommand('decorate', function(source, args, rawCommand)
	TriggerClientEvent("hhrp-housing:client:decorate", source)
end)
...
RegisterServerEvent('hhrp-housing:server:LogoutLocation')
AddEventHandler('hhrp-housing:server:LogoutLocation', function()
	local src = source
...
RegisterServerEvent('hhrp-housing:server:giveHouseKey')
AddEventHandler('hhrp-housing:server:giveHouseKey', function(target, house)
	local src = source
...
RegisterServerEvent('hhrp-housing:server:logoutServer')
AddEventHandler('hhrp-housing:server:logoutServer', function(message)
	DropPlayer(source, message)
end)

RegisterServerEvent('hhrp-housing:server:setLocation')
AddEventHandler('hhrp-housing:server:setLocation', function(coords, house, type)
	local src = source
	local Player = HHCore.GetPlayerFromId(src)
