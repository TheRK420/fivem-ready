...
Citizen.CreateThread(function()
    ...
    while HHCore.IsPlayerLoaded == false do
        Citizen.Wait(10)
    end
...
function CopyFormToPlayer(aPlayer)
    --TriggerServerEvent('hhrp_documents:CopyToPlayer', GetPlayerServerId(player), aDocument)
    TriggerServerEvent('hhrp_documents:CopyToPlayer', aPlayer, CURRENT_DOCUMENT)
...
function DeleteDocument(aDocument)
    ...
    HHCore.TriggerServerCallback('hhrp_documents:deleteDocument', function (cb)
        if cb == true then
            --remove form_close
...
function CreateNewForm(aDocument)
    ...
    PlayerData = HHCore.GetPlayerData()
    HHCore.TriggerServerCallback('hhrp_documents:getPlayerDetails', function (cb_player_details)
...
function ShowDocument(aPlayer)
     --   print("ssss: " .. dump(aPlayer))
        TriggerServerEvent('hhrp_documents:ShowToPlayer', aPlayer, CURRENT_DOCUMENT)
        CURRENT_DOCUMENT = nil
        CloseMenu()
end

RegisterNetEvent('hhrp_documents:viewDocument')
AddEventHandler('hhrp_documents:viewDocument', function( data )
...
RegisterNetEvent('hhrp_documents:copyForm')
AddEventHandler('hhrp_documents:copyForm', function( data )
...
function GetAllUserForms()
    ...
    HHCore.TriggerServerCallback('hhrp_documents:getPlayerDocuments', function (cb_forms)
...
RegisterNUICallback('form_submit', function(data, cb)
   -- print("received: " .. dump(data))
    CloseMenu()
    HHCore.TriggerServerCallback('hhrp_documents:submitDocument', function (cb_form)
        if cb_form ~= nil then
...
function GetNeareastPlayers()
    ...
    local players, nearbyPlayer = HHCore.Game.GetPlayersInArea(GetEntityCoords(playerPed), 3.0)
