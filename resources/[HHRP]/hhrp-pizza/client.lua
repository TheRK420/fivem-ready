...
Citizen.CreateThread(function()
    while true do
            ...
            if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 3 then
                ...
                if IsControlJustPressed(1,38) then
                    ...
                    if (posibilidad > 70) and (posibilidad < 90) then
                        ...
                        TriggerServerEvent("hhrp-pizza:cash", cash)
                    end
                    isToHouse = false
                    isToPizzaria = true
        ...
        if isToPizzaria == true then
            drawTxt("A payslip of $" .. paga .. ' is ready for you back at the Pizzeria',0,1,0.5,0.8,0.35,255,255,255,255)
                ...
                if IsControlJustPressed(1,38) then
                    if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("panto"))  then
                        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("panto")) then
                            ...
                            TriggerServerEvent("hhrp-pizza:cash", paga)
                            isToHouse = false
                            isToPizzaria = false
                            isInJobPizz = false
                            paga = 0
