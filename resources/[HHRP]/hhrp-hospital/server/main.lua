...
RegisterServerEvent('hhrp-hospital:server:RequestBed')
AddEventHandler('hhrp-hospital:server:RequestBed', function()
    for k, v in pairs(beds) do
        if not v.taken then
            v.taken = true
            bedsTaken[source] = k
            TriggerClientEvent('hhrp-hospital:client:SendToBed', source, k, v)
            return
        end
    end
...
RegisterServerEvent('hhrp-hospital:server:RPRequestBed')
AddEventHandler('hhrp-hospital:server:RPRequestBed', function(plyCoords)
    ...
    for k, v in pairs(beds) do
        ...
        if distance < 3.0 then
            if not v.taken then
                v.taken = true
                foundbed = true
                TriggerClientEvent('hhrp-hospital:client:RPSendToBed', source, k, v)
                return
            else
...
RegisterServerEvent('hhrp-hospital:server:EnteredBed')
AddEventHandler('hhrp-hospital:server:EnteredBed', function()
    local src = source
    ...
    TriggerClientEvent('hhrp-hospital:client:FinishServices', src)
end)

RegisterServerEvent('hhrp-hospital:server:LeaveBed')
AddEventHandler('hhrp-hospital:server:LeaveBed', function(id)
    beds[id].taken = false
end)