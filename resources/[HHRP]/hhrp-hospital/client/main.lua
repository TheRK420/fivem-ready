...
function LeaveBed()
    ...
    TriggerServerEvent('hhrp-hospital:server:LeaveBed', bedOccupying)
...
RegisterNetEvent('hhrp-hospital:client:RPCheckPos')
AddEventHandler('hhrp-hospital:client:RPCheckPos', function()
    TriggerServerEvent('hhrp-hospital:server:RPRequestBed', GetEntityCoords(PlayerPedId()))
end)

RegisterNetEvent('hhrp-hospital:client:RPSendToBed')
AddEventHandler('hhrp-hospital:client:RPSendToBed', function(id, data)
    bedOccupying = id
...
RegisterNetEvent('hhrp-hospital:client:SendToBed')
AddEventHandler('hhrp-hospital:client:SendToBed', function(id, data)
    bedOccupying = id
    bedOccupyingData = data
    ...
    Citizen.CreateThread(function ()
        ...
        TriggerServerEvent('hhrp-hospital:server:EnteredBed')
    end)
end)

RegisterNetEvent('hhrp-hospital:client:FinishServices')
AddEventHandler('hhrp-hospital:client:FinishServices', function()
                ...
                if hspDist < 3 then
                    ...
                    if IsControlJustReleased(0, 54) then
                        if (GetEntityHealth(PlayerPedId()) < 201) or (IsInjuredOrBleeding()) then
                            ...
                            exports["hhrp-taskbar"]:taskBar(1500, "Checking Credentials")
                                TriggerEvent('hhrp-ambulancejob:revive')
                                ClearPedTasks(PlayerPedId())
                                Citizen.Wait(1000)
                                TriggerServerEvent('hhrp-hospital:server:RequestBed')
                            else
