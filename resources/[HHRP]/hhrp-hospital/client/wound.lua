...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('hhrp-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('hhrp-hospital:client:SyncBleed')
AddEventHandler('hhrp-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('hhrp-hospital:client:FieldTreatLimbs')
AddEventHandler('hhrp-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('hhrp-hospital:client:FieldTreatBleed')
AddEventHandler('hhrp-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('hhrp-hospital:client:ReduceBleed')
AddEventHandler('hhrp-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('hhrp-hospital:client:ResetLimbs')
AddEventHandler('hhrp-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('hhrp-hospital:client:RemoveBleed')
AddEventHandler('hhrp-hospital:client:RemoveBleed', function()
    isBleeding = 0
...
RegisterNetEvent('hhrp-hospital:client:UsePainKiller')
AddEventHandler('hhrp-hospital:client:UsePainKiller', function(tier)
    if tier < 4 then
        onPainKiller = 90 * tier
    end

    TriggerEvent('DoLongHudText', 'You feel the pain subside temporarily', 1)
    ProcessRunStuff(PlayerPedId())
end)

RegisterNetEvent('hhrp-hospital:client:UseAdrenaline')
AddEventHandler('hhrp-hospital:client:UseAdrenaline', function(tier)
    if tier < 4 then
        onDrugs = 180 * tier
    end

    TriggerEvent('DoLongHudText', 'You\'re Able To Ignore Your Body Failing', 1)
    ProcessRunStuff(PlayerPedId())
end)  
        ...
        if bleedTickTimer >= Config.BleedTickRate then
            if not IsEntityDead(player) then
                if isBleeding > 0 then
                    ...
                    --TriggerEvent('DoLongHudText', 'You Have ' .. Config.BleedingStates[isBleeding], 1)
