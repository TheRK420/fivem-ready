...
AddEventHandler('playerSpawned', function()
	...
	if FirstSpawn then
		...
		HHCore.TriggerServerCallback('hhrp-ambulancejob:getDeathStatus', function(isDead)
...
function OnPlayerDeath()
	...
	TriggerServerEvent('hhrp-ambulancejob:setDeathState', true)
...
function releaseBody()
    ...
    HHCore.TriggerServerCallback("hhrp-jail:retrieveTime", function(inJail, newJailTime)
		if inJail then
...
function StartDeathTimer()
	...
	if Config.EarlyRespawnFine then
		HHCore.TriggerServerCallback('hhrp-ambulancejob:checkBalance', function(canPay)
			canPayFine = canPay
		end)
	end

	local earlySpawnTimer = HHCore.Math.Round(Config.EarlyRespawnTimer / 1000)
	local bleedoutTimer = HHCore.Math.Round(Config.BleedoutTimer / 1000)
	...
	Citizen.CreateThread(function()
		...
		while bleedoutTimer > 0 and IsDead do
			...
			elseif Config.EarlyRespawnFine and canPayFine then
				text = text .. _U('respawn_bleedout_fine', HHCore.Math.GroupDigits(Config.EarlyRespawnFineAmount))

				if IsControlPressed(0, Keys['H']) and timeHeld > 60 then
					TriggerServerEvent('hhrp-ambulancejob:payFine')
					RemoveItemsAfterRPDeath()
...
function RemoveItemsAfterRPDeath()
	TriggerServerEvent('hhrp-ambulancejob:setDeathState', false)
	...
	Citizen.CreateThread(function()
		...
		HHCore.TriggerServerCallback('hhrp-ambulancejob:removeItemsAfterRPDeath', function()
			local playerpos = GetEntityCoords( GetPlayerPed(-1) )
				
			HHCore.SetPlayerData('lastPosition', playerpos)
			HHCore.SetPlayerData('loadout', {})
			RespawnPed(PlayerPedId(), playerpos, Config.RespawnPoint.heading)

			TriggerServerEvent('hhrp:updateLastPosition', playerpos)
...
RegisterNetEvent('hhrp-ambulancejob:useItem')
AddEventHandler('hhrp-ambulancejob:useItem', function(itemName)
	HHCore.UI.Menu.CloseAll()
	...
	if itemName == 'medikit' then
		...
		exports["hhrp-taskbar"]:taskBar(10000, "Healing")
	
			TriggerEvent('hhrp-ambulancejob:heal', 'big', true)
			TriggerEvent('hhrp-hospital:client:RemoveBleed')
			TriggerEvent('hhrp-hospital:client:ResetLimbs')
	...
	elseif itemName == 'bandage' then
		...
		exports["hhrp-taskbar"]:taskBar(10000, "Healing")
			...
			TriggerEvent('hhrp-ambulancejob:heal', 'small', true)
			TriggerEvent('hhrp-hospital:client:RemoveBleed')
			ClearPedTasks(playerPed)
	end
end)
...
function RespawnPed(ped, coords, heading)
	...
	TriggerEvent('hhrp-hospital:client:RemoveBleed', ped) 
	TriggerEvent('hhrp-hospital:client:ResetLimbs', ped)
...
RegisterNetEvent('hhrp-ambulancejob:revive')
AddEventHandler('hhrp-ambulancejob:revive', function()
	attemptRevive()
end)
