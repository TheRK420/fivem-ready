...
RegisterNetEvent('hhrp-ambulancejob:drag')
AddEventHandler('hhrp-ambulancejob:drag', function(copID)
...
RegisterCommand('escort', function()
	...
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		TriggerServerEvent('hhrp-ambulancejob:drag', GetPlayerServerId(closestPlayer))
	else
...
function OpenAmbulanceActionsMenu()
	...
	if Config.EnablePlayerManagement and HHCore.PlayerData.job.grade_name == 'boss' then
		table.insert(elements, {label = _U('boss_actions'), value = 'boss_actions'})
	end

	HHCore.UI.Menu.CloseAll()

	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'ambulance_actions', {
		title    = _U('ambulance'),
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('hhrp-society:openBossMenu', 'ambulance', function(data, menu)
				menu.close()
			end, {wash = false})
		end
...
function OpenMobileAmbulanceActionsMenu()
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
		title    = _U('ambulance'),
		align    = 'bottom-right',
		elements = {
		...
		if data.current.value == 'citizen_interaction' then
			HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer == -1 or closestDistance > 2.0 then
					...
					if data.current.value == 'revive' then
						...
						HHCore.TriggerServerCallback('hhrp-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_a", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_b", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_c", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_d", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_e", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_f", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_g", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_h", 3)then
									...
									TriggerServerEvent('hhrp-ambulancejob:removeItem', 'medikit')
									TriggerServerEvent('hhrp-ambulancejob:revive', GetPlayerServerId(closestPlayer))
						...
						HHCore.TriggerServerCallback('hhrp-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('hhrp-ambulancejob:removeItem', 'bandage')
									TriggerServerEvent('hhrp-ambulancejob:heal', GetPlayerServerId(closestPlayer), 'small')
						...
						HHCore.TriggerServerCallback('hhrp-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('hhrp-ambulancejob:removeItem', 'medikit')
									TriggerServerEvent('hhrp-ambulancejob:heal', GetPlayerServerId(closestPlayer), 'big')
					...
					elseif data.current.value == 'put_in_vehicle' then
						TriggerServerEvent('hhrp-ambulancejob:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif data.current.value == 'pull_out_vehicle' then
						TriggerServerEvent('hhrp-ambulancejob:pullOutVehicle', GetPlayerServerId(closestPlayer))
					elseif data.current.value == 'drag_player' then
						TriggerServerEvent('hhrp-ambulancejob:drag', GetPlayerServerId(closestPlayer))
					elseif data.current.value == 'un_drag' then
						TriggerServerEvent('hhrp-ambulancejob:undrag', GetPlayerServerId(closestPlayer))
					end
				end
			end, function(data, menu)
				menu.close()
...
function FastTravel(coords, heading)
	...
	HHCore.Game.Teleport(playerPed, coords, function()
		DoScreenFadeIn(800)
...
Citizen.CreateThread(function()
	while true do
		...
		if HHCore.PlayerData.job and HHCore.PlayerData.job.name == 'ambulance' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastHospital ~= currentHospital or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('hhrp-ambulancejob:hasExitedMarker', LastHospital, LastPart, LastPartNum)
					hasExited = true
				end

				HasAlreadyEnteredMarker, LastHospital, LastPart, LastPartNum = true, currentHospital, currentPart, currentPartNum

				TriggerEvent('hhrp-ambulancejob:hasEnteredMarker', currentHospital, currentPart, currentPartNum)
...
AddEventHandler('hhrp-ambulancejob:hasEnteredMarker', function(hospital, part, partNum)
	if HHCore.PlayerData.job and HHCore.PlayerData.job.name == 'ambulance' then
...
AddEventHandler('hhrp-ambulancejob:hasExitedMarker', function(hospital, part, partNum)
...
Citizen.CreateThread(function()
	while true do
		...
		elseif HHCore.PlayerData.job ~= nil and HHCore.PlayerData.job.name == 'ambulance' and not IsDead then
...
RegisterNetEvent('hhrp-ambulancejob:putInVehicle')
AddEventHandler('hhrp-ambulancejob:putInVehicle', function()
	local playerPed = PlayerPedId()
...
RegisterNetEvent('hhrp-ambulancejob:pullOutVehicle')
AddEventHandler('hhrp-ambulancejob:pullOutVehicle', function()
	local playerPed = PlayerPedId()
...
RegisterNetEvent('hhrp-ambulancejob:drag')
AddEventHandler('hhrp-ambulancejob:drag', function(cop)
	IsDragged = not IsDragged
	CopPed = tonumber(cop)
end)

RegisterNetEvent('hhrp-ambulancejob:un_drag')
AddEventHandler('hhrp-ambulancejob:un_drag', function(cop)
	IsDragged = not IsDragged
...
function OpenCloakroomMenu()
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		title    = _U('cloakroom'),
		align    = 'bottom-right',
		elements = {
...
function OpenVehicleSpawnerMenu(hospital, partNum)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			local authorizedVehicles = Config.AuthorizedVehicles[HHCore.PlayerData.job.grade_name]

			if #authorizedVehicles > 0 then
				for k,vehicle in ipairs(authorizedVehicles) do
					table.insert(shopElements, {
						label = ('%s - <span style="color:green;">%s</span>'):format(vehicle.label, _U('shop_item', HHCore.Math.GroupDigits(vehicle.price))),
						name  = vehicle.label,
						model = vehicle.model,
						price = vehicle.price,
						type  = 'car'
		...
		elseif data.current.action == 'garage' then
			...
			HHCore.TriggerServerCallback('hhrp-vehicleshop:retrieveJobVehicles', function(jobVehicles)
				if #jobVehicles > 0 then
					...
					HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_garage', {
						title    = _U('garage_title'),
						align    = 'bottom-right',
						elements = garage
						...
						if data2.current.stored then
							...
							if foundSpawn then
								...
								HHCore.Game.SpawnVehicle(data2.current.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
									HHCore.Game.SetVehicleProperties(vehicle, data2.current.vehicleProps)

									TriggerServerEvent('hhrp-vehicleshop:setJobVehicleState', data2.current.vehicleProps.plate, false)
...
function StoreNearbyVehicle(playerCoords)
	local vehicles, vehiclePlates = HHCore.Game.GetVehiclesInArea(playerCoords, 30.0), {}
		...
		for k,v in ipairs(vehicles) do
			...
			if GetVehicleNumberOfPassengers(v) == 0 and IsVehicleSeatFree(v, -1) then
				table.insert(vehiclePlates, {
					vehicle = v,
					plate = HHCore.Math.Trim(GetVehicleNumberPlateText(v))
				})
			end
		end
	else
	...
	HHCore.TriggerServerCallback('hhrp-ambulancejob:storeNearbyVehicle', function(storeSuccess, foundNum)
		if storeSuccess then
			local vehicleId = vehiclePlates[foundNum]
			local attempts = 0
			HHCore.Game.DeleteVehicle(vehicleId.vehicle)
			IsBusy = true
			...
			while DoesEntityExist(vehicleId.vehicle) do
				...
				vehicles = HHCore.Game.GetVehiclesInArea(playerCoords, 30.0)
				if #vehicles > 0 then
					for k,v in ipairs(vehicles) do
						if HHCore.Math.Trim(GetVehicleNumberPlateText(v)) == vehicleId.plate then
							HHCore.Game.DeleteVehicle(v)
							break
						end
					end
				end
...
function GetAvailableVehicleSpawnPoint(hospital, part, partNum)
	...
	for i=1, #spawnPoints, 1 do
		if HHCore.Game.IsSpawnPointClear(spawnPoints[i].coords, spawnPoints[i].radius) then
...
function OpenHelicopterSpawnerMenu(hospital, partNum)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'helicopter_spawner', {
		...
		if data.current.action == 'buy_helicopter' then
			...
			local authorizedHelicopters = Config.AuthorizedHelicopters[HHCore.PlayerData.job.grade_name]

			if #authorizedHelicopters > 0 then
				for k,helicopter in ipairs(authorizedHelicopters) do
					table.insert(shopElements, {
						label = ('%s - <span style="color:green;">%s</span>'):format(helicopter.label, _U('shop_item', HHCore.Math.GroupDigits(helicopter.price))),
		...
		elseif data.current.action == 'garage' then
			...
			HHCore.TriggerServerCallback('hhrp-vehicleshop:retrieveJobVehicles', function(jobVehicles)
				if #jobVehicles > 0 then
					...
					HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'helicopter_garage', {
...
function OpenShopMenu(elements, restoreCoords, shopCoords)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		title    = _U('vehicleshop_title'),
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)

		HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				local vehicle  = GetVehiclePedIsIn(playerPed, false)
				local props    = HHCore.Game.GetVehicleProperties(vehicle)
				props.plate    = newPlate

				HHCore.TriggerServerCallback('hhrp-ambulancejob:buyJobVehicle', function (bought)
					if bought then
						HHCore.ShowNotification(_U('vehicleshop_bought', data.current.name, HHCore.Math.GroupDigits(data.current.price)))
						...
						HHCore.Game.Teleport(playerPed, restoreCoords)
					else
		...
		HHCore.Game.SpawnLocalVehicle(data.current.model, shopCoords, 0.0, function(vehicle)
	...
	HHCore.Game.SpawnLocalVehicle(elements[1].model, shopCoords, 0.0, function(vehicle)
		table.insert(spawnedVehicles, vehicle)
		TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
		FreezeEntityPosition(vehicle, true)
	end)
...
function WarpPedInClosestVehicle(ped)
	...
	local vehicle, distance = HHCore.Game.GetClosestVehicle(coords)
...
RegisterNetEvent('hhrp-ambulancejob:heal')
AddEventHandler('hhrp-ambulancejob:heal', function(healType)
    local playerPed = PlayerPedId()
    ...
    if healType == 'small' then
        ...
        TriggerEvent('hhrp-hospital:client:RemoveBleed')
	elseif healType == 'big' then
		local health = GetEntityHealth(playerPed)
        local newHealth = math.min(maxHealth, math.floor(health + maxHealth / 10))
        SetEntityHealth(playerPed, newHealth)
        TriggerEvent('hhrp-hospital:client:RemoveBleed')
        TriggerEvent('hhrp-hospital:client:ResetLimbs')
    end
