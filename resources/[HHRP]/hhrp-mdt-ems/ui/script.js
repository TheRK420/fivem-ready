const mdtApp = new Vue({
    ...
    methods: {
        changePage(page) {
            ...
            } else if (page == "Search Pacients") {
                $("#search-pacients").addClass("nav-active");
            } else if (page == "Search Vehicles") {
                $("#search-vehicles").addClass("nav-active");
            } else if (page == "Warrants") {
                $.post('http://hhrp-mdt-ems/getWarrants', JSON.stringify({}));
        ...
        closeMDT() {
            $.post('http://hhrp-mdt-ems/close', JSON.stringify({}));
        },
        OffenderSearch() {
            if (this.offender_search) {

                this.offender_results.query = this.offender_search;
                $.post('http://hhrp-mdt-ems/performOffenderSearch', JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    ...
                    $.post('http://hhrp-mdt-ems/viewOffender', JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post('http://hhrp-mdt-ems/performReportSearch', JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && (Object.keys(this.report_new.charges).length > 0) && this.report_new.incident) {
                $.post('http://hhrp-mdt-ems/submitNewReport', JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post('http://hhrp-mdt-ems/getOffender', JSON.stringify({
                char_id: id
            }));
        ...
        DeleteSelectedReport() {
            $.post('http://hhrp-mdt-ems/deleteReport', JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post('http://hhrp-mdt-ems/saveReportChanges', JSON.stringify({
                id: this.report_selected.id,
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post('http://hhrp-mdt-ems/vehicleSearch', JSON.stringify({
        ...
        OpenVehicleDetails(result) {
            $.post('http://hhrp-mdt-ems/getVehicle', JSON.stringify({
                vehicle: result
            }));
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://hhrp-mdt-ems/submitNewWarrant', JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post('http://hhrp-mdt-ems/deleteWarrant', JSON.stringify({
        ...
        OpenReportById(id) {
            $.post('http://hhrp-mdt-ems/getReport', JSON.stringify({
                id: id
            }));
            return;
        },
        ...
        SentencePlayer() {
            ...
            $.post('http://hhrp-mdt-ems/sentencePlayer', JSON.stringify({
    ...
    if (document.readyState === "complete") {
        window.addEventListener('message', function(event) {
            if (event.data.type == "enable") {
                ...
                mdtApp.changePage("Search Pacients");
...
function ClearActiveNavItems() {
    ...
    $("#search-pacients").removeClass("nav-active");
