...
function SetVehicleMaxMods(vehicle)
  ...
  HHCore.Game.SetVehicleProperties(vehicle, props)
    ...
    function(data, menu)
      if data.current.value == 'vehicle_list' then
        ...
        if Config.EnableSocietyOwnedVehicles then
            ...
            HHCore.TriggerServerCallback('hhrp_society:getVehiclesInGarage', function(vehicles)
              ...
              HHCore.UI.Menu.Open(
                ...
                function(data, menu)
                  ...
                  HHCore.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
                    HHCore.Game.SetVehicleProperties(vehicle, vehicleProps)
                    local playerPed = GetPlayerPed(-1)
                    TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
                  end)

                  TriggerServerEvent('hhrp_society:removeVehicleFromGarage', 'weazelnews', vehicleProps)
              ...
              function(data, menu)
                for i=1, #elements, 1 do
                  if Config.MaxInService == -1 then
                    HHCore.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 90.0, function(vehicle)
                      local playerPed = GetPlayerPed(-1)
                      TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
                    ...
                    HHCore.TriggerServerCallback('hhrp_service:enableService', function(canTakeService, maxInService, inServiceCount)
                      if canTakeService then
                        ...
                        HHCore.ShowNotification(_U('service_full') .. inServiceCount .. '/' .. maxInService)
                      end
                    end, 'weazelnews')
                    break
                  end
      ...
      if data.current.value == 'boss_actions' then
        TriggerEvent('hhrp_society:openBossMenu', 'weazelnews', function(data, menu)
          menu.close()
        end)
      end
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('hhrp_billing:sendBill1', GetPlayerServerId(player), 'society_weazelnews', _U('billing'), amount)
        end

      else
        HHCore.ShowNotification(_U('no_players_nearby'))
      end
...
function OpenGetStocksMenu()
  ...
  HHCore.TriggerServerCallback('sp_weazelnews:getStockItems', function(items)
...
HHCore.TriggerServerCallback('sp_weazelnews:getPlayerInventory', function(inventory)
  ...
  if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
    ...
    if DoesEntityExist(vehicle) then
      ...
      Citizen.CreateThread(function()
        ...
        HHCore.ShowNotification(_U('veh_repaired'))
      end)
    end
  end
end)
          ...
          if IsControlJustReleased(0, 38) and PlayerData.job ~= nil and PlayerData.job.name == 'weazelnews' then
            ...
            if CurrentAction == 'delete_vehicle' then
              ...
              if Config.EnableSocietyOwnedVehicles then
                ...
                TriggerServerEvent('hhrp_society:putVehicleInGarage', 'weazelnews', vehicleProps)
                ...
                then
                  TriggerServerEvent('hhrp_service:disableService', 'weazelnews')
                end
