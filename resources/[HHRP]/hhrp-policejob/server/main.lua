...
if Config.MaxInService ~= -1 then
	TriggerEvent('hhrp-service:activateService', 'police', Config.MaxInService)
end
...
AddEventHandler('playerDropped', function()
	...
	if _source ~= nil then
		...
		if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'police' then
			Citizen.Wait(5000)
			TriggerClientEvent('hhrp-policejob:updateBlip', -1)
...
RegisterServerEvent('hhrp-policejob:spawned')
AddEventHandler('hhrp-policejob:spawned', function()
	local _source = source
	local xPlayer = HHCore.GetPlayerFromId(_source)
...
RegisterServerEvent('hhrp-policejob:forceBlip')
AddEventHandler('hhrp-policejob:forceBlip', function()
	TriggerClientEvent('hhrp-policejob:updateBlip', -1)
end)
...
AddEventHandler('reviveGranted', function(target)
	local xPlayer = HHCore.GetPlayerFromId(source)
		...
		--TriggerClientEvent('HHCore_ambulancejob:revive', target)
		Wait(5000)
...
RegisterCommand('givelicense', function(source, cb)
	...
	for k, v in ipairs(players) do
	  ...
	  local xSource = HHCore.GetPlayerFromId(source)
		...
		TriggerEvent('hhrp-license:checkLicense', v, 'weapon', function(cb)
		  if cb == false then 
			TriggerEvent('hhrp-license:addLicense', v, 'weapon', function()
			end)
