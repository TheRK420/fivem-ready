...
AddEventHandler('hhrp:setJob', function(job)
	...
	TriggerServerEvent('hhrp-policejob:forceBlip')
end)
...
AddEventHandler('hhrp-phone:cancelMessage', function(dispatchNumber)
	if PlayerData.job and PlayerData.job.name == 'police' and PlayerData.job.name == dispatchNumber then
		-- if HHCore_service is enabled
...
local evidencelocker = { -- #MarkedForMarker
	vector3(485.53,-989.2,24.23), -- mrpd
...
local trashlocker = {
	vector3(483.76,-987.58,24.23), -- mrpd
...
AddEventHandler('police:gsr', function(t)
	...
	local finished = exports["hhrp-taskbar"]:taskBar(15000,"GSR Testing")
    if finished == 100 then
		t, distance = GetClosestPlayer()
		if(distance ~= -1 and distance < 7) then
			TriggerServerEvent("hhrp-policejob:checkgsr", GetPlayerServerId(t))
		end
	end
end)
...
AddEventHandler('police:getArrested2', function(cuffer)
	...
	local finished = 0
	if not exports["hhrp-ambulancejob"]:GetDeath() then
		finished = exports["hhrp-taskbarskill"]:taskBar(1200,7)
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		...
		if Config.MaxInService ~= -1 then
			TriggerServerEvent('hhrp-service:disableService', 'police')
		end
	end
end)
...
AddEventHandler( 'Police:Radio', function()
	if isCop then
	    ...
	    if ( DoesEntityExist( ped ) and not IsEntityDead( ped )) and exports["hhrp-ambulancejob"]:GetDeath() ~= 1 then
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			...
			if myjob ~= "ambulance" then
				local bandages = exports["hhrp-inventory"]:getQuantity("bandage")
				if bandages == 0 then
					return
				else
...
AddEventHandler("police:rob", function()
	if not exports["hhrp-ambulancejob"]:GetDeath() then
		...
		local finished = exports["hhrp-taskbar"]:taskBar(3500,"Robbing",false,true)	
...
AddEventHandler( 'KneelHU', function()
	...
	if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
		...
		local finished = exports["hhrp-taskbar"]:taskBar(2500,"Surrendering")					
...
AddEventHandler('FlipVehicle', function()
	local finished = exports["hhrp-taskbar"]:taskBar(5000,"Flipping Vehicle Over",false,true)	
...
function deleteVeh(ent)
	...
	local finished = exports["hhrp-taskbar"]:taskBar(1000,"Impounding",false,true)	
...
RegisterNetEvent('hhrp-policejob:updateBlip')
AddEventHandler('hhrp-policejob:updateBlip', function()
	...
	if PlayerData.job and PlayerData.job.name == 'police' or PlayerData.job.name == 'ambulance' then
		HHCore.TriggerServerCallback('hhrp-society:getOnlinePlayers', function(players)
...
AddEventHandler('playerSpawned', function(spawn)
	if not hasAlreadyJoined then
		TriggerServerEvent('hhrp-policejob:spawned')
	end
...
AddEventHandler("tp:pdrevive", function()
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
		...
		TriggerServerEvent('hhrp-ambulancejob:revivePD', GetPlayerServerId(closestPlayer))
