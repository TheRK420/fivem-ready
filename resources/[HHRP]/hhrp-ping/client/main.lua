    ...
    Citizen.CreateThread(function()
        ...
        while isPending do
            ...
            if count >= Config.Timeout and isPending then
                ...
                TriggerServerEvent('hhrp-ping:server:SendPingResult', pendingPing.id, 'timeout')
                pendingPing = nil
                isPending = false
            end
            Citizen.Wait(1000)
...
RegisterNetEvent('hhrp-ping:client:SendPing')
AddEventHandler('hhrp-ping:client:SendPing', function(sender, senderId)
    if pendingPing == nil then
        ...
        TriggerServerEvent('hhrp-ping:server:SendPingResult', pendingPing.id, 'received')
        ...
        TriggerServerEvent('hhrp-ping:server:SendPingResult', senderId, 'unable')
    end
end)

RegisterNetEvent('hhrp-ping:client:AcceptPing')
AddEventHandler('hhrp-ping:client:AcceptPing', function()
    if isPending then
        ...
        TriggerServerEvent('hhrp-ping:server:SendPingResult', pendingPing.id, 'accept')
        isPending = false
    else
...
RegisterNetEvent('hhrp-ping:client:RejectPing')
AddEventHandler('hhrp-ping:client:RejectPing', function()
    if pendingPing ~= nil then
        TriggerEvent('DoLongHudText', 'Rejected Ping From ' .. pendingPing.name, 2)
        TriggerServerEvent('hhrp-ping:server:SendPingResult', pendingPing.id, 'reject')
        pendingPing = nil
        isPending = false
    else
        TriggerEvent('DoLongHudText', 'You Have No Pending Ping', 2)
...
RegisterNetEvent('hhrp-ping:client:RemovePing')
AddEventHandler('hhrp-ping:client:RemovePing', function()
    if pendingPing ~= nil then
        RemoveBlip(pendingPing.blip)
        pendingPing = nil
