...
RegisterServerEvent('hhrp-skin:save')
AddEventHandler('hhrp-skin:save', function()
    local src = source
    TriggerClientEvent('hhrp-clothing:guardarSkin',src)
end)

HHCore.RegisterServerCallback('hhrp-skin:getPlayerSkin', function(source, cb)
...
HHCore.RegisterServerCallback('hhrp-clothing:getSex', function(source, cb)
    local xPlayer = HHCore.GetPlayerFromId(source)
...
RegisterServerEvent("hhrp-clothing:insert_character_current")
AddEventHandler("hhrp-clothing:insert_character_current",function(data)
...
RegisterServerEvent("hhrp-clothing:insert_character_face")
AddEventHandler("hhrp-clothing:insert_character_face",function(data)
    if not data then return end
    local src = source
...
RegisterServerEvent("hhrp-clothing:get_character_face")
    ...
    MySQL.Async.fetchAll("SELECT cc.model, cf.hairColor, cf.headBlend, cf.headOverlay, cf.headStructure FROM users cf INNER JOIN users cc on cc.identifier = cf.identifier WHERE cf.identifier = @identifier", {['identifier'] = characterId}, function(result)
        if (result ~= nil and result[1] ~= nil) then
            ...
            if model == 1885233650 or model == -1667301416 then
                TriggerClientEvent("hhrp-clothing:setpedfeatures", src, temp_data)
            end
        else
            TriggerClientEvent("hhrp-clothing:setpedfeatures", src, false)
        end
	end)
end)

RegisterServerEvent("hhrp-clothing:get_character_current")
AddEventHandler("hhrp-clothing:get_character_current",function(pSrc)
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['identifier'] = characterId}, function(result)
        ...
        TriggerClientEvent("hhrp-clothing:setclothes", src, temp_data,0)
	end)
end)
...
AddEventHandler("hhrp-clothing:retrieve_tats", function(pSrc)
	...
	MySQL.Async.fetchAll("SELECT * FROM playerstattoos WHERE identifier = @identifier", {['identifier'] = user.identifier}, function(result)
        if(#result == 1) then
			...
			TriggerClientEvent("hhrp-clothing:settattoos", src, {})
		end
	end)
end)

RegisterServerEvent("hhrp-clothing:set_tats")
AddEventHandler("hhrp-clothing:set_tats", function(tattoosList)
	local src = source
	local user = HHCore.GetPlayerFromId(src)
...
RegisterServerEvent("hhrp-clothing:get_outfit")
AddEventHandler("hhrp-clothing:get_outfit",function(slot)
    ...
    MySQL.Async.fetchAll("SELECT * FROM character_outfits WHERE steamid = @steamid and slot = @slot", {
        ...
        if result and result[1] then
            ...
            TriggerClientEvent("hhrp-clothing:setclothes", src, data,0)
...
RegisterServerEvent("hhrp-clothing:set_outfit")
AddEventHandler("hhrp-clothing:set_outfit",function(slot, name, data)
    if not slot then return end
    local src = source
...
RegisterServerEvent("hhrp-clothing:remove_outfit")
AddEventHandler("hhrp-clothing:remove_outfit",function(slot)
...
RegisterServerEvent("hhrp-clothing:list_outfits")
AddEventHandler("hhrp-clothing:list_outfits",function()
    local src = source
    local user = HHCore.GetPlayerFromId(src)
    local steamid = user.identifier
    local slot = slot
    ...
    MySQL.Async.fetchAll("SELECT slot, name FROM character_outfits WHERE steamid = @steamid", {['steamid'] = steamid}, function(skincheck)
    	TriggerClientEvent("hhrp-clothing:listOutfits",src, skincheck)
	end)
end)
        ...
        if result[1] == nil then
            MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {["identifier"] = steamid}, function(result)
                if result[1].skin then
                    TriggerClientEvent('hhrp-clothing:setclothes',src,{},json.decode(result[1].skin))
                else
                    TriggerClientEvent('hhrp-clothing:setclothes',src,{},nil)
                end
                return
            end)
        else
            TriggerEvent("hhrp-clothing:get_character_current", src)
        end
        TriggerClientEvent("hhrp-clothing:inService",src,isService)
    end)
end)
...
AddEventHandler("clothing:checkMoney", function(menu,askingPrice)
    local src = source
    local target = HHCore.GetPlayerFromId(src)
    ...
    if (tonumber(target.getMoney()) >= askingPrice) then
        ...
        TriggerClientEvent("hhrp-clothing:hasEnough",src,menu)
    else
