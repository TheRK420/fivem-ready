...
AddEventHandler('hhrp-skin:setLastSkin', function(skin)
    LastSkin = skin
  end)
...
function Save(save)
    if save then
        data = GetCurrentPed()
        TriggerServerEvent("hhrp-clothing:insert_character_current", data)
        if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
            TriggerServerEvent("hhrp-clothing:insert_character_face", data)
            TriggerServerEvent("hhrp-clothing:set_tats", currentTats)
        end
    else
        LoadPed(oldPed)
    end
...
RegisterNetEvent("hhrp-clothing:guardarSkin")
AddEventHandler("hhrp-clothing:guardarSkin", function()
    data = GetCurrentPed()
    TriggerServerEvent("hhrp-clothing:insert_character_current", data)
    if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
        TriggerServerEvent("hhrp-clothing:insert_character_face", data)
...
RegisterNetEvent("hhrp-clothing:inService")
AddEventHandler("hhrp-clothing:inService", function(service)
    isService = service
end)

RegisterNetEvent('hhrp-identity:identityCheck')
AddEventHandler('hhrp-identity:identityCheck', function(identityCheck)
	hasIdentity = identityCheck
end)

RegisterNetEvent("hhrp-clothing:hasEnough")
AddEventHandler("hhrp-clothing:hasEnough", function(menu)
    if menu == "tattoomenu" then
        TriggerServerEvent("hhrp-clothing:retrieve_tats")
...
RegisterNetEvent("hhrp-clothing:setclothes")
AddEventHandler("hhrp-clothing:setclothes", function(data,alreadyExist)
    player = GetPlayerPed(-1)
    local function setDefault()
        Citizen.CreateThread(function()
            firstChar = true
            local gender = nil
            HHCore.TriggerServerCallback('hhrp-clothing:getSex', function(sex)
				gender = sex
			end)
            Citizen.Wait(1000)
            if gender ~= "m" then
            ...
            DoScreenFadeIn(50)
            Wait(5000)
            if not exports["hhrp-inventory"]:hasEnoughOfItem("idcard",1,false) then
                TriggerEvent("player:receiveItem","idcard",1,true)
            end
            if not exports["hhrp-inventory"]:hasEnoughOfItem("mobilephone",1,false)then
    ...
    TriggerServerEvent("hhrp-clothing:get_character_face")
    TriggerServerEvent("hhrp-clothing:retrieve_tats")
end)


RegisterNetEvent("hhrp-clothing:defaultReset")
AddEventHandler("hhrp-clothing:defaultReset", function()
    local gender = nil
    HHCore.TriggerServerCallback('hhrp-clothing:getSex', function(sex)
        gender = sex
    end)
...
RegisterNetEvent("hhrp-clothing:settattoos")
AddEventHandler("hhrp-clothing:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("hhrp-clothing:setpedfeatures")
AddEventHandler("hhrp-clothing:setpedfeatures", function(data)
    player = GetPlayerPed(-1)
    if data == false then
...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if exports["hhrp-housing"]:imClosesToRoom2() or exports["hhrp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] and args[2] then
            TriggerEvent('hhrp-clothing:outfits', 1, args[1], args[2])
        else
...
RegisterCommand("outfituse", function(source, args, rawCommand)
    if exports["hhrp-housing"]:imClosesToRoom2() or exports["hhrp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            TriggerEvent('hhrp-clothing:outfits', 3, args[1])
        else
...
RegisterCommand("outfitdel", function(source, args, rawCommand)
    if exports["hhrp-housing"]:imClosesToRoom2() or exports["hhrp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            TriggerEvent('hhrp-clothing:outfits', 2, args[1])
        else
            TriggerEvent('DoLongHudText', "You need to do something like /outfituse 1 | 1 being the slot id that you will have had previously saved", 1)  
        end
    else
...
RegisterCommand("outfits", function(source, args, rawCommand)
    if exports["hhrp-housing"]:imClosesToRoom2() or exports["hhrp-apartments"]:imClosesToRoom3() or (IsNearShop(clothingShops) < 9.0) then
        TriggerEvent('hhrp-clothing:outfits', 4)
    else
        TriggerEvent('DoLongHudText', "You are not near a wardrobe", 2)
    end
end, false)  

RegisterNetEvent('hhrp-clothing:listOutfits')
AddEventHandler('hhrp-clothing:listOutfits', function(skincheck)
...
RegisterNetEvent('hhrp-clothing:outfits')
AddEventHandler('hhrp-clothing:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("hhrp-clothing:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("hhrp-clothing:remove_outfit",pId)
    elseif pAction == 3 then
        TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("hhrp-clothing:get_outfit", pId)
    else
        TriggerServerEvent("hhrp-clothing:list_outfits")
    end
end)
