...
AddEventHandler("AskForTrainConfirmed", function()
	...
	if metroDist < countryDist then
		if metroDist < 25.0 then 
			exports["hhrp-taskbar"]:taskBar(60000, "Waiting For Train") 	
				StartMetro(closestMetro)
				TriggerEvent('DoLongHudText', 'THOMAS: Choo Choo..')
			else
			TriggerEvent('DoLongHudText', 'THOMAS: You are not near a train station..', 2)
		end
	else
...
function StartMetro(coords)
	...
	TriggerEvent('DoLongHudText', 'THOMAS: Train will depart in 30 seconds.')
