...
SpawnLocalVehicle = function(data)
	...
	if not HHCoreGame.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
	...
	HHCoreGame.SpawnLocalVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
...
SpawnVehicle = function(data,recuperar)
	...
	CloseMenu()
	local gameVehicles = HHCoreGame.GetVehicles()
	...
	HHCoreGame.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
		SetVehicleProperties(yourVehicle, vehicleProps)
...
PutInVehicle = function()
    ...
    if DoesEntityExist(vehicle) then
		...
		HHCoreTriggerServerCallback("erp_garage:validateVehicle", function(valid)
			if valid then
				AbrirMenuGuardar()
            else
                CloseMenu()
...
SetVehicleProperties = function(vehicle, vehicleProps)
    HHCoreGame.SetVehicleProperties(vehicle, vehicleProps)
...
GetVehicleProperties = function(vehicle)
    if DoesEntityExist(vehicle) then
        local vehicleProps = HHCoreGame.GetVehicleProperties(vehicle)
...
DrawScriptMarker = function(markerData)
    DrawMarker(20, markerData["pos"],0,0,0,0,0,0,0.701,1.0001,0.3001,222, 50, 50, 0.05,0, 0,0,0,0,0,0)
      --  HHCoreShowHelpNotification("Press [E] to Access Garage")
end
