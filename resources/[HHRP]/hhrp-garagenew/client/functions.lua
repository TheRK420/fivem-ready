...
SpawnLocalVehicle = function(data)
	...
	if not HHCore.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
	...
	HHCore.Game.SpawnLocalVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
...
SpawnVehicle = function(data,recuperar)
	...
	HHCore.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
		SetVehicleProperties(yourVehicle, vehicleProps)
...
PutInVehicle = function()
    ...
    if DoesEntityExist(vehicle) then
		...
		HHCore.TriggerServerCallback("betrayed_garage:validateVehicle", function(valid)
			if valid then
				AbrirMenuGuardar()
            else
                CloseMenu()
