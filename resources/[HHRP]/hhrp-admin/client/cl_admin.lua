...
function hhrp.Admin.CheckForSessions(self)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            local players = hhrp._Admin.Players

            for k,v in pairs(players) do
                local src = v.src
                local playerId = GetPlayerFromServerId(src)
                if not src then
                    hhrp._Admin.Players[src].sessioned = true
                else
                    if not NetworkIsPlayerActive(playerId) or not NetworkIsPlayerConnected(playerId) then hhrp._Admin.Players[src].sessioned = true end
                end
            end
        end
    end)
...
stop = true
function hhrp.Admin.BeginAFK(self, stop)
    if stop then
        afk.isAfk = false
        afk.msgAFK = false
        afk.stringToType = ""
    ...
    afk.event = AddEventHandler("hhrp-admin:afkStringCheck", function(text)
        if string.lower(text) == afk.stringToType then hhrp.Admin:BeginAFK(true) return end
    end)
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if curTime - beginTime >= 300000 then TriggerServerEvent("hhrp-admin:Disconnect", "AFK Kick") return end
        end
    end)
end

function hhrp.Admin.AFKCheck(self)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            local rankData = hhrp.Admin:GetRankData(rank)
...
RegisterNetEvent("hhrp-admin:afkStringCheck")

RegisterNetEvent("hhrp-admin:setStatus")
AddEventHandler("hhrp-admin:setStatus", function(src, status)
    local player = hhrp._Admin.Players[src]
    if not player then return else hhrp._Admin.Players[src].status = status end
end)

RegisterNetEvent("hhrp-admin:sendPlayerInfo")
AddEventHandler("hhrp-admin:sendPlayerInfo", function(data, discData)
    hhrp._Admin.Players = data
    hhrp._Admin.DiscPlayers = discData
end)

RegisterNetEvent("hhrp-admin:RemovePlayer")
AddEventHandler("hhrp-admin:RemovePlayer", function(data)
    hhrp._Admin.DiscPlayers[data.src] = data
    hhrp._Admin.Players[data.src] = nil
end)

RegisterNetEvent("hhrp-admin:RemoveRecent")
AddEventHandler("hhrp-admin:RemoveRecent", function(data)
    if data ~= nil then
        hhrp._Admin.DiscPlayers[data.src] = nil
    end
end)

RegisterNetEvent("hhrp-admin:AddPlayer")
AddEventHandler("hhrp-admin:AddPlayer", function(player)
    hhrp._Admin.Players[player.src] = player
end)

RegisterNetEvent('hhrp-admin:teleportUser')
AddEventHandler('hhrp-admin:teleportUser', function(x, y, z)
    SetEntityCoords(PlayerPedId(), x, y, z)
    TriggerEvent('DoLongHudText', 'You were summoned')
end)

RegisterNetEvent("hhrp-admin:AddAllPlayers")
AddEventHandler("hhrp-admin:AddAllPlayers", function(data)
    hhrp._Admin.Players[data.src] = data
end)

function hhrp.Admin.RunCommand(self, args)
    if not args or not args.command then return end
    TriggerServerEvent("hhrp-admin:runCommand", args)
end

function hhrp.Admin.RunClCommand(self, cmd, args)
    if not cmd or not self:CommandExists(cmd) then return end
    self:GetCommandData(cmd).runclcommand(args)
end

RegisterNetEvent("hhrp-admin:RunClCommand")
AddEventHandler("hhrp-admin:RunClCommand", function(cmd, args)
    hhrp.Admin:RunClCommand(cmd, args)
end)

RegisterNetEvent("hhrp-admin:updateData")
AddEventHandler("hhrp-admin:updateData", function(src, type, data)
    if not src or not type or not data then return end
    if not hhrp._Admin.Players[src] then return end
    
    hhrp._Admin.Players[src][type] = data
end)

RegisterNetEvent("hhrp-admin:noLongerAdmin")
AddEventHandler("hhrp-admin:noLongerAdmin", function()
    hhrp._Admin.Players = {}
...
RegisterNetEvent("hhrp-admin:bringPlayer")
AddEventHandler("hhrp-admin:bringPlayer", function(targPos)
    local ped = PlayerPedId()
...
RegisterNetEvent("hhrp-admin:bring")
AddEventHandler("hhrp-admin:bring", function(target)
    ...
    TriggerServerEvent("hhrp-admin:bringPlayerServer",pos,target)   
end)



 
RegisterNetEvent("hhrp-admin:runSpawnCommand")
AddEventHandler("hhrp-admin:runSpawnCommand", function(model, livery)
    Citizen.CreateThread(function()
...
function hhrp.Admin.teleportMarker(self)
...
RegisterNetEvent("hhrp-admin:ReviveInDistance")
AddEventHandler("hhrp-admin:ReviveInDistance", function()
    local playerList = {}
