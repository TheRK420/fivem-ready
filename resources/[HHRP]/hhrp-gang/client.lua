...
--AddEventHandler('onResourceStart', function(resourceName) -- For debugging to start/stop resources without server restart
	...
	Citizen.Wait(5000)
	HHCore.TriggerServerCallback('sody_clubs:getPlayerClub', function(playerdata)
...
AddEventHandler('sody_clubs:action', function(zone)
	...
	if zone.Type == "storage" then
		...
		HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'job_actions', {
			title    = _U(menutitle),
			align    = 'top-left',
			elements = elements
		}, function(data, menu)
	...
	elseif zone.Type == "gteleport" then
		...
		elseif not vehicle then
			HHCore.ShowNotification("Must be driving a vehicle to exit through garage")
		elseif vehicle and GetPedInVehicleSeat(vehicle, -1) ~= GetPlayerPed(-1) then
			HHCore.ShowNotification("Must be driving the vehicle to exit garage")
		end
...
Citizen.CreateThread(function()
	while true do
		...
		if hintIsShowed and not menuIsShowed then
			HHCore.ShowHelpNotification(hintToDisplay)
		else
			Citizen.Wait(500)
		end
	end
...
function OpenClubOwnerMenu(club, rank)
	...
	HHCore.TriggerServerCallback('sody_clubs:getOwner', function(result)
		isOwner = tonumber(result)
	end, club)

	HHCore.TriggerServerCallback('sody_clubs:getTreasurer', function(result)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'owner_actions_' .. club, {
		title    = _U('owner_menu'),
		align    = 'top-right',
		elements = elements
	}, function(data, menu)
		...
		elseif data.current.value == 'withdraw_bank_money' then
			...
			HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'withdraw_club_money_amount_' .. club, {
				...
				if amount == nil then
					HHCore.ShowNotification(_U('invalid_amount'))
				else
					menu.close()
		...
		elseif data.current.value == 'deposit_bank_money' then
			...
			HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_bank_money_amount_' .. club, {
...
function OpenManageMembersMenu(club)
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_members_' .. club, {
...
function OpenMemberList(club)
	HHCore.TriggerServerCallback('sody_clubs:getMembers', function(members, club)
		local elements = {}
		...
		HHCore.UI.Menu.Open('list', GetCurrentResourceName(), 'member_list_' .. club, elements, function(data, menu)
			...
			elseif data.value == 'remove' then
				HHCore.ShowNotification(_U('you_have_fired', member.name))

				HHCore.TriggerServerCallback('sody_clubs:setClub', function()
					OpenMemberList(club)
...
function OpenRecruitMenu(club)
	HHCore.TriggerServerCallback('sody_clubs:getOnlinePlayers', function(players)
		...
		HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_' .. club, {
			title    = _U('recruiting'),
			align    = 'top-right',
			elements = elements
		}, function(data, menu)

			HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_confirm_' .. club, {
				...
				if data2.current.value == 'yes' then
					HHCore.ShowNotification(_U('you_have_hired', data.current.name))
...
function OpenPromoteMenu(clubname, member)
	HHCore.TriggerServerCallback('sody_clubs:getClub', function(club)
		...
		HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'promote_member_' .. clubname, {
			title    = _U('promote_member', member.name),
			align    = 'top-right',
			elements = elements
		}, function(data, menu)
			menu.close()
			HHCore.ShowNotification(_U('you_have_promoted', member.name, data.current.label))
...
function OpenManagePayMenu(clubname)
	HHCore.TriggerServerCallback('sody_clubs:getClub', function(club)
		...
		for i=1, #club.ranks, 1 do
			...
			table.insert(elements, {
				label = ('%s - <span style="color:green;">%s</span>'):format(rankLabel, _U('money_generic', HHCore.Math.GroupDigits(club.ranks[i].pay))),
				value = club.ranks[i].club_rank
			})
		end

		HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_pay_' .. clubname, {
			title    = _U('pay_management'),
			align    = 'top-right',
			elements = elements
		}, function(data, menu)

			HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'manage_pay_amount_' .. clubname, {
				title = _U('pay_amount')
			}, function(data2, menu2)
				...
				if amount == nil then
					...
					HHCore.TriggerServerCallback('sody_clubs:setClubPay', function()
...
function playerZones()
	Citizen.CreateThread(function()
		while hasClub do
			...
			if HHCore.Table.SizeOf(zones) > 0 then
...
function OpenGetStocksMenu()
	HHCore.TriggerServerCallback('sody_clubs:getStockItems', function(inventory)
		local elements = {}
		local menutitle = PlayerClub .. "menu"

		if inventory.blackMoney > 0 then
			table.insert(elements, {
				label = _U('dirty_money', HHCore.Math.GroupDigits(inventory.blackMoney)),
				type = 'item_account',
				value = 'black_money'
			})
		end
		...
		for i=1, #inventory.weapons, 1 do
			...
			table.insert(elements, {
				label = HHCore.GetWeaponLabel(weapon.name) .. ' [' .. weapon.ammo .. ']',
				type  = 'item_weapon',
				value = weapon.name,
				ammo  = weapon.ammo
			})
		...
		HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			if data.current.type == 'item_weapon' then
				...
				HHCore.SetTimeout(300, function()
					OpenGetStocksMenu()
				end)
			else
				...
				HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
					...
					if count == nil then
						HHCore.ShowNotification(_U('invalid_quantity'))
					else
						menu2.close()
						menu.close()
...
function OpenGetStocksMenuPub()
	HHCore.TriggerServerCallback('sody_clubs:getStockItemsPub', function(inventory)
		local elements = {}
		local menutitle = PlayerClub .. "menu"
...
function OpenGetStocksMenuPriv()
	HHCore.TriggerServerCallback('sody_clubs:getStockItemsPriv', function(inventory)
		local elements = {}
		local menutitle = PlayerClub .. "menu"
...
function OpenPutStocksMenu()
	HHCore.TriggerServerCallback('sody_clubs:getPlayerInventory', function(inventory)
		local elements = {}
...
function OpenChangingRoomMenu()
	HHCore.TriggerServerCallback('hhrp-housing:server:getSavedOutfits', function(dressing)
		...
		HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing',
		{
			...
			TriggerEvent('hhrp-clothing:getSkin', function(skin)
				HHCore.TriggerServerCallback('hhrp-housing:server:getSavedOutfits', function(clothes)
					TriggerEvent('hhrp-clothing:loadClothes', skin, clothes)
					TriggerEvent('hhrp-skin:setLastSkin', skin)

					TriggerEvent('hhrp-clothing:getSkin', function(skin)
						TriggerServerEvent('hhrp-skin:save', skin)
					end)
	...
	HHCore.UI.Menu.Open(
		...
		function(data, menu)
			...
			if(data.current.value == 'list_vehicles') then
				if Config.GarageScript == 'hhrp-garagenew' then
					ListVehiclesMenu()
				elseif Config.GarageScript == 'hhrp-garagenew' then
					ListVehiclesMenuAG()
				end
			end
...
function ListVehiclesMenu()
	...
	HHCore.TriggerServerCallback('betrayed_garage:obtenerVehiculos', function(vehicles)
	...
	Citizen.Wait(10)
 	...
 	for _,v in pairs(vehicles) do
			...
			local teststring = tostring(HHCore.Math.Trim(GetLabelText(hashVehicule)))
...
function ListVehiclesMenuAG()
	...
	HHCore.TriggerServerCallback('hhrp_advancedgarage:getOwnedCars', function(vehicles)
...
function StockVehicleMenu()
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		HHCore.TriggerServerCallback('eden_garage:stockv',function(valid)

			if (valid) then
				HHCore.TriggerServerCallback('eden_garage:getVehicles', function(vehicules)
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		HHCore.TriggerServerCallback('hhrp_advancedgarage:storeVehicle',function(valid)
			...
			if (valid) then
				...
				TriggerServerEvent('hhrp_advancedgarage:setVehicleState', vehicleProps.plate, true)
...
function SpawnVehicleClubAG(vehicle)
	...
	TriggerServerEvent('hhrp_advancedgarage:setVehicleState', vehicle.plate, false)
end
