...
AddEventHandler('jail:lockdown', function(lockdownState)
    ...
    if lockdown and imjailed and not hasMoved then
        ...
        TriggerEvent('chatMessagess', 'STATUS: ', 1, "You have been placed into lockdown due to a disturbance.")
        hasMoved = true
    elseif not lockdown and imjailed then
        hasMoved = false
        TriggerEvent('chatMessagess', 'STATUS: ', 2, "Lockdown has ended.")
    end
end)
...
AddEventHandler('TimeRemaining', function(TimeRemaining, release)
    ...
    TriggerEvent('chatMessagess', '', 2, 'DOC | You have ' .. TimeR .. ' seconds remaining')
end)
