$(document).ready(function(){
  ...
  function playGame(DropAmount,letterSet,speed,intervalSet) {
    ...
    function gameOver() {
      ...
      $.post('http://hhrp-thermite/failure', JSON.stringify({}));
      gameOn = false;
      clearInterval(cleanupInterval);
      getAllAnimations().forEach(function(anim) {
        anim.pause();
      ...
      if (char.length === 1) {
        ...
        if (currentkey == null){
          ...
          if (animations[char] && animations[char].length) {
            ...
            if(last_element.animation.currentTime > 8000.34 && last_element.animation.currentTime < 9487.2)
            {
    ...
    function setupNextLetter() {
      ...
      if(currentAmount == 0 && !soundSet)
      {
        ...
        $.post('http://hhrp-thermite/complete', JSON.stringify({}));
      }
    }
    setupNextLetter();
  }
  ...
  document.onkeyup = function (data) {
    if (data.which == 27 ) {
      $.post('http://hhrp-thermite/close', JSON.stringify({}));
    }
  };
  ...
  document.body.addEventListener('keyup', upPress);
