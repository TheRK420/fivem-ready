...
$(document).ready(function() {
    ...
    $.post('http://hhrp-phone/getWeather', JSON.stringify({}));
    ...
    document.onkeyup = function(data) {
        ...
        if (data.which == 27) {
            $.post('http://hhrp-phone/close', JSON.stringify({}));
        }
    }
        ...
        if (action !== undefined) {
            switch (action) {
                case "yellow-pages-delete":
                    $.post('http://hhrp-phone/deleteYP', JSON.stringify({}));
                    break;
                case "racing-create":
                ...
                case "newPostSubmit":
                    e.preventDefault();
                    $.post('http://hhrp-phone/newPostSubmit', JSON.stringify({
                ...
                case "newCPostSubmit":
                    e.preventDefault();
                    $.post('http://hhrp-phone/newCPostSubmit', JSON.stringify({
                        advert: escapeHtml($("#criminal-pages-form #criminal-pages-form-advert").val())
                    }));
                    $("#criminal-pages-form #criminal-pages-form-advert").attr("style", "").val('')
                    break;
                case "group-manage":
                    console.log('this is duty and off duty')
                    $.post('http://hhrp-phone/manageGroup', JSON.stringify({ GroupID: $(this).data('action-data') }));
                    break;
                case "btnTaskGang":
                    manageGroup = $(this).data('action-data');
                    $.post('http://hhrp-phone/btnTaskGang', JSON.stringify({}));
                    break;
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        $.post('http://hhrp-phone/' + action, JSON.stringify({}));
                    break;
                case "dabcoin":
    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.isHomeOwner === true) {
            $('.btn-home-owner').hide().fadeIn(150);
        }
        ...
        switch (item.openSection) {
            ...
            case "twatter":
                ...
                // console.log(item.twats, item.myhandle)
            ...
            case "GPS":
                ...
                //console.log(JSON.stringify(item.locations)
                openContainer("gps")
                break;
            case "Garage":
            ...
            case "SellHouse":
                    $('.house-entries').empty();
                    addforSellHouse(item.houseData)
                    openContainer("house");
                    break;
            case "addStocks":
            ...
            case "racing:hud:update":
                switch (item.hudState) {
                    ...
                    case "finished":
                        ...
                        drawRaceStats();
                        $.post('http://hhrp-phone/race:completed', JSON.stringify({
...
function addEmails(emails) {
    console.log('this is wrap for email')
        ...
        let deliveryElement =
            ...
            <li>
                ...
                <span class="secondary-content delivery-job-accept" data-job-id="${deliveryEntry.jobId}" data-job-type="${deliveryEntry.jobType}" aria-label="Click to accept job" data-balloon-pos="left">
                    <i class="fas fa-clipboard-check fa-2x" data-job-id="${deliveryEntry.jobId}"></i>
                </span>
            </li>
        `
            ...
            <li data-key-type="${keyType}">
                <div class="collapsible-header">
                    ...
                    <div class="col s2 right-align">
                        ...
                        <i class="fas fa-map-marker-alt teal-text gps-house-location-click" data-house-type="${key.house_model}" data-house-id="${key.house_id}"></i>
                        <i class="fas fa-map-marker-alt teal-text house-location-click" data-house-type="${key.house_model}" data-house-id="${key.house_id}"></i>
                    </div>
                </div>
...
function openBrowser(url) {
    ...
    closePhoneShell();
    $.post('http://hhrp-phone/btnCamera', JSON.stringify({}));
    $("#browser").fadeIn(300);
}
...
function openRadio() {
    let browserRadio =
        `
        <object type="text/html" data="http://heisthome.tk:8080" class="browser-radio-window">
        </object>
    `;
...
var vehicleElement = `
        ...
        vehicleElement += `<button class="waves-effect waves-light btn-small red garage-pay" data-plate="${vehicleData[vehicle].plate}"><i class="fas fa-hand-holding-usd"></i> Pay Finance</button> `
            
        if (vehicleData[vehicle].garage == "Garage Impound Lot"){
            vehicleElement += `<button class="waves-effect waves-light btn-small red impoundcar-pay" data-plate="${vehicleData[vehicle].plate}"><i class="fas fa-hand-holding-usd"></i> Pay</button> `
...
function addforSellHouse(houseData) {
    ...
    var vehicleElement = `
              <li>
                    <div class="collapsible-header">
                        <i class="fas fa-home ${carIconColor}-text"></i>${houseData[house].name}
                        <span class="new badge" style="text-overflow:ellipsis;overflow:hidden;max-width:15ch;white-space: nowrap;" data-badge-caption="">$${numberWithCommas(houseData[house].price)}</span>
                    </div>
                    <div class="collapsible-body garage-body">
                        <div class="row">
                            <div class="col s12"> 
                                <ul class="collection">
                                    <li class="collection-item"><i class="fas fa-home"></i>  ${houseData[house].name}</li>`

                                        vehicleElement += `
                                        <li class="collection-item"><i class="fas fa-car"></i>   <i class="${houseData[house].nogarage == 0 ? "fas fa-times red-text" : "fas fa-check green-text"}"></i></li>
                                                            `
            ...
            vehicleElement += `
                        ...
                        <div class="row">
                            <div class="col s12 center-align">`
                vehicleElement += `<button class="waves-effect waves-light btn-small house-location-click" data-house-x=${houseData[house].x} data-house-y=${houseData[house].y} data-house-z=${houseData[house].z}><i class="fas fa-map-marker-alt"></i> Visit</button>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col s12 center-align">`
                vehicleElement += `<button class="waves-effect waves-light btn-small house-location-click" data-house-x=${houseData[house].x} data-house-y=${houseData[house].y} data-house-z=${houseData[house].z}>Contact Agent</button>
                            </div>
                        </div>
                    </div>
                </li>
            `
            $('.house-entries').append(vehicleElement);
        }
}
...
function addGPSLocations(locations) {
    ...
    for (let location of Object.keys(locations)) {
        ...
        if (houseInfo !== undefined) {
            for (let i = 0; i < houseInfo.length; i++) {
                console.log('UNDEFINED', JSON.stringify(houseInfo[i]))
                const houseMapping = {
    ...
    for (let j = 0; j < unorderedAdressess.length; j++) {
        console.log('ewn', JSON.stringify(unorderedAdressess[j]))
        let htmlData = `<li class="collection-item" data-house-type="${unorderedAdressess[j].houseType}">
                            <div>
                                ...
                                <span class="secondary-content gps-location-click" data-house-x=${unorderedAdressess[j].x} data-house-y=${unorderedAdressess[j].y} data-house-z=${unorderedAdressess[j].z} data-house-type=${unorderedAdressess[j].houseTypeId} data-house-id="${unorderedAdressess[j].houseId}"><i class="fas fa-map-marker-alt"></i></span>
                            </div>
                        </li>`
...
function addAccountInformation(accountInfo) {
    if (accountInfo) {
        ...
        for (const key of Object(licenses)) {
            ...
            licenseTable += `<tr>
                                <td>${string}</td>
                                <td><i class="${key["status"] == 0 ? "fas fa-times red-text" : "fas fa-check green-text"}"></i></td>
                            </tr>`
        }
...
function addPaySlip(accountInfo) {
    for (const key of Object(accountInfo.payslip)) {
                    var twatEntry = $(`<div class="row no-padding">
                            <div class="col s12">
                                <div class="card green darken-3 twat-card">

                                    <div class="card-action" style="padding-top: 8px;padding-right: 16px;padding-bottom: 8px;padding-left: 16px;">
                                        <span data-poster="" class="twat-reply white-text">${key["job"]}</i></span>
                                        <span class="right white-text" aria-label="" data-balloon-pos="down">$${key["amount"]}</span>
                                    </div>

                            </div>
                         </div>`);
                    $('.account-payslip').prepend(twatEntry);
                    
                }
                $('.total-paycheck').html('<b>Collect: $'+accountInfo.total+'</b>')
}
...
function addCriminalPage(item) {
    // console.log('adding page here')
    // console.log(item.message, item.phoneNumber, item.name, item.phoneNumber)
    var criminalPage = $(`
        <div class="row no-padding">
            <div class="col s12">
                <div class="card criminal darken-1 criminal-page-entry">
                    <div class="card-content black-text criminal-page-body center-align">
                        <strong>${item.message}</strong>
                    </div>
                    <div class="card-action" style="padding-top: 8px;padding-right: 16px;padding-bottom: 8px;padding-left: 16px;font-size:14px">
                        <div class="row no-padding">
                            <div class="col s6">
                                <span aria-label="Call" data-balloon-pos="up-left" data-number="${item.phoneNumber}" class="criminal-pages-call"><i class="fas fa-phone-alt fa-1x"></i> ${item.phoneNumber}</span>
                            </div>
                            
                                <span class="truncate"><i class="fas fa-user-circle fa-1x"></i> <b>Anonymous</b> <span class="pidtrash" data-adv="${item.message}" data-number="${item.phoneNumber}"><i class="fas fa-trash fa-1x"></i></span></span>
    ...
    $('.criminal-pages-entries').prepend(criminalPage);
}
...
function addMessage(item) {
    ...
    element.click(function () {
        $.post('http://hhrp-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: item.msgDisplayName }));
    });
...
function addMessageOther(item) {
    ...
    element.id = item.id;
    element.click(function () {
        $.post('http://hhrp-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: receiver, clientPhone: item.clientNumber }));
    });
    $(".messages-entries").prepend(element);
}
...
function HOUSEFilter() {
    var filter = $('#house-search').val();
    $("ul.house-entries li").each(function () {
        if ($(this).text().search(new RegExp(filter, "i")) < 0) {
            $(this).hide();
        } else {
            if (gpsFilters.includes($(this).data('house-type')))
...
function updateSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            ...
            break;
        case "control":
            $.post('http://hhrp-phone/settingsUpdateToko', JSON.stringify({tag: "controlUpdate",controls: currentBinds}));
            break;
        case "browser":
            break;
        default:
...
function ResetSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            $.post('http://hhrp-phone/settingsResetToko', JSON.stringify());
            break;
        case "control":
            $.post('http://hhrp-phone/settingsResetControls', JSON.stringify());
            break;
        case "browser":
            break;
        default:
...
async function updateTokoSettings()
{
    ...
    await delayedLog();
    $.post('http://hhrp-phone/settingsUpdateToko', JSON.stringify({
        tag: "settings",
...
$('#house-search').keyup(debounce(function () {
    HOUSEFilter();
}, 500));
...
$('#racing-create-form').on('reset', function (e) {
    $.post('http://hhrp-phone/racing:map:cancel', JSON.stringify({}));
});

$('#racing-start-tracks').on('change', function (e) {
    let selectedMap = $(this).val();
    if(maps[selectedMap] !== undefined) {
        $.post('http://hhrp-phone/racing:map:removeBlips', JSON.stringify({}));
        $.post('http://hhrp-phone/racing:map:load', JSON.stringify({ id: selectedMap}));
...
$('#racing-start').submit(function (e) {
    e.preventDefault();
    $.post('http://hhrp-phone/racing:event:start', JSON.stringify({
...
$('#racing-create-form').submit(function (e) {
    e.preventDefault();
    $.post('http://hhrp-phone/racing:map:save', JSON.stringify({
...
$("#real-estate-sell-form").submit(function (e) {
    e.preventDefault();
    $.post('http://hhrp-phone/btnAttemptHouseSale', JSON.stringify());

});

$("#real-estate-create-form").submit(function (e) {
    e.preventDefault();
    $.post('http://hhrp-phone/btnAttempCreateHouse', JSON.stringify({
        price: escapeHtml($("#real-estate-create-form #real-estate-create-price").val()),
        tier: escapeHtml($("#real-estate-create-form #real-estate-create-tier").val()),
    }));

    $('#real-estate-create-form').trigger('reset');
    $('#real-estate-create-modal').modal('close');
});

$('#real-estate-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://hhrp-phone/btnTransferHouse', JSON.stringify({
...
$('#real-estate-transfers-form').submit(function (e) {
    e.preventDefault();
    $.post('http://hhrp-phone/btnSellingHousePlayer', JSON.stringify({
        cid: escapeHtml($("#real-estate-transfers-form #real-estate-transfers-id").val()),
    }));
    $('#real-estate-transfers-form').trigger('reset');
    $('#real-estate-transfers-modal').modal('close');
});

$('#home-owner-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://hhrp-phone/btnTransferHouse2', JSON.stringify({
        cid: escapeHtml($("#home-owner-transfer-form #home-owner-transfer-id").val()),
    }));
    $('#real-estate-transfer-form').trigger('reset');
    $('#real-estate-transfer-modal').modal('close');
});
...
$("#group-manage-pay-form").submit(function (e) {
    ...
    $.post('http://hhrp-phone/payGroup', JSON.stringify({
...
$("#group-manage-rank-form").submit(function (e) {
    e.preventDefault();
    $.post('http://hhrp-phone/promoteGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-manage-bank-form").submit(function (e) {
    ...
    $.post('http://hhrp-phone/bankGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-tasks-assign-modal-form").submit(function (e) {
    ...
    $.post('http://hhrp-phone/btnGiveTaskToPlayer', JSON.stringify({
...
$("#contacts-form").submit(function (e) {
    ...
    $.post('http://hhrp-phone/newContactSubmit', JSON.stringify({
        name: clean,
...
$("#stock-form").submit(function (event) {
    event.preventDefault();
    $.post('http://hhrp-phone/stocksTradeToPlayer', JSON.stringify({
...
$("#twat-form").submit(function (event) {
    event.preventDefault();
    $.post('http://hhrp-phone/newTwatSubmit', JSON.stringify({
...
$("#call-form").submit(function (event) {
    event.preventDefault();
    $.post('http://hhrp-phone/callContact', JSON.stringify({
        name: '',
...
$("#criminal-pages-form").submit(function (event) {
    event.preventDefault();
    $.post('http://hhrp-phone/newCPostSubmit', JSON.stringify({
        advert: escapeHtml($("#criminal-pages-form #criminal-pages-body").val())
    }));
    $("#criminal-pages-form #yellcriminalow-pages-body").attr("style", "").val('')
    $('#criminal-pages-modal').modal('close');
});

$("#new-message-form").submit(function (event) {
    event.preventDefault();

    $.post('http://hhrp-phone/newMessageSubmit', JSON.stringify({
    ...
    switch (currentContainer) {
        case "message":
            setTimeout(function () {
                ...
                $.post('http://hhrp-phone/messageRead', JSON.stringify({ sender: sender, receiver: receiver, displayName: displayName }));
            }, 300);
            break;
        case "messages":
            setTimeout(function () {
                $.post('http://hhrp-phone/messages', JSON.stringify({}));
            }, 300);
            break;
    }
...
$('.racing-map-delete-confirm').click(function () {  
    ...
    $.post('http://hhrp-phone/racing:map:delete', JSON.stringify({ id: raceMap }));
    $('.racing-delete-confirm').fadeOut(150)
...
$('#real-estate-evict-modal-accept').click(function () {
    $.post('http://hhrp-phone/btnEvictHouse', JSON.stringify({}));
    $('#real-estate-evict-modal-').modal('close');
});

$('#real-estate-delete-modal-accept').click(function () {
    $.post('http://hhrp-phone/btnDeleteHouse', JSON.stringify({}));
    $('#real-estate-delete-modal-').modal('close');
    $('#real-estate-delete-modal').modal('close');
});
...
$('.house-toggle-filter').click(function () {
    let filterData = $(this).data('filter');

    if ($(this).hasClass("grey-text")) {
        if (!houseFilters.includes(filterData))
            houseFilters.push(filterData);
    }
    else
        houseFilters = houseFilters.filter(filter => filter !== filterData);
...
$('.twatter-toggle-notification').click(function () {
    ...
    $.post('http://hhrp-phone/btnNotifyToggle', JSON.stringify({}));
});

$('.account-information-toggle-pager').click(function () {
    $.post('http://hhrp-phone/btnPagerToggle', JSON.stringify({}));
...
$('.racing-entries').on('click', '.racing-entries-join', function () {
    $.post('http://hhrp-phone/racing:event:join', JSON.stringify({ identifier: $(this).data('id') }));
});

$('.keys-entries').on('click', '.manage-keys', function () {
    $.post('http://hhrp-phone/retrieveHouseKeys', JSON.stringify({}));
});

$('.keys-entries').on('click', '.remove-shared-key', function(e) {
    $.post('http://hhrp-phone/removeSharedKey', JSON.stringify({
...
$('.manage-keys-entries').on('click', '.manage-keys-remove', function () {
    $.post('http://hhrp-phone/removeHouseKey', JSON.stringify({
...
$('.criminal-pages-entries').on('click', '.criminal-pages-call', function () {
    $.post('http://hhrp-phone/callContact', JSON.stringify({
        name: '',
        number: $(this).data('number')
    }));
...
$('.criminal-pages-entries').on('click', '.pidtrash', function () {
    $.post('http://hhrp-phone/deleteCPpost', JSON.stringify({
        adv: $(this).data('adv'),
        name: '',
        number: $(this).data('number')
    }));
...
$('.group-tasks-entries').on('click', '.group-tasks-track', function () {
    $.post('http://hhrp-phone/trackTaskLocation', JSON.stringify({ TaskIdentifier: $(this).data('id') }));
});

$('.delivery-job-entries').on('click', '.delivery-job-accept', function (e) {
    $.post('http://hhrp-phone/selectedJob', JSON.stringify({ jobType: $(this).data('job-type'), jobId: $(this).data('job-id') }));
});
...
$('.garage-entries').on('click', '.garage-spawn', function (e) {
    e.preventDefault();
    $.post('http://hhrp-phone/vehspawn', JSON.stringify({ vehplate: $(this).data('plate') }));
    $.post('http://hhrp-phone/btnGarage', JSON.stringify({}));
});

$('.garage-entries').on('click', '.garage-track', function () {
    $.post('http://hhrp-phone/vehtrack', JSON.stringify({ vehplate: $(this).data('plate') }));
});

$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://hhrp-phone/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate') }));
    setTimeout(function () {
        $.post('http://hhrp-phone/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
$('.gps-entries, .keys-entries').on('click', '.gps-location-click', function () {
    $.post('http://hhrp-phone/loadUserGPS', 
    JSON.stringify({ 
    house_id: $(this).data('house-id'), 
    house_type: $(this).data('house-type'),
    x: $(this).data('house-x'),
    y: $(this).data('house-y'),
    z: $(this).data('house-z')
...
$('.house-entries, .keys-entries').on('click', '.house-location-click', function () {
    $.post('http://hhrp-phone/loadUserGPS', 
    JSON.stringify({ 
    x: $(this).data('house-x'),
    y: $(this).data('house-y'),
...
$('.contacts-entries, .call-history-entries').on('click', '.btn-contacts-call', function () {
    $.post('http://hhrp-phone/callContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
});
...
$('#confirm-modal-accept').click(function (event) {
    $.post('http://hhrp-phone/removeContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
