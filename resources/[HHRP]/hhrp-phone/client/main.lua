...
RegisterNUICallback('btnTaskGroups', function()
    local jobgrade = HHCore.PlayerData.job.grade
    local rankConverted = HHCore.PlayerData.job.grade_label

    local groupObject = {}
    table.insert(groupObject, {
        namesent = HHCore.PlayerData.job.label,
...
RegisterNUICallback('btnAttempCreateHouse', function(data, cb)
  TriggerEvent("hhrp-housing:client:createHouses", data.price, data.tier)
  loadedHouse = false
...
function hasPhone()
  ...
  if
    (
    (exports["hhrp-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
    exports["hhrp-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
    exports["hhrp-inventory"]:hasEnoughOfItem("stolens8",1,false) or
    exports["hhrp-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
    exports["hhrp-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
    exports["hhrp-inventory"]:hasEnoughOfItem("boomerphone",1,false))
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsDisabledControlJustReleased(1, 199) then
      TriggerEvent("hhrp:heHasPhone")
    end
  end
end)

RegisterNetEvent('hhrp:heHasPhone')
AddEventHandler('hhrp:heHasPhone', function()
    GotPhone()
end)


function GotPhone()
  local dead = exports["hhrp-ambulancejob"]:GetDeath()
      if not dead then
        openGuiNow()
      else
...
function openGuiNow()
  ...
  if hasPhone() then
    ...
    if HHCore.PlayerData.job.name == "realestateagent" then
     isREAgent = true
    end
...
RegisterNUICallback('newPostSubmit', function(data, cb)
  if HHCore.PlayerData.job.name ~= "taxi" and HHCore.PlayerData.job.name ~= "towtruck" and HHCore.PlayerData.job.name ~= "trucker" then
    TriggerServerEvent('phone:updatePhoneJob', data.advert)
  else
    TriggerServerEvent('phone:updatePhoneJob', HHCore.PlayerData.job.name .. " | " .. data.advert)
...
AddEventHandler('getAccountInfo', function(cash, bank, licences)
    local responseObject = {
        cash = cash,
        bank = bank,
        job = HHCore.PlayerData.job.name .. ', ' .. HHCore.PlayerData.job.grade_label,
...
RegisterNetEvent("hhrp-phone:userTwitter")

AddEventHandler("hhrp-phone:userTwitter", function(user)
  userTwitter = user
 end)
