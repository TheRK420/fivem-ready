	...
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.zones.process.coords, true) < 1 then
			if not isProcessing then
				HHCore.ShowHelpNotification(_U('Procress'))
			end
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
				...
				if not isPickingUp then
					HHCore.ShowHelpNotification(_U('Milk'))
				end

					if IsControlJustReleased(0, 38) and not isPickingUp then
						HHCore.TriggerServerCallback('crossbite_milk:haveItem', function(haveItem)
							if haveItem then
								isPickingUp = true

								HHCore.TriggerServerCallback('crossbite_milk:canPickUp', function(canPickUp)
									...
									HHCore.ShowNotification(_U('Needbuy'))
							end
