...
function OpenShop()
	...
	for k, v in pairs(HHCore.GetPlayerData().inventory) do
		local price = Config.items[v.name]

		if price and v.count > 0 then
			table.insert(elements, {
				label = ('%s - <span style="color:green;">$%s</span>'):format(v.label, HHCore.Math.GroupDigits(price)),
				name = v.name,
				price = price,
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), 'milk_shop', {
		title    = _U('Title'),
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
	...
	while true do
		...
		if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			...
			if GetDistanceBetweenCoords(coords, Config.zones.dealer.coords, true) < 2 then
				...
				HHCore.ShowHelpNotification(_U('Sell'))
