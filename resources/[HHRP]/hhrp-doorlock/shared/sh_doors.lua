hhrp = hhrp or {}
hhrp.Doors = hhrp.Doors or {}
hhrp.DoorCoords = hhrp.DoorCoords or {}
hhrp.offSet = hhrp.offSet or {}
...
function hhrp.alterState(alterNum)

    if hhrp.DoorCoords[alterNum]["lock"] == 0 then 
        hhrp.DoorCoords[alterNum]["lock"] = 1 
    else 
        hhrp.DoorCoords[alterNum]["lock"] = 0 
    end
    TriggerClientEvent("hhrp:Door:alterState",-1,alterNum,hhrp.DoorCoords[alterNum]["lock"])

end

RegisterNetEvent( 'hhrp:Door:alterState' )
AddEventHandler( 'hhrp:Door:alterState', function(alterNum,num)
	hhrp.DoorCoords[alterNum]["lock"] = num
end)

RegisterNetEvent("hhrp-doors:alterlockstateclient")
AddEventHandler("hhrp-doors:alterlockstateclient", function(doorCoords)
    hhrp.DoorCoords = doorCoords
end)
