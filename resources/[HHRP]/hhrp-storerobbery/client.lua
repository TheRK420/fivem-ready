...
RegisterNetEvent('hhrp-storerobbery:onPedDeath')
AddEventHandler('hhrp-storerobbery:onPedDeath', function(store)
    SetEntityHealth(peds[store], 0)
end)

RegisterNetEvent('hhrp-storerobbery:msgPolice')
AddEventHandler('hhrp-storerobbery:msgPolice', function(store, robber)
    --TriggerEvent('hhrp-alerts:storerob')
    TriggerEvent('hhrp-dispatch:storerobbery')
end)

RegisterNetEvent('hhrp-storerobbery:removePickup')
AddEventHandler('hhrp-storerobbery:removePickup', function(bank)
    for i = 1, #objects do 
...
RegisterNetEvent('hhrp-storerobbery:robberyOver')
AddEventHandler('hhrp-storerobbery:robberyOver', function()
    robbing = false
end)

RegisterNetEvent('hhrp-storerobbery:talk')
AddEventHandler('hhrp-storerobbery:talk', function(store, text, time)
    robbing = false
...
RegisterNetEvent('hhrp-storerobbery:rob')
AddEventHandler('hhrp-storerobbery:rob', function(i)
                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                ...
                                TriggerServerEvent('hhrp-storerobbery:pickUp', i)
                                break
                            end
                        else
                            break
...
RegisterNetEvent('hhrp-storerobbery:resetStore')
AddEventHandler('hhrp-storerobbery:resetStore', function(i)
    while not HHCoreLoaded do Wait(0) end
...
Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
        while true do
            for i = 1, #peds do
                if IsPedDeadOrDying(peds[i]) then
                    TriggerServerEvent('hhrp-storerobbery:pedDead', i)
                end
            end
            Wait(5000)
        end
            ...
            if IsPlayerFreeAiming(PlayerId()) then
                for i = 1, #peds do
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 5.0 then
                        if not robbing then
                            local canRob = nil
                            HHCore.TriggerServerCallback('hhrp-storerobbery:canRob', function(cb)
                                canRob = cb
                            end, i)
                            while canRob == nil do
                                Wait(0)
                            ...
                            if canRob == true then
                                ...
                                TriggerServerEvent('hhrp-storerobbery:notif', i)
                                ...
                                if GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 7.5 then
                                    if not IsPedDeadOrDying(peds[i]) then
                                        TriggerServerEvent('hhrp-storerobbery:rob', i)
                                        while robbing do Wait(0) if IsPedDeadOrDying(peds[i]) then robbing = false end end
                                    end
                                else
                                    ClearPedTasks(peds[i])
                            ...
                            elseif canRob == 'no_cops' then
                                ...
                                TriggerEvent('hhrp-storerobbery:talk', i, '~g~*' .. Translation[Config.Locale]['shopkeeper'] .. '* ~w~' .. Translation[Config.Locale]['robbed'], 5)
                                Wait(2500)
                            end
                        end
                    end
