    ...
    if xPlayer.job.name == 'police' or xPlayer.job.name == 'sasp' then
    	MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_reports` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(reports)
    		...
    		MySQL.Async.fetchAll("SELECT * FROM (SELECT * FROM `mdt_warrants` ORDER BY `id` DESC LIMIT 3) sub ORDER BY `id` DESC", {}, function(warrants)
    			...
    			TriggerClientEvent('hhrp-mdt:toggleVisibilty', usource, reports, warrants, officer)
    		end)
    	end)
    end
end)

RegisterServerEvent("hhrp-mdt:hotKeyOpen")
AddEventHandler("hhrp-mdt:hotKeyOpen", function()
	local usource = source
    local xPlayer = HHCore.GetPlayerFromId(source)
...
RegisterServerEvent("hhrp-mdt:getOffensesAndOfficer")
AddEventHandler("hhrp-mdt:getOffensesAndOfficer", function()
	local usource = source
	local charges = {}
	local jailtime = {}
	MySQL.Async.fetchAll('SELECT * FROM fine_types', {
		...
		TriggerClientEvent("hhrp-mdt:returnOffensesAndOfficer", usource, charges, officer)
	end)
end)

RegisterServerEvent("hhrp-mdt:performOffenderSearch")
AddEventHandler("hhrp-mdt:performOffenderSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `characters` WHERE LOWER(`firstname`) LIKE @query OR LOWER(`lastname`) LIKE @query OR CONCAT(LOWER(`firstname`), ' ', LOWER(`lastname`)) LIKE @query", {
		...
		TriggerClientEvent("hhrp-mdt:returnOffenderSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("hhrp-mdt:getOffenderDetails")
AddEventHandler("hhrp-mdt:getOffenderDetails", function(offender)
	local usource = source
	...
	MySQL.Async.fetchAll('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
		...
		MySQL.Async.fetchAll('SELECT * FROM `user_convictions` WHERE `char_id` = @id', {
			...
			MySQL.Async.fetchAll('SELECT * FROM `mdt_warrants` WHERE `char_id` = @id', {
				...
				TriggerClientEvent("hhrp-mdt:returnOffenderDetails", usource, offender)
			end)
		end)
	end)
end)

RegisterServerEvent("hhrp-mdt:getOffenderDetailsById")
AddEventHandler("hhrp-mdt:getOffenderDetailsById", function(char_id)
	local usource = source
...
RegisterServerEvent("hhrp-mdt:saveOffenderChanges")
AddEventHandler("hhrp-mdt:saveOffenderChanges", function(id, changes, identifier)
...
RegisterServerEvent("hhrp-mdt:saveReportChanges")
AddEventHandler("hhrp-mdt:saveReportChanges", function(data)
    local usource = source
	...
	sendToDiscord('hhrp- Report Logs', author .. ' Updated a report with id: '..data.id..' Title: '..data.title..' Incident: '..data.incident..'')
end)

RegisterServerEvent("hhrp-mdt:deleteReport")
AddEventHandler("hhrp-mdt:deleteReport", function(id)
    local usource = source
	local author = GetCharacterName(source)
	MySQL.Async.execute('DELETE FROM `mdt_reports` WHERE `id` = @id', {
		['@id']  = id
	})
	sendToDiscord('hhrp- Report Logs', author .. ' Deleted a report with id: '..id..'')
end)

RegisterServerEvent("hhrp-mdt:submitNewReport")
AddEventHandler("hhrp-mdt:submitNewReport", function(data)
	local usource = source
	local author = GetCharacterName(source)
	...
	MySQL.Async.insert('INSERT INTO `mdt_reports` (`char_id`, `title`, `incident`, `charges`, `author`, `name`, `date`, `jailtime`) VALUES (@id, @title, @incident, @charges, @author, @name, @date, @sentence)', {
		...
		TriggerEvent("hhrp-mdt:getReportDetailsById", id, usource)
		sendToDiscord('hhrp- Report Logs', author .. ' Created a report with id: '..data.char_id..' Title: '..data.title..' Incident: '..data.incident..' Author of report: '..author..' Date of report: '..data.date..' Name: '..data.name..'')
	end)
...
RegisterServerEvent("hhrp-mdt:sentencePlayer")
AddEventHandler("hhrp-mdt:sentencePlayer", function(jailtime, charges, char_id, fine, players)
	local usource = source
	local jailmsg = ""
	local xPlayer = HHCore.GetPlayerFromId(source)
			...
			MySQL.Async.fetchAll('SELECT * FROM `characters` WHERE `identifier` = @identifier', {
				...
				if result[1].id == char_id then
					if jailtime and jailtime > 0 then
						...
						TriggerEvent("hhrp-jail:jailPlayer", src, jailtime, jailmsg)
					end
					if fine > 0 then
						TriggerClientEvent("hhrp-mdt:billPlayer", usource, src, 'society_police', 'Fine: '..jailmsg, fine)
					end
					return
				end
			end)
...
RegisterServerEvent("hhrp-mdt:performReportSearch")
AddEventHandler("hhrp-mdt:performReportSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` LIKE @query OR LOWER(`title`) LIKE @query OR LOWER(`name`) LIKE @query OR LOWER(`author`) LIKE @query or LOWER(`charges`) LIKE @query", {
		...
		TriggerClientEvent("hhrp-mdt:returnReportSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("hhrp-mdt:performVehicleSearch")
AddEventHandler("hhrp-mdt:performVehicleSearch", function(query)
	local usource = source
	local matches = {}
	MySQL.Async.fetchAll("SELECT * FROM `owned_vehicles` WHERE LOWER(`plate`) LIKE @query", {
		...
		TriggerClientEvent("hhrp-mdt:returnVehicleSearchResults", usource, matches)
	end)
end)

RegisterServerEvent("hhrp-mdt:performVehicleSearchInFront")
AddEventHandler("hhrp-mdt:performVehicleSearchInFront", function(query)
	local usource = source
    ...
    if xPlayer.job.name == 'police' then
		MySQL.Async.fetchAll("SELECT * FROM `owned_vehicles` WHERE `plate` = @query", {
			...
			TriggerClientEvent("hhrp-mdt:toggleVisibilty", usource)
			TriggerClientEvent("hhrp-mdt:returnVehicleSearchInFront", usource, result, query)
		end)
	end
end)

RegisterServerEvent("hhrp-mdt:getVehicle")
AddEventHandler("hhrp-mdt:getVehicle", function(vehicle)
	local usource = source
	MySQL.Async.fetchAll("SELECT * FROM `characters` WHERE `identifier` = @query", {
		...
		TriggerClientEvent("hhrp-mdt:returnVehicleDetails", usource, vehicle)
	end)
end)

RegisterServerEvent("hhrp-mdt:getWarrants")
AddEventHandler("hhrp-mdt:getWarrants", function()
	local usource = source
	MySQL.Async.fetchAll("SELECT * FROM `mdt_warrants`", {}, function(warrants)
		...
		TriggerClientEvent("hhrp-mdt:returnWarrants", usource, warrants)
	end)
end)

RegisterServerEvent("hhrp-mdt:submitNewWarrant")
AddEventHandler("hhrp-mdt:submitNewWarrant", function(data)
	local usource = source
	...
	MySQL.Async.insert('INSERT INTO `mdt_warrants` (`name`, `char_id`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (@name, @char_id, @report_id, @report_title, @charges, @date, @expire, @notes, @author)', {
		...
		TriggerClientEvent("hhrp-mdt:completedWarrantAction", usource)
		sendToDiscord('hhrp- Warrant Logs', data.author .. ' Created a Warrant with id: '..data.name..' Report id: '..data.report_id..' Title: '..data.report_title..' Author of report: '..data.author..' Date of report: '..data.date..'')
	end)
end)

RegisterServerEvent("hhrp-mdt:deleteWarrant")
AddEventHandler("hhrp-mdt:deleteWarrant", function(id)
	local usource = source
	local author = GetCharacterName(source)
	MySQL.Async.execute('DELETE FROM `mdt_warrants` WHERE `id` = @id', {
		...
		TriggerClientEvent("hhrp-mdt:completedWarrantAction", usource)
		sendToDiscord('hhrp- Warrant Logs', author .. ' Deleted a warrant with id: '..id..'')
	end)
end)

RegisterServerEvent("hhrp-mdt:getReportDetailsById")
AddEventHandler("hhrp-mdt:getReportDetailsById", function(query, _source)
	...
	MySQL.Async.fetchAll("SELECT * FROM `mdt_reports` WHERE `id` = @query", {
		...
		if result and result[1] then
			...
			TriggerClientEvent("hhrp-mdt:returnReportDetails", usource, result[1])
		end
	end)
end)
