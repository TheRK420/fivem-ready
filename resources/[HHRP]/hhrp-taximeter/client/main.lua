...
AddEventHandler('hhrp:playerLoaded', function(xPlayer)
  currentJob = HHCore.GetPlayerData().job.name
end)

RegisterNetEvent('hhrp:setJob')
AddEventHandler('hhrp:setJob', function(xPlayer)
  currentJob = HHCore.GetPlayerData().job.name
end)

RegisterNetEvent("hhrp_taximeter:updatePassenger")
AddEventHandler("hhrp_taximeter:updatePassenger", function(attributes)
  meterAttrs = attributes
  meterOwner = false
  updateMeter()
end)

RegisterNetEvent("hhrp_taximeter:updateLocation")
AddEventHandler("hhrp_taximeter:updateLocation", function()
  lastLocation = GetEntityCoords(GetPlayerPed(-1))
end)



RegisterNetEvent("hhrp_taximeter:resetMeter")
AddEventHandler("hhrp_taximeter:resetMeter", function()
  resetMeter()
  updateMeter()
end)
...
function hasMeterAppropriateJob()
  if not Config.RestrictToCertainHHCoreJobs then
    return true
  end
...
function updatePassengerMeters()
  ...
  for index,player in ipairs(players) do
    ...
    if not has_value(playersInVehicle, playerId) then
      TriggerServerEvent('hhrp_taximeter:updatePassengerMeters', playerId, meterAttrs)
...
function resetMeter()
  ...
  for i, player in ipairs(playersInVehicle) do
    TriggerServerEvent('hhrp_taximeter:resetPassengerMeters', player)
  end
end

function updatePassengerLocations()
  for i, player in ipairs(playersInVehicle) do
    TriggerServerEvent('hhrp_taximeter:updatePassengerLocation', player)
  end
end
