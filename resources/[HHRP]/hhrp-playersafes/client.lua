...
function MFS:Awake(...)
    while not HHCore do Citizen.Wait(0); end
    while not HHCore.IsPlayerLoaded() do Citizen.Wait(0); end
    self.PlayerData = HHCore.GetPlayerData()
    HHCore.TriggerServerCallback('MF_PlayerSafes:GetStartData', function(retVal,retData) self.dS = true; self.cS = retVal; self:Start(retData); end)
end
  ...
  while true do
    ...
    if closestDist and closestDist < self.DrawTextDist and not self.UsingSafe and ((not self.Instance and closestSafe.instance == 'false') or (self.Instance and self.Instance == closestSafe.instance)) then
      ...
      if IsControlJustPressed(0, 38) and closestDist <= self.InteractDist then
        HHCore.TriggerServerCallback('MF_PlayerSafes:TryAccessSafe',function(inUse)
          if inUse then
            HHCore.ShowNotification("Somebody else is using this safe.")
          else
            if not isOwner then
              ...
              self.NUIClosed = false
              self.UsingSafe = closestSafe
              -- HHCore.TriggerServerCallback('MF_PlayerSafes:GetSafeInventory', function(inventory)
              --   TriggerEvent("hhrp_inventoryhud:openPropertyInventory", inventory, closestSafe)
              -- end, closestSafe.safeid)
              TriggerEvent("server-inventory-open", "1", "Plsafe: "..closestSafe)      
            end
          end
        end,closestSafe.safeid)
...
function MFS:EndMinigame(didWin)
  ...
  if didWin then
    -- HHCore.TriggerServerCallback('MF_PlayerSafes:GetSafeInventory', function(inventory)
    --   TriggerEvent("hhrp_inventoryhud:openPropertyInventory", inventory, safe)
    -- end, safe.safeid)  
    TriggerEvent("server-inventory-open", "1", "Plsafe: "..safe) 
    self.NUIClosed = false
    self.UsingSafe = safe
...
function MFS:DoNotifyPolice(pos)
  Citizen.CreateThread(function(...)
    ...
    HHCore.ShowNotification("Somebody reported suspicious activity at "..nearStreet..". [~g~LEFTALT~s~]")
...
function MFS:EnterInstance(instance)  
  ...
  HHCore.TriggerServerCallback('MF_PlayerSafes:GetInstanceOwner', function(identifier)
...
AddEventHandler('hhrp_inventoryhud:closeInventory', function(...) MFS.NUIClosed = true; end)
