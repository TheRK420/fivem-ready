...
function MFS:NotifyPolice(pos)
  Citizen.CreateThread(function(...)
    for k,v in pairs(HHCore.GetPlayers()) do
      local xPlayer = HHCore.GetPlayerFromId(v)
      while not xPlayer do xPlayer = HHCore.GetPlayerFromId(v); Citizen.Wait(0); end
...
HHCore.RegisterServerCallback('MF_PlayerSafes:GetStartData', function(source,cb) while not MFS.dS or not MFS.wDS do Citizen.Wait(0); end; cb(MFS.cS,MFS.Safes); end)
HHCore.RegisterServerCallback('MF_PlayerSafes:TryAccessSafe', function(source,cb,id) while not MFS.dS or not MFS.wDS do Citizen.Wait(0); end; cb(MFS:TryUseSafe(id)); end)
HHCore.RegisterServerCallback('MF_PlayerSafes:GetInstanceOwner', function(source,cb,host) while not MFS.dS or not MFS.wDS do Citizen.Wait(0); end cb(MFS:GetHostId(host)); end)
Citizen.CreateThread(function(...) MFS:Awake(...); end)

function MFS:GetHostId(host)
  local tick = 0 
  local xPlayer = HHCore.GetPlayerFromId(host) 
  while not xPlayer and tick < 1000 do 
    xPlayer = HHCore.GetPlayerFromId(host)
    tick = tick + 1
    Citizen.Wait(0)
  end; 
  local id
